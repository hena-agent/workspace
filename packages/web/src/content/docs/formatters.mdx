---
title: Formatters
description: Hena Agent uses language specific formatters.
---

Hena Agent can format files after they are written or edited using language-specific formatters. Formatters are disabled by default; enable them in your config before Hena Agent will run them.

---

## Built-in

Hena Agent comes with several built-in formatters for popular languages and frameworks. Below is a list of the formatters, supported file extensions, and commands or config options it needs.
...
When formatters are enabled, Hena Agent will use `prettier` for matching files if your project has `prettier` in `package.json`.

---

## How it works

When Hena Agent writes or edits a file and formatters are enabled, it:
...
You can enable and customize formatters through the `formatter` section in your Hena Agent config.
