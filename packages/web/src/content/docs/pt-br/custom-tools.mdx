...
description: Crie ferramentas que o LLM pode chamar no Hena Agent.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do hena-agent, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.hena-agent/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/hena-agent/tools/`.
