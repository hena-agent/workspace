...
// etc.).
//
// The visual language mirrors the Hena Agent app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Hena Agent is now connected to ${escapeHtml(provider)}.` : "Hena Agent is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Hena Agent couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Hena Agent couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Hena Agent.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Hena Agent</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Hena Agent couldn't finish connecting to "+PROVIDER+"."):"Hena Agent couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Hena Agent."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Hena Agent is now connected to "+PROVIDER+"."):"Hena Agent is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Hena Agent wordmark uses the shared application logo geometry.
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="Hena Agent" role="img">
