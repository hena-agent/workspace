...
import { SessionV1 } from "@hena-agent/core/v1/session"
...
import { LLMEvent, LLMResponse } from "@hena-agent/llm"
import { RequestExecutor } from "@hena-agent/llm/route"
...
const zenURL = (connection: string) => `https://console.hena.dev/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.HENA_AGENT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["hena-agent", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("HENA_AGENT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("HENA_AGENT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["hena-agent", "native", "oauth", "tool-loop"],
  ...
  {
    id: "hena-agent-proxy",
    name: "Hena Agent proxy",
    providerID: ProviderV2.ID.henaAgent,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["hena-agent", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.HENA_AGENT_RECORD_CONSOLE_TOKEN && process.env.HENA_AGENT_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.henaAgent,
        name: "Hena Zen",
        env: ["HENA_AGENT_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.HENA_AGENT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.HENA_AGENT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.HENA_AGENT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["hena-agent", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("HENA_AGENT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("HENA_AGENT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("HENA_AGENT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set HENA_AGENT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the Hena Agent LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "hena-agent.json"),
      JSON.stringify({ $schema: "https://hena.dev/config.json", ...scenario.config(model) }),
    ),
  )
