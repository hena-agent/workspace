...
The Hena server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@hena/llm` tool loop after replacing its
...
- plugins get global "hena" instance like in that post i showed
- hena instance has stuff like `hena.session.prompt()` or
  `hena.tool.register({...})`
...
- decide whether to preserve deprecated `@hena/llm` orchestration exports
