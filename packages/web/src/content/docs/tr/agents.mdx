...
hena'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
hena, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Hena, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Hena iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Harici dokümanlar ve bağımlılık araştırmaları için salt okunur bir agent. Bir bağımlılık repository'sini Hena'un yönetilen cache'ine clone etmeniz, kütüphane kaynak kodunu incelemeniz veya workspace'inizi değiştirmeden yerel kodu upstream implementasyonlarla karşılaştırmanız gerektiğinde bunu kullanın.
...
Agent'ları `hena.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse hena modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global hena yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
hena yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Hena Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `hena/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `hena models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/hena-agent/hena).
:::
