...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. Hena est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans Hena.
...
:::note
Cet outil n'est disponible que lorsque `HENA_EXPERIMENTAL_LSP_TOOL=true` (ou `HENA_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur Hena ou lorsque la variable d'environnement `HENA_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de Hena :
