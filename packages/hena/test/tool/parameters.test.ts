...
describe("tool parameters", () => {
  ...
  describe("todo", () => {
    ...
    test("accepts new todos without an ID", () => {
      expect(accepts(Todo, { todos: [{ content: "do x", status: "pending", priority: "medium" }] })).toBe(true)
    })
  ...
  describe("websearch", () => {
    test("accepts query", () => {
      expect(parse(WebSearch, { query: "hena" }).query).toBe("hena")
    })
  })
