...
Mit Hena kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Hena das `hena`-Theme.
...
Hena liefert mehrere eingebaute Themes mit.
...
- Hena optisch ans Terminal anpassen wollen
...
Hena unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/hena/themes/*.json` oder `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.hena/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.hena/themes/*.json`
