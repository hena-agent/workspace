---
title: Policies
description: Control which configured resources Hena Agent may use.
---

Policies control whether Hena Agent may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `hena-agent.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Hena Agent may use a resource such as an LLM provider.
...
Hena Agent currently supports one policy action:
