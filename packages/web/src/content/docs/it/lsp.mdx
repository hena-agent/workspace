---
title: Server LSP
description: Hena Agent si integra con i tuoi server LSP.
---

Hena Agent può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Hena Agent include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `HENA_AGENT_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e hena-agent apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di Hena Agent.
