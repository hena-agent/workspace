---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Hena zu erweitern.
---

Mit Plugins können Sie Hena erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Hena zu ändern.
...
- `.hena/plugins/` – Plugins auf Projektebene
- `~/.config/hena/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/hena/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/hena/hena.json`)
2. Projektkonfiguration (`hena.json`)
3. Globales Plugin-Verzeichnis (`~/.config/hena/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.hena/plugins/`)
...
Hena führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Hena-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Hena erweitern können.
...
:::note
Wenn Sie die Hena-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Hena `.env`-Dateien liest:
...
Plugins können Hena auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Hena aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Hena zur Verfügung.
