---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Hena Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena Agent bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Hena Agent-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/hena-agent/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Hena Agent-konfigurationen.
...
Hena Zen er en liste over modeller leveret af Hena Agent-teamet, som er blevet testet og verificeret til at fungere godt med Hena Agent. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Hena Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg hena-agent og gå til [hena.dev/auth](https://hena.dev/auth).
...
Det fungerer som alle andre udbydere i Hena Agent og er helt valgfrit at bruge.

---

## Hena Go

Hena Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Hena Agent-teamet, som er testet og verificeret til at fungere godt med Hena Agent.

1. Kør kommandoen `/connect` i TUI, vælg `Hena Go`, og gå til [hena.dev/auth](https://hena.dev/zen).
...
Sådan bruger du Amazon Bedrock med Hena Agent:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører hena-agent:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `hena-agent.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Hena Agent understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere hena-agent til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at hena-agent skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem hena-agent-konfigurationen.
...
gemme tokenet i hena-agent auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Hena Agent bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Hena Agent
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`hena-agent.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Hena Agent:
...
Sådan bruger du Google Vertex AI med Hena Agent:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører Hena Agent.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem hena-agent, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Hena Agent-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Hena Agent-samtale opført som en separat session.
...
Du kan konfigurere hena-agent til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere hena-agent til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere hena-agent til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Hena Agent. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Hena Agent:
...
3. Kopier API-nøglen til brug i Hena Agent.
...
6. **Vigtigt**: Før du bruger skymodeller i Hena Agent, skal du hente modelinformationen lokalt:
...
Hena Zen er en liste over testede og verificerede modeller leveret af Hena Agent-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Hena Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Hena Zen**.

   ```txt
   /connect
   ```

3. Indtast Hena Agent API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem hena-agent-konfigurationen.
...
5. Du kan også tilpasse dem gennem hena-agent-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Hena Agent:
...
Du kan også tilpasse modeller gennem hena-agent-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med Hena Agent. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `hena-agent.json`-filen i projektkataloget:
...
`limit`-felterne lader Hena Agent forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `hena-agent auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek hena-agent-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i hena-agent-konfigurationen.
