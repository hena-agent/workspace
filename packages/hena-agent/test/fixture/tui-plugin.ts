...
import type { HostPluginApi } from "@hena-agent/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createHenaAgentClient({
  ...
  let selected = opts.theme?.selected ?? "hena-agent"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "hena-agent.default",
