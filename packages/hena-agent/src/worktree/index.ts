...
import { path } from "@hena-agent/core/effect/app-node-platform"
...
import type { ProjectV2 } from "@hena-agent/core/project"
...
import { WorktreeEvent } from "@hena-agent/schema/worktree-event"
...
export class Service extends Context.Service<Service, Interface>()("@hena-agent/Worktree") {}
  ...
  Effect.gen(function* () {
    ...
    const candidate = Effect.fn("Worktree.candidate")(function* (input: {
      ...
      for (const attempt of Array.from({ length: MAX_NAME_ATTEMPTS }, (_, i) => i)) {
        ...
        const branch = input.detached ? undefined : `hena-agent/${name}`
