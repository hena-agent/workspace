...
import type { LLMClientShape } from "@hena-agent/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("hena-agent"))
    return { type: "unsupported", reason: "provider is not Hena Agent, OpenAI, or Anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @hena-agent/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains Hena Agent-owned. The native runtime only adapts
      // the @hena-agent/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
