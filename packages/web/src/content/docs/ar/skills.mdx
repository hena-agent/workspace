...
تتيح مهارات الوكيل لـ Hena Agent اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Hena Agent في هذه المواقع:

- إعدادات المشروع: `.hena-agent/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/hena-agent/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Hena Agent صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.hena-agent/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/hena-agent/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.hena-agent/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Hena Agent المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `hena-agent.json`:
...
**للوكلاء المدمجين** (في `hena-agent.json`):
