...
hena-agent ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak hena-agent kendi `hena-agent` temasını kullanır.
...
hena-agent birden fazla yerleşik temayla gelir.
...
- hena-agent'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
hena-agent, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/hena-agent/themes/*.json` veya `$XDG_CONFIG_HOME/hena-agent/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.hena-agent/themes/*.json`
4. **Geçerli çalışma dizini** - `./.hena-agent/themes/*.json`
