...
For at fejlfinde problemer med Hena, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\hena\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `hena --log-level DEBUG`.

---

## Lagring

hena gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/hena/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\hena`
...
Hena Desktop kører en lokal Hena-server (`hena-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Hena`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/hena/hena.jsonc` (eller `~/.config/hena/hena.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/hena/hena.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\hena\hena.jsonc`
...
Hena kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/hena/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\hena\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Hena kan genopbygge den.

1. Afslut Hena Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/hena`
- **Linux**: slet `~/.cache/hena` (eller kør `rm -rf ~/.cache/hena`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\hena`

3. Start Hena Desktop på ny.

---

### Løs problemer med serverforbindelse

Hena Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `hena.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `HENA_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `HENA_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Hena Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Hena.

---

### Meddelelser vises ikke

Hena Desktop viser kun systemvarsler når:

- varsler er aktiveret for Hena i dine OS-indstillinger, og
...
1. Afslut Hena Desktop.
2. Find og slet disse filer (de findes i Hena Desktop-appens datamappe):

- `hena.settings.dat` (skrivebordsstandardserver URL)
- `hena.global.dat` og `hena.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Hena:
...
For at finde ud af hvilke modeller du har adgang til, kør `hena models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\hena`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. hena installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\hena`

2. Start hena på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge hena til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
hena vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
