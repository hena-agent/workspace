---
title: Konfigurasjon
description: Ved å bruke hena-agent JSON-konfigurasjonen.
---

Du kan konfigurere Hena Agent ved å bruke en JSON konfigurasjonsfil.

---

## Format

Hena Agent støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "hena-agent"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/hena-agent`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/hena-agent/hena.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`HENA_AGENT_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`hena-agent.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.hena-agent` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`HENA_AGENT_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.hena-agent` og `~/.config/hena-agent` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/hena-agent`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Hena Agent-konfigurasjon i `~/.config/hena-agent/hena.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `hena-agent.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Hena Agent starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `HENA_AGENT_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `HENA_AGENT_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.hena-agent` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.hena-agent`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`hena.dev/config.json`**](https://hena.dev/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `hena-agent serve` og `hena-agent web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Hena Agent-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `hena-agent.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Hena Agent-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Hena Agent å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/hena-agent/agents/` eller `.hena-agent/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Hena Agent falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`hena-agent run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/hena-agent/commands/` eller `.hena-agent/commands/`. [Les mer her](/docs/commands).
...
Hena Agent vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Hena Agent alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Hena Agent med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.hena-agent/plugins/` eller `~/.config/hena-agent/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Hena Agent til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
