...
Создайте `.hena-agent/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию hena-agent или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем hena-agent [config](/docs/config):
...
- Глобальный: `~/.config/hena-agent/commands/`
- Для каждого проекта: `.hena-agent/commands/`
...
hena-agent включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
