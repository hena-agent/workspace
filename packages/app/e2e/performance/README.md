...
Set `HENA_AGENT_PERFORMANCE_TRACE_DIR` to emit a standard Chrome DevTools trace for every benchmark page automatically:

```sh
HENA_AGENT_PERFORMANCE_TRACE_DIR=/tmp/hena-agent-performance-traces \
...
INP analysis requires a trace with a supported navigation/interaction insight. Selector statistics require a trace captured with `HENA_AGENT_PERFORMANCE_SELECTOR_TRACE=1`.
