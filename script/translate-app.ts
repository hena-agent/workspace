...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "hena/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".hena/glossary/zh-cn.md"
  if (locale === "zht") return ".hena/glossary/zh-tw.md"
  return `.hena/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Hena did not report a session ID.")
  return match[1]
}
...
export function sessionModels(value: unknown) {
  ...
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Hena session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Hena runs for 'all' (default: 4)
      --model <provider/id>  Hena model (default: hena/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Hena
...
`)
  ...
  if (failed.length) console.error(`\nHena failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.HENA_DISABLE_PROJECT_CONFIG = "1"
  env.HENA_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["hena", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.HENA_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["hena", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.HENA_CONFIG
  delete env.HENA_CONFIG_DIR
  delete env.HENA_CONFIG_CONTENT
  delete env.HENA_PERMISSION
  delete env.HENA_AUTO_SHARE
  return env
}
