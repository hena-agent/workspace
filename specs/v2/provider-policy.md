...
Both `action` and `resource` use Hena's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.hena` files is intentionally deferred until `.hena` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether Hena permits an operation through its managed execution paths.
