...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "hena.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "hena.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "hena.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "hena.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["hena.json", "hena.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "hena.json"),
        path.join(project, "hena.jsonc"),
      ])
    })
  })
