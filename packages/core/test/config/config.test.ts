...
import { ConfigMigrateV1 } from "@hena/core/v1/config/migrate"
...
import { Policy } from "@hena/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads Hena JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "hena.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "hena.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "hena.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "hena.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .hena configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".hena"), { recursive: true })
            await fs.mkdir(path.join(directory, ".hena"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "hena.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "hena.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "hena.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "hena.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "hena.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".hena", "hena.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(path.join(directory, ".hena", "hena.jsonc"), JSON.stringify({ $schema: "directory-dot" })),
            ])
          })
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".hena")),
              AbsolutePath.make(path.join(directory, ".hena")),
            ])
