---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Hena Agent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hena Agent fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Hena Agent avvia la TUI per la directory corrente.
...
Quando usi la TUI di Hena Agent, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Hena Agent. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Hena Agent. _Alias_: `/quit`, `/q`
...
Questo è separato da `hena-agent.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Hena Agent attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `hena-agent.default`.
