...
describe("hena-agent acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ henaAgent }))
        ...
        expect(initialized.agentInfo?.name).toBe("Hena Agent")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ henaAgent })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("hena-agent-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "hena-agent-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.HENA_AGENT_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ henaAgent }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("hena-agent-login")
