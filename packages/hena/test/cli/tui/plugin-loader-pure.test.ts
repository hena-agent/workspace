...
`,
  ...
  const pure = process.env.HENA_PURE
  const meta = process.env.HENA_PLUGIN_META_FILE
  process.env.HENA_PURE = "1"
  process.env.HENA_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.HENA_PURE
    } else {
      process.env.HENA_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.HENA_PLUGIN_META_FILE
    } else {
      process.env.HENA_PLUGIN_META_FILE = meta
    }
  }
})
