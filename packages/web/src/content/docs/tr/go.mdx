...
Hena Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Hena Agent'daki diğer sağlayıcılar gibi çalışır. Hena Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Hena Agent'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Hena Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Hena Go, Hena Agent'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Hena Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Hena Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Hena Go'ya abone olabilir.
:::
...
Hena Go aşağıdaki limitleri içerir:
...
Hena Agent yapılandırmanızdaki [model id](/docs/config/#models) formatı `hena-agent-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `hena-agent-go/kimi-k3` kullanmalısınız.
...
Hena Go'yu şu amaçlarla oluşturduk:
...
4. Hena Agent ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
