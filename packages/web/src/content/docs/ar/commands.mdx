...
أنشئ الملف `.hena/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Hena أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Hena:
...
- على مستوى النظام: `~/.config/hena/commands/`
- لكل مشروع: `.hena/commands/`
...
يتضمن hena عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
