...
O Hena Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Hena Agent. Você assina o Hena Go e
...
O Hena Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Hena Go funciona como qualquer outro provedor no Hena Agent.

1. Você entra no **<a href={console}>Hena Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Hena Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Hena Go.
:::
...
O Hena Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Hena Agent
usa o formato `hena-agent-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`hena-agent-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Hena Agent também.
