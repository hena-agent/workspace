---
title: TUI
description: استخدام واجهة المستخدم TUI في Hena Agent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Hena Agent واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Hena Agent إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Hena Agent في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Hena Agent. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Hena Agent. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `hena-agent.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Hena Agent بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `hena-agent.default`.
