...
Agent Skills erlauben Hena, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Hena sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Hena vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.hena/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/hena/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.hena/skills/git-release/SKILL.md` zum Beispiel so:
...
Hena listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `hena.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `hena.json`):
