# Remove `packages/hena-agent/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/hena-agent/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the Hena Agent legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/hena-agent/src/storage/db.ts` are concentrated in 22 source files:

- `packages/hena-agent/src/account/repo.ts`
- `packages/hena-agent/src/cli/cmd/db.ts`
- `packages/hena-agent/src/cli/cmd/import.ts`
- `packages/hena-agent/src/cli/cmd/stats.ts`
- `packages/hena-agent/src/control-plane/workspace.ts`
- `packages/hena-agent/src/index.ts`
- `packages/hena-agent/src/node.ts`
- `packages/hena-agent/src/permission/index.ts`
- `packages/hena-agent/src/project/project.ts`
- `packages/hena-agent/src/server/projectors.ts`
- `packages/hena-agent/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/hena-agent/src/server/shared/fence.ts`
- `packages/hena-agent/src/session/message-v2.ts`
- `packages/hena-agent/src/session/projectors.ts`
- `packages/hena-agent/src/session/prompt.ts`
- `packages/hena-agent/src/session/session.ts`
- `packages/hena-agent/src/session/todo.ts`
- `packages/hena-agent/src/share/share-next.ts`
- `packages/hena-agent/src/storage/db.ts`
- `packages/hena-agent/src/sync/index.ts`
- `packages/hena-agent/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy Hena Agent database wrapper; `packages/hena-agent/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the Hena Agent wrapper.
...
Status: Completed. `SyncEvent` and the Hena Agent projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy Hena Agent database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy Hena Agent database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy Hena Agent database wrapper.
...
All migration groups are complete or superseded. `packages/hena-agent/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No Hena Agent data-migration group remains.

The previous Hena Agent `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate Hena Agent data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/hena-agent`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/hena-agent/src`
- `bun typecheck` from `packages/hena-agent`
- Relevant package tests from `packages/hena-agent`, not the repo root
