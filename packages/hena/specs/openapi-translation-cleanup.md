...
Trim `packages/hena/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/hena/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/hena`.
- `[x]` `bun typecheck` from `packages/hena`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/hena/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/hena`.
- `[x]` `bun dev generate > /tmp/hena-openapi.json` from `packages/hena`.
...
- Edit `packages/hena/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/hena-openapi.json` from `packages/hena`.
...
- Check source schemas in files like `packages/hena/src/session/schema.ts`, `packages/hena/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/hena/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/hena/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
