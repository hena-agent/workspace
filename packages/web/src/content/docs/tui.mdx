---
title: TUI
description: Using the Hena terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hena provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Hena starts the TUI for the current directory.
...
When using the Hena TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Hena. Allows you to select from available providers and add their API keys.
...
This is separate from `hena.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Hena waits after the leader key. Defaults to `2000`.
...
Use `HENA_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `hena.default`.
