...
Both `action` and `resource` use Hena Agent's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.hena-agent` files is intentionally deferred until `.hena-agent` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether Hena Agent permits an operation through its managed execution paths.
