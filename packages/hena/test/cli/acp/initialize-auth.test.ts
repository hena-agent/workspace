...
describe("hena acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ hena }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ hena }))
        ...
        expect(initialized.agentInfo?.name).toBe("Hena")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ hena }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ hena })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("hena-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "hena-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.HENA_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ hena }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("hena-login")
