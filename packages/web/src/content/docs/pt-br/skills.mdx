...
As habilidades do agente permitem que o hena-agent descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O hena-agent pesquisa nesses locais:

- Configuração do projeto: `.hena-agent/skills/<name>/SKILL.md`
- Configuração global: `~/.config/hena-agent/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o hena-agent sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.hena-agent/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/hena-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.hena-agent/skills/git-release/SKILL.md` assim:
...
O hena-agent lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `hena-agent.json`:
...
**Para agentes embutidos** (em `hena-agent.json`):
