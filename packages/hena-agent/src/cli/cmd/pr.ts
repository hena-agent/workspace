...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run Hena Agent",
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        const sessionMatch = prInfo.body.match(/https:\/\/hena\.ai\/s\/([a-zA-Z0-9_-]+)/)
        if (sessionMatch) {
          const sessionUrl = sessionMatch[0]
          UI.println(`Found Hena Agent session: ${sessionUrl}`)
          UI.println(`Importing session...`)

          const importResult = yield* Effect.promise(() =>
            Process.text(["hena-agent", "import", sessionUrl], { nothrow: true }),
          )
    ...
    UI.println()
    UI.println("Starting Hena Agent...")
    UI.println()

    const henaAgentArgs = sessionId ? ["-s", sessionId] : []
    const code = yield* Effect.promise(
      () =>
        Process.spawn(["hena-agent", ...henaAgentArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`hena-agent exited with code ${code}`))
  }),
})
