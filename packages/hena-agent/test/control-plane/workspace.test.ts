...
import { EventSequenceTable } from "@hena-agent/core/event/sql"
...
import { WorkspaceTable } from "@hena-agent/core/control-plane/workspace.sql"
...
const originalEnv = {
  HENA_AGENT_AUTH_CONTENT: process.env.HENA_AGENT_AUTH_CONTENT,
  HENA_AGENT_EXPERIMENTAL_WORKSPACES: process.env.HENA_AGENT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.HENA_AGENT_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@hena-agent.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.HENA_AGENT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=hena-agent-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.HENA_AGENT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.HENA_AGENT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.HENA_AGENT_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=hena-agent-test")
