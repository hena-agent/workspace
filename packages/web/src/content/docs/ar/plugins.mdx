---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Hena.
---

تتيح لك الإضافات توسيع Hena عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Hena.
...
- `.hena/plugins/` - إضافات على مستوى المشروع
- `~/.config/hena/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/hena/node_modules/`.
...
1. إعدادات عامة (`~/.config/hena/hena.json`)
2. إعدادات المشروع (`hena.json`)
3. دليل الإضافات العام (`~/.config/hena/plugins/`)
4. دليل إضافات المشروع (`.hena/plugins/`)
...
يشغّل Hena الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ hena للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع Hena.
...
:::note
إذا كنت تستخدم تطبيق Hena لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى hena:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ hena استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ hena إلى جانب الأدوات المضمنة.
