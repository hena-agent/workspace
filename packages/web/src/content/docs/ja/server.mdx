---
title: サーバー
description: HTTP 経由で Hena Agent サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`hena-agent serve` コマンドは、Hena Agent クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `hena-agent.local` |
...
HTTP 基本認証でサーバーを保護するには、`HENA_AGENT_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `hena-agent` になるか、`HENA_AGENT_SERVER_USERNAME` を設定してオーバーライドします。これは、`hena-agent serve` と `hena-agent web` の両方に当てはまります。
...
`hena-agent` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Hena Agent サーバーを使用して、プログラムで Hena Agent と対話します。
:::

`hena-agent serve` を実行してスタンドアロンサーバーを起動できます。すでに Hena Agent TUI を実行している場合でも、`hena-agent serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Hena Agent [IDE](/docs/ide) プラグインによって使用されます。
