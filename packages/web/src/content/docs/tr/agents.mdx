...
hena-agent'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
hena-agent, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Hena Agent, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Hena Agent iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `hena-agent.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.hena-agent/agents/`
...
Sıcaklık belirtilmezse hena-agent modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global hena-agent yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
hena-agent yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Hena Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `hena-agent/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `hena-agent models` komutunu çalıştırın.
:::
