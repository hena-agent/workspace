...
Opret `.hena-agent/commands/test.md`:
...
Du kan tilføje brugerdefinerede kommandoer gennem Hena Agent-konfigurationen eller ved at oprette markdown-filer i `commands/`-mappen.

---

### JSON

Brug indstillingerne `command` i din Hena Agent [config](/docs/config):
...
- Globalt: `~/.config/hena-agent/commands/`
- Pr. projekt: `.hena-agent/commands/`
...
hena-agent indeholder flere indbyggede kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
