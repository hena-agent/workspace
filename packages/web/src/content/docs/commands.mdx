...
Create `.hena-agent/commands/test.md`:
...
You can add custom commands through the Hena Agent config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Hena Agent [config](/docs/config):
...
- Per-project: `.hena-agent/commands/`
...
hena-agent includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
