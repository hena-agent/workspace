---
title: Formateurs
description: Hena Agent utilise des formateurs spécifiques au langage.
---

Hena Agent formate automatiquement les fichiers après leur écriture ou leur modification à l'aide de formateurs spécifiques au langage. Cela garantit que le code généré suit les styles de code de votre projet.

---

## Formateurs intégrés

Hena Agent est livré avec plusieurs formateurs intégrés pour les langages et frameworks populaires. Vous trouverez ci-dessous une liste des formateurs, des extensions de fichiers prises en charge et des commandes ou options de configuration dont il a besoin.
...
Ainsi, si votre projet a `prettier` dans votre `package.json`, Hena Agent l'utilisera automatiquement.

---

## Comment ça marche

Lorsque Hena Agent écrit ou modifie un fichier, il :
...
Vous pouvez personnaliser les formateurs via la section `formatter` de votre configuration Hena Agent.
...
```json title="hena-agent.json" {4-14}
{
  "$schema": "https://hena.dev/config.json",
  "formatter": {
    "prettier": {
