---
title: SDK
description: Client JS de type sécurisé pour le serveur Hena.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

Le SDK hena JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler hena par programme.
...
Créez une instance de hena :

```javascript
import { createHena } from "@hena/sdk"

const { client } = await createHena()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `hena.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createHena } from "@hena/sdk"

const agent = await createHena({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${agent.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de hena, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@hena/sdk"
```
