...
Wenn Hena Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\hena\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `hena --log-level DEBUG`.

---

## Speicher

hena speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/hena/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\hena`
...
Hena Desktop startet im Hintergrund einen lokalen Hena-Server (`hena-cli`-Sidecar).
...
- Nur macOS: `Hena`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/hena/hena.jsonc` (oder `~/.config/hena/hena.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/hena/hena.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\hena\hena.jsonc`
...
Hena kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/hena/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\hena\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Hena ihn neu aufbauen kann.

1. Beende Hena Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/hena`
- **Linux**: loesche `~/.cache/hena` (oder fuehre aus: `rm -rf ~/.cache/hena`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\hena`

3. Starte Hena Desktop neu.

---

### Server-Verbindungsprobleme beheben

Hena Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `hena.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `HENA_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `HENA_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Hena Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Hena-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Hena Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Hena in den OS-Einstellungen aktiviert sind, und
...
1. Beende Hena Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Hena Desktop):

- `hena.settings.dat` (desktop default server URL)
- `hena.global.dat` und `hena.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Hena hast:
...
3. Stelle sicher, dass du die neueste Version hast: `hena upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `hena models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\hena`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. hena installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\hena`

2. Starte hena neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt hena, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
hena erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
