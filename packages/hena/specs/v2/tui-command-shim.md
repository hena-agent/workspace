...
Delete `packages/hena/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/hena/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/hena/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/hena`
- TUI plugin loader tests in `packages/hena` if runtime plugin API wiring changed
