...
Hena koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM providera**, uključujući lokalne modele.

## Provajderi

Većina popularnih provajdera su unaprijed učitani prema zadanim postavkama. Ako ste dodali vjerodajnice za provajdera putem naredbe `/connect`, oni će biti dostupni kada pokrenete Hena.
...
Evo nekoliko modela koji dobro rade sa Hena, bez posebnog redosleda. (Ovo nije potpuna lista niti je nužno ažurirana):
...
Ovdje je puni ID `provider_id/model_id`. Na primjer, ako koristite [Hena Zen](/docs/zen), koristili biste `hena/gpt-5.1-codex` za GPT 5.1 Codex.
...
Mnogi modeli podržavaju više varijanti sa različitim konfiguracijama. Hena se isporučuje sa ugrađenim podrazumevanim varijantama za popularne provajdere.

### Ugrađene varijante

Hena se isporučuje sa zadanim varijantama za mnoge provajdere:
**Anthropic**:
...
Kada se Hena pokrene, on provjerava modele u sljedećem prioritetnom redoslijedu:

1. Oznaka komandne linije `--model` ili `-m`. Format je isti kao u konfiguracijskoj datoteci: `provider_id/model_id`.
2. Lista modela u Hena konfiguraciji.
