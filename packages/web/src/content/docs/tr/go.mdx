...
Go, Hena'daki diğer sağlayıcılar gibi çalışır. Hena Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Hena'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Hena Go, Hena'daki diğer sağlayıcılar gibi çalışır.
...
Hena yapılandırmanızdaki [model id](/docs/config/#models) formatı `hena-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `hena-go/kimi-k3` kullanmalısınız.
...
4. Hena ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
