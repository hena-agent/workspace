---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

hena używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji Hena.
...
w `~/.local/share/hena/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim hena
config.
...
Hena Zen to lista modeli dostarczonych przez zespół hena, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z Hena. [Dowiedz się więcej](/docs/zen).
...
Działa jak każdy inny dostawca w hena i jest całkowicie opcjonalny w użyciu.

---

## Hena Go

Hena Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół hena, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z Hena.
...
Aby używać Amazon Bedrock z hena:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania hena:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `hena.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w hena nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować hena do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod hena działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji Hena.
...
hena używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować hena
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`hena.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem hena:
...
Aby używać Google Vertex AI z hena:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania Hena.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą hena, zawsze możesz ją skonfigurować samodzielnie.
...
Możesz skonfigurować hena tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować hena tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować hena tak, aby korzystał z modeli lokalnych poprzez Ollamę.
...
Aby korzystać z Ollama Cloud z hena:
...
3. Skopiuj klucz API do użycia w Hena.
...
6. **Ważne**: Przed użyciem modeli chmurowych w hena musisz pobrać informacje o modelu lokalnie:
...
Hena Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół Hena. [Dowiedz się więcej](/docs/zen).
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji Hena.
...
5. Możesz je także dostosować za pomocą konfiguracji Hena. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z hena:
...
Możesz także dostosować modele za pomocą konfiguracji Hena. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z Hena. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `hena.json` w katalogu projektu:
...
Pola `limit` pozwalają hena zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `hena auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację hena i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji Hena.
