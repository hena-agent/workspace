---
title: LSP Servers
description: Hena Agent integrates with your LSP servers.
---

Hena Agent can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Hena Agent comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `HENA_AGENT_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and hena-agent opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your hena-agent config.
...
```json title="hena-agent.json" {5-8}
{
  "$schema": "https://hena.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="hena-agent.json" {5-13}
{
  "$schema": "https://hena.ai/config.json",
  "lsp": {
    "custom-lsp": {
