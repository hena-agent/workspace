---
title: Server
description: Interagisci con il server hena via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

Il comando `hena serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client Hena.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `hena.local` |
...
Imposta `HENA_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `hena`, oppure imposta `HENA_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `hena serve` sia per `hena web`.
...
Quando esegui `hena` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server hena per interagire con hena in modo programmatico.
:::

Questa architettura permette a hena di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `hena serve` per avviare un server standalone. Se la TUI di hena e gia in esecuzione, `hena serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Hena.
...
Il server hena espone le seguenti API.
