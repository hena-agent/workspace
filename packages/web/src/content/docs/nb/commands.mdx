...
Opprett `.hena-agent/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom Hena Agent-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i Hena Agent [config](/docs/config):
...
- Per prosjekt: `.hena-agent/commands/`
...
Hena Agent inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
