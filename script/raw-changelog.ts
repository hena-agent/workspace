...
const repo = process.env.GH_REPO ?? "hena-agent/hena"
const bot = ["actions-user", "github-actions[bot]", "hena", "hena[bot]"]
const team = [
...
const order = ["Core", "Desktop", "SDK", "Extensions"] as const
const sections = {
  core: "Core",
  app: "Desktop",
  tauri: "Desktop",
...
function section(areas: Set<string>) {
  const priority = ["core", "app", "tauri", "sdk", "plugin", "extensions/vscode", "github"]
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/hena packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/hena/")) areas.add("core")
