// Shared type vocabulary for the direct interactive mode (`hena --mini`).
//
...
import type { HenaClient, PermissionRequest, QuestionRequest, ToolPart } from "@hena/sdk/v2"
import type { TuiConfig } from "@hena/tui/config"
...
type PromptModel = Parameters<HenaClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<HenaClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<HenaClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<HenaClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<HenaClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<HenaClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<HenaClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<HenaClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<HenaClient["question"]["reject"]>[0]
