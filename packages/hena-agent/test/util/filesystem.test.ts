...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "hena-agent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "hena-agent.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "hena-agent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "hena-agent.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["hena-agent.json", "hena-agent.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "hena-agent.json"),
        path.join(project, "hena-agent.jsonc"),
      ])
    })
  })
