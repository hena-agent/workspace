---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Hena.
---

Dodaci vam omogućavaju da proširite Hena spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Hena.
...
- `.hena/plugins/` - Dodaci na nivou projekta
- `~/.config/hena/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/hena/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/hena/hena.json`)
2. Konfiguracija projekta (`hena.json`)
3. Globalni direktorij dodataka (`~/.config/hena/plugins/`)
4. Direktorij dodataka projekta (`.hena/plugins/`)
...
Hena pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Hena SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Hena.
...
Spriječite hena da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u hena:
...
Pomoćnik `tool` kreira prilagođeni alat koji hena može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za hena zajedno sa ugrađenim alatima.
