---
title: Servidor
description: Interactuar con el servidor hena-agent a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `hena-agent serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente hena-agent puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `hena-agent.local` |
...
Configure `HENA_AGENT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `hena-agent`, o configure `HENA_AGENT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `hena-agent serve` como a `hena-agent web`.
...
Cuando ejecuta `hena-agent`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor hena-agent para interactuar con hena-agent mediante programación.
:::

Esta arquitectura permite que hena-agent admita múltiples clientes y le permite interactuar con hena-agent mediante programación.

Puede ejecutar `hena-agent serve` para iniciar un servidor independiente. Si tienes el
hena-agent TUI ejecutándose, `hena-agent serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Hena Agent [IDE](/docs/ide).
...
El servidor hena-agent expone las siguientes API.
