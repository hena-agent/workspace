---
title: Server
description: Interagisci con il server hena-agent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `hena-agent serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client Hena Agent.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `hena-agent.local` |
...
Imposta `HENA_AGENT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `hena-agent`, oppure imposta `HENA_AGENT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `hena-agent serve` sia per `hena-agent web`.
...
Quando esegui `hena-agent` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server hena-agent per interagire con hena-agent in modo programmatico.
:::

Questa architettura permette a hena-agent di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `hena-agent serve` per avviare un server standalone. Se la TUI di hena-agent e gia in esecuzione, `hena-agent serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Hena Agent.
...
Il server hena-agent espone le seguenti API.
