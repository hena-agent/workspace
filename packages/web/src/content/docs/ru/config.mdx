---
title: Конфигурация
description: Использование конфигурации hena-agent JSON.
---

Вы можете настроить hena-agent, используя файл конфигурации JSON.

---

## Формат

hena-agent поддерживает форматы **JSON** и **JSONC** (JSON с комментариями).
...
Например, если ваша глобальная конфигурация устанавливает `theme: "hena-agent"` и `autoupdate: true`, а конфигурация вашего проекта устанавливает `model: "anthropic/claude-sonnet-4-5"`, окончательная конфигурация будет включать все три параметра.
...
1. **Удаленная конфигурация** (от `.well-known/hena-agent`) – организационные настройки по умолчанию.
2. **Глобальная конфигурация** (`~/.config/hena-agent/hena.json`) — настройки пользователя.
3. **Пользовательская конфигурация** (`HENA_AGENT_CONFIG` env var) – пользовательские переопределения
4. **Конфигурация проекта** (`hena-agent.json` в проекте) — настройки, специфичные для проекта.
5. **Каталоги `.hena-agent`** — агенты, команды, плагины
6. **Встроенная конфигурация** (`HENA_AGENT_CONFIG_CONTENT` env var) – переопределяет время выполнения
...
:::note
В каталогах `.hena-agent` и `~/.config/hena-agent` для подкаталогов используются **множественные имена**: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` и `themes/`. Единственные имена (например, `agent/`) также поддерживаются для обратной совместимости.
:::
...
Организации могут предоставить конфигурацию по умолчанию через конечную точку `.well-known/hena-agent`. Он извлекается автоматически при аутентификации у провайдера, который его поддерживает.
...
Поместите глобальную конфигурацию hena-agent в `~/.config/hena-agent/hena.json`. Используйте глобальную конфигурацию для общепользовательских настроек, таких как темы, поставщики или привязки клавиш.

Для настроек, специфичных для TUI, используйте `~/.config/hena-agent/tui.json`.
...
Добавьте `hena-agent.json` в корень вашего проекта. Конфигурация проекта имеет наивысший приоритет среди стандартных файлов конфигурации — она переопределяет как глобальные, так и удаленные конфигурации.
...
Когда hena-agent запускается, он ищет файл конфигурации в текущем каталоге или переходит к ближайшему каталогу Git.
...
Укажите собственный путь к файлу конфигурации, используя переменную среды `HENA_AGENT_CONFIG`.
...
Укажите пользовательский каталог конфигурации, используя переменную среды `HENA_AGENT_CONFIG_DIR`. В этом каталоге будет выполняться поиск агентов, команд, режимов и плагинов (аналогично стандартному каталогу `.hena-agent`), и он должен иметь ту же структуру.
...
Пользовательский каталог загружается после каталогов global config и `.hena-agent`, поэтому он **может переопределить** их настройки.

---

## Схема

Файл конфигурации имеет схему, определенную в [**`hena.dev/config.json`**](https://hena.dev/config.json).

Конфигурация TUI использует [**`hena.dev/tui.json`**](https://hena.dev/tui.json).
...
Используйте `HENA_AGENT_TUI_CONFIG`, чтобы указать на пользовательский файл конфигурации TUI.

Устаревшие ключи `theme`, `keybinds` и `tui` в `hena-agent.json` устарели и автоматически переносятся, когда это возможно.
...
Вы можете настроить параметры сервера для команд `hena-agent serve` и `hena-agent web` с помощью опции `server`.
...
- `mdns` — включить обнаружение службы mDNS. Это позволит другим устройствам в сети обнаружить ваш сервер Hena Agent.
- `mdnsDomain` — собственное доменное имя для службы mDNS. По умолчанию `hena-agent.local`. Полезно для запуска нескольких экземпляров в одной сети.
...
Вы можете настроить поставщиков и модели, которые хотите использовать в своей конфигурации hena-agent, с помощью параметров `provider`, `model` и `small_model`.
...
Опция `small_model` настраивает отдельную модель для облегченных задач, таких как создание заголовков. По умолчанию hena-agent пытается использовать более дешевую модель, если она доступна у вашего провайдера, в противном случае он возвращается к вашей основной модели.
...
Вы также можете определить агентов, используя файлы Markdown в `~/.config/hena-agent/agents/` или `.hena-agent/agents/`. [Подробнее здесь](/docs/agents).
...
Агент по умолчанию должен быть основным агентом (а не субагентом). Это может быть встроенный агент, например `"build"` или `"plan"`, или [пользовательский агент](/docs/agents), который вы определили. Если указанный агент не существует или является субагентом, hena-agent вернется к `"build"` с предупреждением.

Этот параметр применяется ко всем интерфейсам: TUI, CLI (`hena-agent run`), настольному приложению и действию GitHub.
...
Вы также можете определять команды, используя файлы Markdown в `~/.config/hena-agent/commands/` или `.hena-agent/commands/`. [Подробнее здесь](/docs/commands).
...
hena-agent автоматически загрузит все новые обновления при запуске. Вы можете отключить это с помощью опции `autoupdate`.
...
По умолчанию hena-agent **разрешает все операции**, не требуя явного разрешения. Вы можете изменить это, используя опцию `permission`.
...
[Плагины](/docs/plugins) расширяют hena-agent с помощью пользовательских инструментов, перехватчиков и интеграций.

Поместите файлы плагина в `.hena-agent/plugins/` или `~/.config/hena-agent/plugins/`. Вы также можете загружать плагины из npm с помощью опции `plugin`.
...
Это полезно, если вы хотите ограничить hena-agent использованием только определенных поставщиков, а не отключать их по одному.
