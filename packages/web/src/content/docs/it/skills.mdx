...
Le skill degli agenti permettono a Hena Agent di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Hena Agent cerca in queste posizioni:

- Config di progetto: `.hena-agent/skills/<name>/SKILL.md`
- Config globale: `~/.config/hena-agent/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Hena Agent risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.hena-agent/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/hena-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.hena-agent/skills/git-release/SKILL.md` cosi':
...
Hena Agent elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `hena-agent.json`:
...
**Per agenti integrati** (in `hena-agent.json`):
