...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-hena-directory", "directory"],
    ["x-hena-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-hena-workspace")
  return next
}

export function createHenaClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-hena-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of Hena Server (Server responded with text/html)")
