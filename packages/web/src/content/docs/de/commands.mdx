...
Erstellen Sie `.hena/commands/test.md`:

```md title=".hena/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Hena-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Hena [config](/docs/config):

```json title="hena.jsonc" {4-12}
{
...
- Global: `~/.config/hena/commands/`
- Pro Projekt: `.hena/commands/`

```markdown title="~/.config/hena/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".hena/commands/component.md"
---
...
```md title=".hena/commands/create-file.md"
---
...
```md title=".hena/commands/analyze-coverage.md"
---
...
```md title=".hena/commands/review-changes.md"
---
...
```md title=".hena/commands/review-component.md"
---
...
Hena enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
