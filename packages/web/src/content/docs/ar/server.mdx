---
title: الخادم
description: تفاعل مع خادم hena عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `hena serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل hena استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `hena.local` |
...
عيّن `HENA_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `hena`، أو عيّن `HENA_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `hena serve` و `hena web`.
...
عند تشغيل `hena` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم hena للتفاعل مع hena برمجيا.
:::

تتيح هذه البنية لـ hena دعم عدة عملاء وتمكّنك من التفاعل مع hena برمجيا.

يمكنك تشغيل `hena serve` لبدء خادم مستقل. إذا كانت واجهة hena في terminal (TUI)
قيد التشغيل، فسيبدأ `hena serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Hena لـ [IDE](/docs/ide).
...
يعرض خادم hena واجهات API التالية.
