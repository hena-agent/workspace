...
export function createAcpClient(input: Pick<CliFixture, "henaAgent">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.henaAgent.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "hena-agent-local-acp", version: "0.1.0" },
      }),
    )
  })
}
