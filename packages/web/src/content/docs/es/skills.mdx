...
Las habilidades del agente permiten a Hena Agent descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Hena Agent busca estas ubicaciones:

- Configuración del proyecto: `.hena-agent/skills/<name>/SKILL.md`
- Configuración global: `~/.config/hena-agent/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Hena Agent sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.hena-agent/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/hena-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.hena-agent/skills/git-release/SKILL.md` así:
...
Hena Agent enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `hena-agent.json`:
...
**Para agentes integrados** (en `hena-agent.json`):
