...
`.hena-agent/commands/test.md` oluştur:
...
hena-agent işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

hena-agent [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/hena-agent/commands/`
- Proje başına: `.hena-agent/commands/`
...
hena-agent, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
