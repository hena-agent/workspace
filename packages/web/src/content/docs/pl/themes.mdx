...
Dzięki hena-agent możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie hena-agent używa naszego własnego motywu `hena-agent`.
...
hena-agent ma kilka wbudowanych motywów.
...
- Chcą, aby Hena Agent pasował do wyglądu ich terminala
...
hena-agent obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/hena-agent/themes/*.json` lub `$XDG_CONFIG_HOME/hena-agent/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.hena-agent/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.hena-agent/themes/*.json`
