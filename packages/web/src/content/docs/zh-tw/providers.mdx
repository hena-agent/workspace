---
title: 提供商
description: 在 Hena 中使用任意 LLM 提供商。
---

import config from "../../../../config.mjs"
export const console = config.console

Hena 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支援 **75+ LLM 提供商**，同時也支援執行本地模型。
...
`~/.local/share/hena/auth.json` 中。

---

### 設定

您可以透過 Hena 設定中的 `provider` 部分來自訂提供商。
...
Hena Zen 是由 Hena 團隊提供的模型列表，這些模型已經過測試和驗證，能夠與 Hena 良好配合使用。[了解更多](/docs/zen)。
...
它的使用方式與 Hena 中的其他提供商完全相同，且完全可選。

---

## Hena Go

Hena Go 是一個低成本的訂閱計畫，提供對 Hena 團隊提供的流行開放編碼模型的可靠存取，這些模型已經過測試和驗證，能夠與 Hena 良好配合使用。
...
它的使用方式與 Hena 中的任何其他提供商相同，且完全可選。
...
:::info
在 Hena 中使用 Claude Pro/Max 訂閱不是 [Anthropic](https://anthropic.com) 官方支援的用法。
:::
...
你可以透過 [Atomic Chat](https://atomic.chat) 設定 hena 以使用本地模型。Atomic Chat 是一款桌面應用程式，它在 OpenAI 相容的 API 伺服器後方執行本地 LLM（預設端點 `http://127.0.0.1:1337/v1`）。
...
:::note
您也可以透過指定 `GITLAB_TOKEN` 環境變數來避免將權杖儲存在 Hena 的認證儲存中。
:::

##### 自架 GitLab

:::note[合規說明]
Hena 會使用一個小模型來執行部分 AI 任務，例如產生工作階段標題。預設情況下使用由 Zen 託管的 gpt-5-nano。如果您需要讓 Hena 僅使用您自己的 GitLab 託管實例，請在 `hena.json` 檔案中新增以下內容。同時建議停用工作階段分享。
...
要在 Hena 中使用您的 GitHub Copilot 訂閱：
...
如果 Helicone 的某些功能或模型未透過 Hena 自動設定，您隨時可以手動設定。
...
您可以透過 [llama.cpp](https://github.com/ggml-org/llama.cpp) 的 llama-server 工具設定 Hena 使用本地模型。
...
您可以透過 LM Studio 設定 Hena 使用本地模型。
...
6. **重要**：在 Hena 中使用雲端模型之前，必須先將模型資訊拉取到本地：
...
Hena Zen 是由 Hena 團隊提供的經過測試和驗證的模型列表。[了解更多](/docs/zen)。
...
5. 您還可以透過 Hena 設定自訂模型。以下是指定提供商的範例：
...
要在 Hena 中使用 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)：
...
您也可以透過 Hena 設定自訂模型。以下是指定提供商路由順序的範例。
...
:::tip
您可以在 Hena 中使用任何 OpenAI 相容的提供商。大多數現代 AI 提供商都提供 OpenAI 相容的 API。
:::
...
`limit` 欄位讓 Hena 了解您還剩餘多少上下文空間。標準提供商會自動從 models.dev 拉取這些資訊。
...
1. **檢查認證設定**：執行 `hena auth list` 查看該提供商的憑證是否已新增到設定中。

   這不適用於 Amazon Bedrock 等依賴環境變數進行認證的提供商。

2. 對於自訂提供商，請檢查 Hena 設定並確認：
   - `/connect` 指令中使用的提供商 ID 與 Hena 設定中的 ID 一致。
