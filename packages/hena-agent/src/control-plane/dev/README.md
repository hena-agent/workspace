This is a plugin to simulate a remote environment locally. Add this to `.hena-agent/hena.jsonc`:

```json
  "plugin": ["../packages/hena-agent/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate Hena Agent server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/hena-agent/script/run-workspace-server
```

With the plugin install, you can now run Hena Agent and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
