...
export async function checkPluginCompatibility(target: string, henaVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(henaVersion) || semver.major(henaVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines["hena"]
  if (typeof range !== "string") return
  if (!semver.satisfies(henaVersion, range)) {
    throw new Error(`Plugin requires Hena ${range} but running ${henaVersion}`)
  }
}
