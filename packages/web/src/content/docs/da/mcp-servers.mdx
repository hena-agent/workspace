...
Du kan tilføje eksterne værktøjer til Hena Agent ved hjælp af _Model Context Protocol_ eller MCP. Hena Agent understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Hena Agent Config](https://hena.dev/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/hena-agent`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Hena Agent håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Hena Agent:
...
Hvis serveren kræver godkendelse, vil Hena Agent bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `hena-agent mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Hena Agent opbevare tokens sikkert i `~/.local/share/hena-agent/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Hena Agent-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Hena Agent til din Sentry-konto.
