...
import { FrontmatterError } from "@hena/core/v1/config/error"
...
const HENA_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with Hena. The model's intuition for what an
// hena.json should look like is often wrong, and Hena hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch Hena's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_HENA_SKILL_NAME = "customize-hena"
const CUSTOMIZE_HENA_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating Hena's own configuration: hena.json, hena.jsonc, files under .hena/, or files under ~/.config/hena/. Also use when creating or fixing Hena agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring Hena itself."
const CUSTOMIZE_HENA_SKILL_BODY = SkillPlugin.CustomizeHenaContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, HENA_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@hena/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_HENA_SKILL_NAME] = {
          name: CUSTOMIZE_HENA_SKILL_NAME,
          description: CUSTOMIZE_HENA_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_HENA_SKILL_BODY,
        }
