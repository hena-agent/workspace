---
title: TUI
description: Usando a interface de usuário de terminal Hena Agent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

O hena-agent fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar hena-agent inicia o TUI para o diretório atual.
...
Ao usar o TUI do hena-agent, você pode digitar `/` seguido pelo nome de um comando para executar ações rapidamente. Por exemplo:
...
Adicione um provedor ao Hena Agent. Permite que você selecione entre os provedores disponíveis e adicione suas chaves de API.
...
Saia do Hena Agent. _Aliases_: `/quit`, `/q`
...
Isso é separado do `hena-agent.json`, que configura o comportamento do servidor/runtime.
...
- `leader_timeout` - Controla por quanto tempo o Hena Agent espera depois da leader key. O padrão é `2000`.
...
Use `HENA_AGENT_TUI_CONFIG` para carregar um caminho de configuração TUI personalizado.

### Attention

Attention permite que o TUI avise você quando o Hena Agent espera uma resposta, precisa da aprovação de uma permissão, encontra um erro de sessão ou conclui uma sessão. Ative com `attention.enabled`; eventos integrados reproduzem sons quando acontecem. As notificações de desktop só são enviadas quando a janela do terminal não está em foco e não são usadas para eventos de subagent.
...
- `sound_pack` - ID do sound pack a ser usado. O padrão é `hena-agent.default`.
