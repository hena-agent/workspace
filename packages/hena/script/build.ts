...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/hena`,
      execArgv: [`--user-agent=hena/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    files: {
    ...
    define: {
      ...
      HENA_MODELS_DEV: generated.modelsData,
      OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + treeSitterWorkerPath,
      HENA_WORKER_PATH: workerPath,
      HENA_CHANNEL: `'${Script.channel}'`,
      HENA_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/hena`
