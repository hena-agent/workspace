import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@hena-agent/plugin/tui"
...
import type { useHenaAgentKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useHenaAgentKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getHenaAgentModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getHenaAgentModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
