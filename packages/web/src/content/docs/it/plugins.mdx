---
title: Plugin
description: Scrivi plugin per estendere Hena Agent.
---

I plugin ti permettono di estendere Hena Agent agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Hena Agent.
...
- `.hena-agent/plugins/` - plugin a livello progetto
- `~/.config/hena-agent/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/hena-agent/node_modules/`.
...
1. Config globale (`~/.config/hena-agent/hena.json`)
2. Config di progetto (`hena-agent.json`)
3. Directory plugin globale (`~/.config/hena-agent/plugins/`)
4. Directory plugin di progetto (`.hena-agent/plugins/`)
...
Hena Agent esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di hena-agent per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere Hena Agent.
...
:::note
Se usi l'app desktop di Hena Agent, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a hena-agent di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a hena-agent:
...
L'helper `tool` crea uno strumento personalizzato che hena-agent può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in hena-agent insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://hena.dev/docs/sdk) per i dettagli.
