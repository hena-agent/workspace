...
import type { SessionNotFoundError } from "@hena-agent/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em hena-agent.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
