...
Agent Skills erlauben Hena Agent, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Hena Agent sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Hena Agent vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.hena-agent/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/hena-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.hena-agent/skills/git-release/SKILL.md` zum Beispiel so:
...
Hena Agent listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `hena-agent.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `hena-agent.json`):
