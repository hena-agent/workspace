---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w Hena Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

hena-agent używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji Hena Agent.
...
w `~/.local/share/hena-agent/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim hena-agent
config.
...
Hena Zen to lista modeli dostarczonych przez zespół hena-agent, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z Hena Agent. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Hena Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Hena Zen` i przejdź do [hena.dev/auth](https://hena.dev/zen).
...
Działa jak każdy inny dostawca w hena-agent i jest całkowicie opcjonalny w użyciu.

---

## Hena Go

Hena Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół hena-agent, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z Hena Agent.

1. Uruchom polecenie `/connect` w TUI, wybierz `Hena Go` i przejdź do [hena.dev/auth](https://hena.dev/zen).
...
Aby używać Amazon Bedrock z hena-agent:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania hena-agent:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `hena-agent.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w hena-agent nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować hena-agent do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod hena-agent działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji Hena Agent.
...
hena-agent używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować hena-agent
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`hena-agent.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem hena-agent:
...
Aby używać Google Vertex AI z hena-agent:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania Hena Agent.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą hena-agent, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację hena-agent jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja hena-agent będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować hena-agent tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować hena-agent tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować hena-agent tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem Hena Agent. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
Aby korzystać z Ollama Cloud z hena-agent:
...
3. Skopiuj klucz API do użycia w Hena Agent.
...
6. **Ważne**: Przed użyciem modeli chmurowych w hena-agent musisz pobrać informacje o modelu lokalnie:
...
Hena Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół Hena Agent. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Hena Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Hena Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API Hena Agent.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji Hena Agent.
...
5. Możesz je także dostosować za pomocą konfiguracji Hena Agent. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z hena-agent:
...
Możesz także dostosować modele za pomocą konfiguracji Hena Agent. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z Hena Agent. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `hena-agent.json` w katalogu projektu:
...
Pola `limit` pozwalają hena-agent zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `hena-agent auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację hena-agent i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji Hena Agent.
