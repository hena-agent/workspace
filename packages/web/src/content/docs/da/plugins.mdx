---
title: Plugins
description: Skriv dine egne plugins for at udvide Hena.
---

Plugins giver dig mulighed for at udvide Hena ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Henas standardadfærd.
...
- `.hena/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/hena/node_modules/`.
...
1. Global konfiguration (`~/.config/hena/hena.json`)
2. Projektkonfiguration (`hena.json`)
3. Global plugin-mappe (`~/.config/hena/plugins/`)
4. Projekt plugin bibliotek (`.hena/plugins/`)
...
Hena kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En hena SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide Hena.
...
:::note
Hvis du bruger Hena desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå hena i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til hena:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som hena kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for hena sammen med indbyggede værktøjer.
