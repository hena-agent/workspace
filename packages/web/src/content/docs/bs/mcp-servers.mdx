...
Možete dodati vanjske alate u Hena koristeći _Model Context Protocol_, ili MCP. Hena podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Hena Config](https://hena.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/hena`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Hena automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Hena će:
...
Ako server zahtijeva autentifikaciju, Hena će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `hena mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Hena sigurno čuva tokene u `~/.local/share/hena/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Hena, zajedno s ugrađenim alatima. Možete njima upravljati kroz Hena konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Hena sa vašim Sentry nalogom.
