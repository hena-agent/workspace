---
title: Complementos
description: Escriba sus propios complementos para extender Hena Agent.
---

Los complementos le permiten extender Hena Agent al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Hena Agent.
...
- `.hena-agent/plugins/` - Complementos a nivel de proyecto
- `~/.config/hena-agent/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/hena-agent/node_modules/`.
...
1. Configuración global (`~/.config/hena-agent/hena.json`)
2. Configuración del proyecto (`hena-agent.json`)
3. Directorio global de complementos (`~/.config/hena-agent/plugins/`)
4. Directorio de complementos del proyecto (`.hena-agent/plugins/`)
...
```json title=".hena-agent/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Hena Agent ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".hena-agent/plugins/my-plugin.ts"
...
```js title=".hena-agent/plugins/example.js"
...
- `client`: Un cliente SDK hena-agent para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar Hena Agent.
...
```js title=".hena-agent/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "hena-agent"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Hena Agent, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que hena-agent lea archivos `.env`:

```javascript title=".hena-agent/plugins/env-protection.js"
...
```javascript title=".hena-agent/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a hena-agent:

```ts title=".hena-agent/plugins/custom-tools.ts"
import { type Plugin, tool } from "@hena-agent/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que hena-agent puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para hena-agent junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://hena.dev/docs/sdk) para obtener más detalles.
...
```ts title=".hena-agent/plugins/compaction.ts"
import type { Plugin } from "@hena-agent/plugin"
...
```ts title=".hena-agent/plugins/custom-compaction.ts"
import type { Plugin } from "@hena-agent/plugin"
