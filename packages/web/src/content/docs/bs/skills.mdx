...
Agent skills omogucavaju Hena da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Hena pretrazuje ove lokacije:

- Konfiguracija projekta: `.hena/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/hena/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Hena ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.hena/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/hena/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.hena/skills/git-release/SKILL.md` ovako:
...
Hena navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `hena.json`:
...
**Za ugradene agente** (u `hena.json`):
