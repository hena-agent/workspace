...
Agent skills omogucavaju Hena Agent da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Hena Agent pretrazuje ove lokacije:

- Konfiguracija projekta: `.hena-agent/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/hena-agent/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Hena Agent ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.hena-agent/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/hena-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.hena-agent/skills/git-release/SKILL.md` ovako:
...
Hena Agent navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `hena-agent.json`:
...
**Za ugradene agente** (u `hena-agent.json`):
