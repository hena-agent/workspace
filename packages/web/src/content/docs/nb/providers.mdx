---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Hena Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena Agent bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Hena Agent-konfigurasjonen.
...
i `~/.local/share/hena-agent/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Hena Agent
konfig.
...
Hena Zen er en liste over modeller levert av Hena Agent-teamet som har vært
testet og verifisert for å fungere godt med Hena Agent. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Hena Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Hena Zen` og gå til [hena.dev/auth](https://hena.dev/zen).
...
Det fungerer som alle andre leverandører i Hena Agent og er helt valgfritt å bruke.

---

## Hena Go

Hena Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Hena Agent-teamet som har vært
testet og verifisert for å fungere godt med Hena Agent.

1. Kjør kommandoen `/connect` i TUI, velg `Hena Go`, og gå til [hena.dev/auth](https://hena.dev/zen).
...
Slik bruker du Amazon Bedrock med Hena Agent:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Hena Agent:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `hena-agent.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i hena-agent støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere hena-agent til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Hena Agent skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Hena Agent-konfigurasjonen.
...
for å lagre token i Hena Agent auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Hena Agent bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Hena Agent
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`hena-agent.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `hena-agent.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Hena Agent:
...
Slik bruker du Google Vertex AI med Hena Agent:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Hena Agent.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Hena Agent, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Hena Agent-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Hena Agent-samtale oppført som en separat økt.
...
Du kan konfigurere Hena Agent for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Hena Agent for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Hena Agent for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Hena Agent. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Hena Agent:
...
3. Kopier API-nøkkelen for bruk i Hena Agent.
...
6. **Viktig**: Før du bruker skymodeller i Hena Agent, må du hente modellinformasjonen lokalt:
...
Hena Zen er en liste over testede og verifiserte modeller levert av Hena Agent-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Hena Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Hena Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Hena Agent API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Hena Agent-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Hena Agent-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Hena Agent:
...
Du kan også tilpasse modeller gjennom Hena Agent-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Hena Agent. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `hena-agent.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Hena Agent forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `hena-agent auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Hena Agent-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Hena Agent-konfigurasjonen.
