// Subprocess test harness for the hena-agent CLI. Spawns the real binary against
...
//
// Configuration flows through hena-agent's built-in test affordances:
//   - HENA_AGENT_CONFIG_CONTENT      : provider config inline, no files to find
//   - HENA_AGENT_TEST_HOME           : pins os.homedir() → tmpdir
//   - HENA_AGENT_DISABLE_PROJECT_CONFIG : skip walking up for hena-agent.json
//   - HENA_AGENT_PURE                : skip external plugin discovery + install
//   - HENA_AGENT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `henaAgent.run` is fully wired. The shape supports adding more
// builders (`henaAgent.serve(opts)`, `henaAgent.acp(opts)`, `henaAgent.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of HenaAgentCli.
...
import { AppProcess } from "@hena-agent/core/process"
...
const henaAgentRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(henaAgentRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    HENA_AGENT_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    HENA_AGENT_CONFIG_CONTENT: configJson,
    HENA_AGENT_DISABLE_PROJECT_CONFIG: "1",
    HENA_AGENT_PURE: "1",
    HENA_AGENT_DISABLE_AUTOUPDATE: "1",
    HENA_AGENT_DISABLE_AUTOCOMPACT: "1",
    HENA_AGENT_DISABLE_MODELS_FETCH: "1",
    HENA_AGENT_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `hena-agent run`. New flags should
...
// `hena-agent serve` is a long-lived process — it never exits on its own.
...
// `hena-agent acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type HenaAgentCli = {
  ...
  // Spawn `hena-agent serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `hena-agent acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly henaAgent: HenaAgentCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("hena-agent.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          HENA_AGENT_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("hena-agent.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("hena-agent.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "hena-agent server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `hena-agent serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("hena-agent.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const henaAgent: HenaAgentCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, henaAgent })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `henaAgent.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "hena-agent") {
...
// every call site. Use this for any test that needs the hena-agent CLI fixture.
//
...
// resources (e.g. `henaAgent.serve`) without an extra `Effect.scoped` wrapper —
