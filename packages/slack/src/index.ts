...
import { createHenaAgent, type ToolPart } from "@hena-agent/sdk"
...
console.log("🚀 Starting Hena Agent server...")
const henaAgent = await createHenaAgent({
  port: 0,
})
console.log("✅ Hena Agent server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await henaAgent.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new Hena Agent session...")
    const { client, server } = henaAgent
    ...
    console.log("✅ Created Hena Agent session:", createResult.data.id)
  ...
  console.log("📝 Sending to Hena Agent:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Hena Agent response:", JSON.stringify(result, null, 2))
