...
最も一般的なプロバイダーはデフォルトでプリロードされています。 `/connect` コマンドを使用してプロバイダーの資格情報を追加した場合は、Hena Agent を開始するときにそれらの資格情報を使用できるようになります。
...
ここでは、Hena Agent で適切に動作するいくつかのモデルを順不同で示します。 (これは完全なリストではなく、必ずしも最新であるとは限りません):
...
これらのいずれかをデフォルトのモデルとして設定するには、`model` キーを Hena Agent の設定に追加します。
...
ここでの完全な ID は `provider_id/model_id` です。たとえば、[Hena Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `hena-agent/gpt-5.1-codex` を使用します。
...
多くのモデルは、異なる構成の複数のバリアントをサポートしています。 Hena Agent には、一般的なプロバイダーのデフォルトのバリアントが組み込まれています。

### 組み込みバリアント

Hena Agent には、多くのプロバイダーのデフォルトのバリアントが付属しています。
...
Hena Agent が起動すると、次の優先順位でモデルがチェックされます。

1. `--model` または `-m` コマンドラインフラグ。形式は構成ファイルと同じです: `provider_id/model_id`。

2. Hena Agent 構成内のモデルのリスト。
