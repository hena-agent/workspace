...
Med Hena kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Hena vårt eget `hena`-tema.
...
Hena støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/hena/themes/*.json` eller `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.hena/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.hena/themes/*.json`
