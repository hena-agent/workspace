---
title: Serveur
description: Interagissez avec le serveur hena via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

La commande `hena serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client hena peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `hena.local` |
...
Définissez `HENA_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `hena`, ou définissez `HENA_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `hena serve` et à `hena web`.
...
Lorsque vous exécutez `hena`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur hena pour interagir avec hena par programmation.
:::

Cette architecture permet à hena de prendre en charge plusieurs clients et vous permet d'interagir avec hena par programme.

Vous pouvez exécuter `hena serve` pour démarrer un serveur autonome. Si vous avez le
hena TUI en cours d'exécution, `hena serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Hena [IDE](/docs/ide).
...
Le serveur hena expose les API suivantes.
