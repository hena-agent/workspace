---
title: المزوّدون
description: استخدام أي موفّر LLM في Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Hena ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Hena.
...
في `~/.local/share/hena/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Hena.
...
Hena Zen هي قائمة نماذج يوفّرها فريق Hena وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Hena. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Hena Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر hena، ثم انتقل إلى [hena.dev/auth](https://hena.dev/auth).
...
يعمل مثل أي مزوّد آخر في Hena واستخدامه اختياري بالكامل.

---

## Hena Go

Hena Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Hena، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Hena.
...
لاستخدام Amazon Bedrock مع Hena:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل hena:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `hena.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Hena غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين hena لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل hena بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات Hena.
...
تخزين الرمز في مساحة تخزين مصادقة Hena.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Hena نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Hena
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`hena.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع hena:
...
لاستخدام Google Vertex AI مع Hena:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر hena، فيمكنك دائما ضبطه بنفسك.
...
يمكنك ضبط hena لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط hena لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط hena لاستخدام نماذج محلية عبر Ollama.
...
3. انسخ مفتاح API لاستخدامه في Hena.
...
6. **مهم**: قبل استخدام نماذج السحابة في Hena، يجب سحب معلومات النموذج محليا:
...
Hena Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Hena. [اعرف المزيد](/docs/zen).
...
3. أدخل مفتاح API الخاص بـ Hena.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات Hena.
...
5. يمكنك أيضا تخصيصها عبر إعدادات Hena. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Hena:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات Hena. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع Hena. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `hena.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Hena بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `hena auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات hena وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات Hena.
