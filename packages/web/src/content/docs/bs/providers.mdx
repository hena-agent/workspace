---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Hena konfiguraciji.
...
u `~/.local/share/hena/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Hena
config.
...
Hena Zen je lista modela koje je obezbedio Hena tim koji su bili
testirano i potvrđeno da dobro radi sa Hena. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Hena Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite hena i idite na [hena.dev/auth](https://hena.dev/auth).
...
Radi kao i svaki drugi provajder u Hena i potpuno je opcionalan za korištenje.

---

## Hena Go

Hena Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Hena tim i koji su testirani i verificirani da dobro rade s Hena-om.
...
Da biste koristili Amazon Bedrock s Hena:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete hena:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `hena.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Hena.
:::
...
Možete konfigurirati hena za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi hena ispravno radio.
   :::
...
Također možete dodati modele kroz svoju hena konfiguraciju.
...
da pohrani token u hena auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Hena koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Hena
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`hena.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s hena:
...
Za korištenje Google Vertex AI s Hena:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće Hena.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem henaa, uvijek ga možete sami konfigurirati.
...
Možete konfigurirati hena za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati hena za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati hena za korištenje lokalnih modela putem Ollame.
...
Da biste koristili Ollama Cloud s Hena:
...
3. Kopirajte API ključ za korištenje u Hena.
...
6. **Važno**: Prije upotrebe modela oblaka u Hena, morate lokalno povući informacije o modelu:
...
Hena Zen je lista testiranih i verifikovanih modela koju je obezbedio Hena tim. [Saznajte više](/docs/zen).
...
Također možete dodati dodatne modele putem vaše hena konfiguracije.
...
5. Također ih možete prilagoditi putem vaše hena konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Henaom:
...
Također možete prilagoditi modele kroz svoju hena konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s hena-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `hena.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Hena da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `hena auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Hena konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj hena konfiguraciji.
