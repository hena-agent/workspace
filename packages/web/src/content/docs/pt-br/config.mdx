---
title: Configuração
description: Usando a configuração JSON do Hena Agent.
---

Você pode configurar o hena-agent usando um arquivo de configuração JSON.

---

## Formato

O hena-agent suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/hena-agent`) - padrões organizacionais
2. **Configuração global** (`~/.config/hena-agent/hena.json`) - preferências do usuário
3. **Configuração personalizada** (`HENA_AGENT_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`hena-agent.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.hena-agent`** - agentes, comandos, plugins
6. **Configuração inline** (`HENA_AGENT_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.hena-agent` e `~/.config/hena-agent` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/hena-agent`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do hena-agent em `~/.config/hena-agent/hena.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/hena-agent/tui.json`.
...
Adicione `hena-agent.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o hena-agent é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `HENA_AGENT_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `HENA_AGENT_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.hena-agent`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.hena-agent`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`hena.dev/config.json`**](https://hena.dev/config.json).

A configuração do TUI usa [**`hena.dev/tui.json`**](https://hena.dev/tui.json).
...
Use `HENA_AGENT_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `hena-agent.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `hena-agent serve` e `hena-agent web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor Hena Agent.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `hena-agent.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do hena-agent através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o hena-agent tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/hena-agent/agents/` ou `.hena-agent/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o hena-agent recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`hena-agent run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/hena-agent/commands/` ou `.hena-agent/commands/`. [Saiba mais aqui](/docs/commands).
...
O hena-agent fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o hena-agent **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o hena-agent com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.hena-agent/plugins/` ou `~/.config/hena-agent/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o hena-agent para usar apenas provedores específicos, em vez de desabilitá-los um a um.
