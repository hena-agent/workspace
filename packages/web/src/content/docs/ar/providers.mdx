---
title: المزوّدون
description: استخدام أي موفّر LLM في Hena Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Hena Agent ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Hena Agent.
...
في `~/.local/share/hena-agent/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Hena Agent.
...
Hena Zen هي قائمة نماذج يوفّرها فريق Hena Agent وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Hena Agent. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Hena Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر hena-agent، ثم انتقل إلى [hena.dev/auth](https://hena.dev/auth).
...
يعمل مثل أي مزوّد آخر في Hena Agent واستخدامه اختياري بالكامل.

---

## Hena Go

Hena Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Hena Agent، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Hena Agent.

1. شغّل الأمر `/connect` في TUI، واختر `Hena Go`، ثم انتقل إلى [hena.dev/auth](https://hena.dev/zen).
...
لاستخدام Amazon Bedrock مع Hena Agent:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل hena-agent:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `hena-agent.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Hena Agent غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين hena-agent لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل hena-agent بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات Hena Agent.
...
تخزين الرمز في مساحة تخزين مصادقة Hena Agent.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Hena Agent نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Hena Agent
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`hena-agent.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع hena-agent:
...
لاستخدام Google Vertex AI مع Hena Agent:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر hena-agent، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Hena Agent تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Hena Agent مدرجة كجلسة مستقلة.
...
يمكنك ضبط hena-agent لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط hena-agent لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط hena-agent لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Hena Agent. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Hena Agent:
...
3. انسخ مفتاح API لاستخدامه في Hena Agent.
...
6. **مهم**: قبل استخدام نماذج السحابة في Hena Agent، يجب سحب معلومات النموذج محليا:
...
Hena Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Hena Agent. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Hena Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Hena Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Hena Agent.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات Hena Agent.
...
5. يمكنك أيضا تخصيصها عبر إعدادات Hena Agent. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Hena Agent:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات Hena Agent. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع Hena Agent. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `hena-agent.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Hena Agent بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `hena-agent auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات hena-agent وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات Hena Agent.
