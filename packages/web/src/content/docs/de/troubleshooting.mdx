...
Wenn Hena Agent Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\hena-agent\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `hena-agent --log-level DEBUG`.

---

## Speicher

hena-agent speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/hena-agent/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\hena-agent`
...
Hena Agent Desktop startet im Hintergrund einen lokalen Hena Agent-Server (`hena-agent-cli`-Sidecar).
...
- Nur macOS: `Hena Agent`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/hena-agent/hena.jsonc` (oder `~/.config/hena-agent/hena.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/hena-agent/hena.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\hena-agent\hena-agent.jsonc`
...
Hena Agent kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/hena-agent/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\hena-agent\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Hena Agent ihn neu aufbauen kann.

1. Beende Hena Agent Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/hena-agent`
- **Linux**: loesche `~/.cache/hena-agent` (oder fuehre aus: `rm -rf ~/.cache/hena-agent`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\hena-agent`

3. Starte Hena Agent Desktop neu.

---

### Server-Verbindungsprobleme beheben

Hena Agent Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `hena-agent.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `HENA_AGENT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `HENA_AGENT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Hena Agent Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Hena Agent-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Hena Agent Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Hena Agent in den OS-Einstellungen aktiviert sind, und
...
1. Beende Hena Agent Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Hena Agent Desktop):

- `hena-agent.settings.dat` (desktop default server URL)
- `hena-agent.global.dat` und `hena-agent.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Hena Agent hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `hena-agent models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\hena-agent`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. hena-agent installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\hena-agent`

2. Starte hena-agent neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt hena-agent, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
hena-agent erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
