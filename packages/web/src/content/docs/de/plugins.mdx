---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Hena Agent zu erweitern.
---

Mit Plugins können Sie Hena Agent erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Hena Agent zu ändern.
...
- `.hena-agent/plugins/` – Plugins auf Projektebene
- `~/.config/hena-agent/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/hena-agent/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/hena-agent/hena.json`)
2. Projektkonfiguration (`hena-agent.json`)
3. Globales Plugin-Verzeichnis (`~/.config/hena-agent/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.hena-agent/plugins/`)
...
Hena Agent führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Hena Agent-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Hena Agent erweitern können.
...
:::note
Wenn Sie die Hena Agent-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Hena Agent `.env`-Dateien liest:
...
Plugins können Hena Agent auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Hena Agent aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Hena Agent zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://hena.dev/docs/sdk).
