...
Les compétences d'agent permettent à Hena Agent de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Hena Agent recherche ces emplacements :

- Configuration du projet : `.hena-agent/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/hena-agent/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Hena Agent remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.hena-agent/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/hena-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.hena-agent/skills/git-release/SKILL.md` comme ceci :
...
Hena Agent répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `hena-agent.json` :
...
**Pour les agents intégrés** (dans `hena-agent.json`) :
