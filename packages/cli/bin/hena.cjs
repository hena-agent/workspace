...
const envPath = process.env.HENA_BIN_PATH
const scriptDir = path.dirname(fs.realpathSync(__filename))
const cached = path.join(scriptDir, ".hena")
const platform = { darwin: "darwin", linux: "linux", win32: "windows" }[os.platform()] || os.platform()
const arch = { x64: "x64", arm64: "arm64", arm: "arm" }[os.arch()] || os.arch()
const base = "@hena/cli-" + platform + "-" + arch
const binary = platform === "windows" ? "hena.exe" : "hena"
...
if (!resolved) {
  console.error(
    "It seems that your package manager failed to install the right Hena CLI package. Try manually installing " +
