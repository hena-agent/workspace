...
Você pode adicionar ferramentas externas ao hena-agent usando o _Model Context Protocol_, ou MCP. O hena-agent suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do hena-agent](https://hena.dev/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/hena-agent`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O hena-agent lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o hena-agent irá:
...
Se o servidor requer autenticação, o hena-agent solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `hena-agent mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o hena-agent armazenará os tokens de forma segura em `~/.local/share/hena-agent/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no hena-agent, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do hena-agent como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o hena-agent à sua conta do Sentry.
