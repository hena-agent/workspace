...
import type { Event, HenaClient } from "@hena/sdk/v2"
...
type PermissionReplyParams = Parameters<HenaClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "hena-acp-permission-"))
