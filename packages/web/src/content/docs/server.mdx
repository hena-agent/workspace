---
title: Server
description: Interact with hena server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

The `hena serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an hena client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `hena.local` |
...
Set `HENA_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `hena`, or set `HENA_SERVER_USERNAME` to override it. This applies to both `hena serve` and `hena web`.
...
When you run `hena` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the hena server to interact with hena programmatically.
:::

This architecture lets hena support multiple clients and allows you to interact with hena programmatically.

You can run `hena serve` to start a standalone server. If you have the
hena TUI running, `hena serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Hena [IDE](/docs/ide) plugins.
...
The hena server exposes the following APIs.
