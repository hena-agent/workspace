...
Med Hena Agent kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Hena Agent vårt eget `hena-agent`-tema.
...
Hena Agent støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/hena-agent/themes/*.json` eller `$XDG_CONFIG_HOME/hena-agent/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.hena-agent/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.hena-agent/themes/*.json`
