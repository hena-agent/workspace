---
title: LSP serveri
description: Hena se integriše sa vašim LSP serverima.
---

Hena se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Hena dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `HENA_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i hena otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj hena konfiguraciji.
