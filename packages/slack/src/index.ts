...
import { createHena, type ToolPart } from "@hena/sdk"
...
console.log("🚀 Starting Hena server...")
const hena = await createHena({
  port: 0,
})
console.log("✅ Hena server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await hena.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new Hena session...")
    const { client, server } = hena
    ...
    console.log("✅ Created Hena session:", createResult.data.id)
  ...
  console.log("📝 Sending to Hena:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Hena response:", JSON.stringify(result, null, 2))
