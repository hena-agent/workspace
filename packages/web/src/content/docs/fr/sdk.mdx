---
title: SDK
description: Client JS de type sécurisé pour le serveur Hena Agent.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK hena-agent JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler hena-agent par programme.
...
Créez une instance de hena-agent :

```javascript
import { createHenaAgent } from "@hena-agent/sdk"

const { client } = await createHenaAgent()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `hena-agent.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createHenaAgent } from "@hena-agent/sdk"

const agent = await createHenaAgent({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
Si vous disposez déjà d'une instance en cours d'exécution de hena-agent, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@hena-agent/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*hena-agent" },
})
