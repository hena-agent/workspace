...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips hena-agent-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-hena-agent-directory"]).toBeUndefined()
      expect(forwarded["x-hena-agent-workspace"]).toBeUndefined()
