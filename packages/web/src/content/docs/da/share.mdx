...
Hena Agents delingsfunksjon lar deg lage offentlige lenker til dine Hena Agent-samtaler, slik at du kan samarbeide med lagkamerater eller få hjelp fra andre.
...
3. Gør samtalen tilgængelig via den delbare lenken — `hena.dev/s/<share-id>`
...
Som standard bruger Hena Agent manuell delingsmodus. Sessioner deles ikke automatisk, men du kan dele dem manuelt ved at bruge kommandoen `/share`:
...
For at håndheve dette på tvers av teamet ditt for et gitt prosjekt, tilføj det til `hena-agent.json` i prosjektet ditt og tjek inn i Git.
...
[Finn ut mer](/docs/enterprise) om brug av hena-agent i organisasjonen din.
