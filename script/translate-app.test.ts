...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".hena-agent/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".hena-agent/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".hena-agent/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "hena-agent/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "hena-agent/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "hena-agent/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
