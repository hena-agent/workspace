...
Go works like any other provider in Hena. You subscribe to Hena Go and
...
Hena Go works like any other provider in Hena.
...
uses the format `hena-go/<model-id>`. For example, for Kimi K3, you would
use `hena-go/kimi-k3` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Hena as well.
