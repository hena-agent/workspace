...
import { InstallationChannel } from "@hena/core/installation/version"
  ...
  describe("latest", () => {
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(npmCalls).toContain(`https://registry.npmjs.org/hena/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(bunCalls).toContain(`https://registry.npmjs.org/hena/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(pnpmCalls).toContain(`https://registry.npmjs.org/hena/${InstallationChannel}`)
      }),
    )
    ...
    testEffect(
      testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("hena")) return "hena"
          return ""
        },
      ),
      ...
      testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes("--json=v2")) {
            expect(args).toContain("hena-agent/tap/hena")
            return brewInfoJson
          }
          return ""
        },
