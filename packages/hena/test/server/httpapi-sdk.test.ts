...
type Sdk = ReturnType<typeof createHenaClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.HENA_SERVER_PASSWORD = input?.password
      Flag.HENA_SERVER_USERNAME = input?.username
...
function capture(request: () => Promise<SdkResult>) {
  return call(request).pipe(
    Effect.map((result) => {
      if (!result.response) throw result.error ?? new Error("SDK request failed without an HTTP response")
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".hena", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-hena-directory")).toBe(false)
        expect(request!.headers.has("x-hena-workspace")).toBe(false)
      }),
    ),
  )
