...
Agentferdigheter lar Hena Agent oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Hena Agent søker etter disse stedene:

- Prosjektkonfigurasjon: `.hena-agent/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/hena-agent/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Hena Agent opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.hena-agent/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/hena-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.hena-agent/skills/git-release/SKILL.md` slik:
...
Hena Agent viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `hena-agent.json`:
...
**For innebygde agenter** (i `hena-agent.json`):
