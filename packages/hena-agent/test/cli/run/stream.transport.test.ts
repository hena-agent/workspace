...
import { HenaAgentClient, type GlobalEvent } from "@hena-agent/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<HenaAgentClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<HenaAgentClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<HenaAgentClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<HenaAgentClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<HenaAgentClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://hena-agent.test"),
...
function sdk(
  input: {
    ...
    subscribe?: HenaAgentClient["event"]["subscribe"]
    globalEvent?: HenaAgentClient["global"]["event"]
    promptAsync?: HenaAgentClient["session"]["promptAsync"]
    status?: HenaAgentClient["session"]["status"]
    messages?: HenaAgentClient["session"]["messages"]
    children?: HenaAgentClient["session"]["children"]
    permissions?: HenaAgentClient["permission"]["list"]
    questions?: HenaAgentClient["question"]["list"]
  } = {},
) {
  const client = new HenaAgentClient()

  const subscribe: HenaAgentClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: HenaAgentClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: HenaAgentClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: HenaAgentClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: HenaAgentClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: HenaAgentClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: HenaAgentClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: HenaAgentClient["question"]["list"] = input.questions ?? (() => ok([]))
