...
const withoutWatcher = <A, E, R>(effect: Effect.Effect<A, E, R>) => {
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.HENA_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER
      process.env.HENA_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
      return previous
    }),
    () => effect,
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.HENA_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER
        else process.env.HENA_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER = previous
      }),
  )
}
