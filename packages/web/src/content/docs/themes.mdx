...
With Hena you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Hena uses our own `hena` theme.
...
Hena comes with several built-in themes.
...
- Want Hena to match their terminal's appearance
...
Hena supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/hena/themes/*.json` or `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.hena/themes/*.json`
4. **Current working directory** - Defined in `./.hena/themes/*.json`
