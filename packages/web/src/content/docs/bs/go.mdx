...
Hena Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Hena Agent-u. Pretplatite se na Hena Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Hena Agent.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Hena Agent-om.
:::
...
Hena Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Hena Go radi kao bilo koji drugi provajder u Hena Agent-u.

1. Prijavite se na **<a href={console}>Hena Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Hena Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Hena Go.
:::
...
Hena Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Hena Agent konfiguraciji
koristi format `hena-agent-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`hena-agent-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo Hena Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Hena Agent koristite i bilo kojeg drugog provajdera.
