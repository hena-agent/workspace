...
import { isRecord } from "@hena/tui/util/record"
...
export class Service extends Context.Service<Service, Interface>()("@hena/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old hena.json shape) still get their settings applied.
  ...
  // folders between cwd and home, and HENA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.HENA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit HENA_TUI_CONFIG override, if set.
  if (Flag.HENA_TUI_CONFIG) {
    const configFile = Flag.HENA_TUI_CONFIG
  ...
  // 4. Project config directories (and HENA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".hena") || dir === Flag.HENA_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".hena") && dir !== Flag.HENA_CONFIG_DIR) continue
