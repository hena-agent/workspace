---
title: Konfiguration
description: Ved at bruge Hena Agent JSON konfig.
---

Du kan konfigurere Hena Agent ved hjælp af en JSON-konfigurationsfil.

---

## Format

Hena Agent understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/hena-agent`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/hena-agent/hena.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`HENA_AGENT_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`hena-agent.json` i projekt) - projektspecifikke indstillinger
5. **`.hena-agent` mapper** - agent, kommandoer, plugins
6. **Inline config** (`HENA_AGENT_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.hena-agent` og `~/.config/hena-agent` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/hena-agent`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Hena Agent-konfiguration i `~/.config/hena-agent/hena.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `hena-agent.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Hena Agent starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `HENA_AGENT_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `HENA_AGENT_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.hena-agent` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.hena-agent` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`hena.dev/config.json`**](https://hena.dev/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `hena-agent serve` og `hena-agent web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Hena Agent-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `hena-agent.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Hena Agent-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Hena Agent at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Hena Agent-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/hena-agent/agents/` eller `.hena-agent/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Hena Agent falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`hena-agent run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/hena-agent/commands/` eller `.hena-agent/commands/`. [Learn more here](/docs/commands).
...
Hena Agent vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard hena-agent **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Hena Agent med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.hena-agent/plugins/` eller `~/.config/hena-agent/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Hena Agent til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
