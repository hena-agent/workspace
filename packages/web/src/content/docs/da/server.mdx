---
title: Server
description: Interager med hena-agent-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `hena-agent serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en hena-agent-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `hena-agent.local` |
...
Indstil `HENA_AGENT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `hena-agent`, eller indstil `HENA_AGENT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `hena-agent serve` og `hena-agent web`.
...
Når du kører `hena-agent` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug hena-agent-serveren til at interagere med hena-agent programmatisk.
:::

Denne arkitektur lader hena-agent understøtte flere klienter og lader dig interagere med hena-agent programmatisk.

Du kan køre `hena-agent serve` for at starte en selvstændig server. Hvis du har
hena-agent TUI kørende, vil `hena-agent serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Hena Agent [IDE](/docs/ide) plugins.
...
Hena Agent-serveren viser følgende API'er.
