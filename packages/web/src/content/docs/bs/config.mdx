---
title: Konfiguracija
description: Korištenje Hena JSON konfiguracije.
---

Možete konfigurirati Hena koristeći JSON konfiguracijski fajl.

---

## Format

Hena podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/hena`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/hena/hena.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`HENA_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`hena.json` u projektu) - postavke specifične za projekat
5. **`.hena` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`HENA_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.hena` i `~/.config/hena` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/hena` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Hena konfiguraciju u `~/.config/hena/hena.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/hena/tui.json`.
...
Dodajte `hena.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Hena pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `HENA_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `HENA_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.hena` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.hena`, tako da **može nadjačati** njihove postavke.
...
Koristite `HENA_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `hena.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `hena serve` i `hena web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Hena server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `hena.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Hena konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Hena pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/hena/agents/` ili `.hena/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Hena će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`hena run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/hena/commands/` ili `.hena/commands/`. [Saznajte više ovdje](/docs/commands).
...
Hena će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Hena **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Hena sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.hena/plugins/` ili `~/.config/hena/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Hena da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
