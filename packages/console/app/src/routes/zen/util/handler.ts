...
export async function handler(
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://hena.ai/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://hena.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://hena.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://hena.ai/workspace/${authInfo.workspaceID}/members`
