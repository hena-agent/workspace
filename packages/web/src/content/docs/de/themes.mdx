...
Mit Hena Agent kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Hena Agent das `hena-agent`-Theme.
...
Hena Agent liefert mehrere eingebaute Themes mit.
...
- Hena Agent optisch ans Terminal anpassen wollen
...
Hena Agent unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/hena-agent/themes/*.json` oder `$XDG_CONFIG_HOME/hena-agent/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.hena-agent/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.hena-agent/themes/*.json`
