---
title: 공유
description: Hena Agent 대화를 공유하세요.
---

hena-agent의 공유 기능은 hena-agent 대화에 대한 공개 링크를 만들 수 있으므로 Teammates와 협업하거나 다른 곳에서 도움을받을 수 있습니다.
...
대화를 공유할 때, hena-agent:

1. 세션에 대한 독특한 공개 URL 만들기
2. 우리의 서버에 대화 기록 동기화
3. 공유 가능한 링크를 통해 접근 할 수있는 대화 만들기 — `hena.dev/s/<share-id>`

---

## 공유

hena-agent는 대화가 공유되는 방법을 제어하는 세 가지 공유 모드를 지원합니다.

---

## 수동 (기본)

기본적으로 hena-agent는 수동 공유 모드를 사용합니다. 세션은 자동으로 공유되지 않지만 `/share` 명령을 사용하여 수동으로 공유 할 수 있습니다.
...
주어진 프로젝트를 위해 팀 전반에 걸쳐 이것을 시행하려면 프로젝트에 `hena-agent.json`에 추가하고 Git로 확인하십시오.
...
[Learn more](/docs/enterprise) 귀하의 조직에서 hena-agent를 사용하여.
