...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@hena-agent/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.HENA_AGENT_ZED_DB
  process.env.HENA_AGENT_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.HENA_AGENT_ZED_DB
    else process.env.HENA_AGENT_ZED_DB = previous
    home.mockRestore()
  }
})
