...
describe("hena-agent read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // HENA_AGENT_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const r = yield* henaAgent.spawn(["mcp", "list"])
        henaAgent.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const r = yield* henaAgent.spawn(["providers", "list"])
        henaAgent.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const r = yield* henaAgent.spawn(["models"])
        henaAgent.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const r = yield* henaAgent.spawn(["agent", "list"])
        henaAgent.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh HENA_AGENT_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const r = yield* henaAgent.spawn(["session", "list"])
        henaAgent.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const r = yield* henaAgent.spawn(["stats"])
        henaAgent.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under HENA_AGENT_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const r = yield* henaAgent.spawn(["db", "path"])
        henaAgent.expectExit(r, 0, "db path")
