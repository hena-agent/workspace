---
title: Plugins
description: Escreva seus próprios plugins para estender o Hena.
---

Plugins permitem que você estenda o hena conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do Hena.
...
- `.hena/plugins/` - Plugins em nível de projeto
- `~/.config/hena/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/hena/node_modules/`.
...
1. Configuração global (`~/.config/hena/hena.json`)
2. Configuração do projeto (`hena.json`)
3. Diretório de plugins global (`~/.config/hena/plugins/`)
4. Diretório de plugins do projeto (`.hena/plugins/`)
...
O hena executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do hena para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o Hena.
...
:::note
Se você estiver usando o aplicativo desktop hena, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o hena de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao hena:
...
O helper `tool` cria uma ferramenta personalizada que o hena pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o hena junto com as ferramentas integradas.
