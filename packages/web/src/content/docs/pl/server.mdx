---
title: Serwer
description: Interakcja z serwerem hena-agent za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `hena-agent serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient Hena Agent.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `hena-agent.local` |
...
Ustaw `HENA_AGENT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `hena-agent` lub ustaw `HENA_AGENT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `hena-agent serve`, jak i `hena-agent web`.
...
Kiedy uruchomisz `hena-agent`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera hena-agent do programowej interakcji z Hena Agent.
:::

Ta architektura umożliwia obsługę wielu klientów przez hena-agent i programową interakcję z kodem otwartym.

Możesz uruchomić `hena-agent serve`, aby uruchomić samodzielny serwer. Jeśli masz
hena-agent TUI działa, `hena-agent serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki hena-agent [IDE](/docs/ide).
...
Serwer hena-agent udostępnia następujące interfejsy API.
