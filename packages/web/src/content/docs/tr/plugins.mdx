---
title: Eklentiler
description: Hena'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Hena'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Hena'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.hena/plugins/` - Proje düzeyinde eklentiler
- `~/.config/hena/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/hena/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/hena/hena.json`)
2. Project config (`hena.json`)
3. Global eklenti dizini (`~/.config/hena/plugins/`)
4. Proje eklenti dizini (`.hena/plugins/`)
...
Hena bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Hena SDK istemcisi.
...
Hena'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Hena masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Hena'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Hena'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Hena'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Hena'a da hazır olacaktır.
