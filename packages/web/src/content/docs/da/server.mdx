---
title: Server
description: Interager med hena-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `hena serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en hena-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `hena.local` |
...
Indstil `HENA_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `hena`, eller indstil `HENA_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `hena serve` og `hena web`.
...
Når du kører `hena` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug hena-serveren til at interagere med hena programmatisk.
:::

Denne arkitektur lader hena understøtte flere klienter og lader dig interagere med hena programmatisk.

Du kan køre `hena serve` for at starte en selvstændig server. Hvis du har
hena TUI kørende, vil `hena serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Hena [IDE](/docs/ide) plugins.
...
Hena-serveren viser følgende API'er.
