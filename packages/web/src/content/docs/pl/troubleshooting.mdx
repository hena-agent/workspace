...
Aby debugować problemy z hena-agent, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\hena-agent\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `hena-agent --log-level DEBUG`.

---

## Przechowywanie danych

hena-agent przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/hena-agent/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\hena-agent`
...
hena-agent Desktop uruchamia w tle lokalny serwer hena-agent (sidecar `hena-agent-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Hena Agent` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/hena-agent/hena.jsonc` (lub `~/.config/hena-agent/hena.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/hena-agent/hena.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\hena-agent\hena-agent.jsonc`
...
hena-agent może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/hena-agent/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\hena-agent\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby hena-agent mógł ją odbudować.

1. Całkowicie zamknij hena-agent Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/hena-agent`
- **Linux**: usuń `~/.cache/hena-agent` (lub uruchom `rm -rf ~/.cache/hena-agent`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\hena-agent`

3. Uruchom ponownie hena-agent Desktop.

---

### Napraw problemy z połączeniem z serwerem

hena-agent Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `hena-agent.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `HENA_AGENT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `HENA_AGENT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows hena-agent Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami Hena Agent.

---

### Brak powiadomień

hena-agent Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla hena-agent w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie hena-agent Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji hena-agent Desktop):

- `hena-agent.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `hena-agent.global.dat` i `hena-agent.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z hena-agent:
...
3. Upewnij się, że masz najnowszą wersję z `hena-agent upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `hena-agent models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\hena-agent`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. hena-agent dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\hena-agent`

2. Uruchom ponownie kod hena-agent, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to hena-agent do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
hena-agent wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
