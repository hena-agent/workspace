---
title: Server
description: Komunicirajte s hena-agent serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `hena-agent serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji hena-agent klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `hena-agent.local` |
...
Postavite `HENA_AGENT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `hena-agent`, ili postavite `HENA_AGENT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `hena-agent serve` i za `hena-agent web`.
...
Kada pokrenete `hena-agent`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite hena-agent server za programsku interakciju sa hena-agent-om.
:::

Ova arhitektura omogucava hena-agent podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `hena-agent serve` da startate standalone server. Ako je hena-agent TUI vec pokrenut, `hena-agent serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Hena Agent [IDE](/docs/ide) pluginovi.
...
hena-agent server izlaže sljedece API-je.
