...
- Running `bun dev` from `packages/hena-agent` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s hena-agent-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt hena-agent-dev`.
- Stop the session explicitly when done: `tmux kill-session -t hena-agent-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@hena-agent/Foo") {}
