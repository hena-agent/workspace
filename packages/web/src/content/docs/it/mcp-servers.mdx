...
Puoi aggiungere strumenti esterni a Hena usando il _Model Context Protocol_, o MCP. Hena supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Hena Config](https://hena.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/hena`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Hena gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Hena:
...
Se il server richiede autenticazione, Hena ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `hena mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Hena memorizzera' i token in modo sicuro in `~/.local/share/hena/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Hena insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Hena come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Hena al tuo account Sentry.
