...
Avec Hena Agent, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Hena Agent utilise notre propre thème `hena-agent`.
...
Hena Agent est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Hena Agent corresponde à l'apparence de leur terminal
...
Hena Agent prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/hena-agent/themes/*.json` ou `$XDG_CONFIG_HOME/hena-agent/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.hena-agent/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.hena-agent/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/hena-agent/themes
vim ~/.config/hena-agent/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .hena-agent/themes
vim .hena-agent/themes/my-theme.json
```
...
{
  "$schema": "https://hena.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
