---
title: TUI
description: استخدام واجهة المستخدم TUI في Hena.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Hena واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Hena إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Hena في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Hena. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Hena. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `hena.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Hena بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `hena.default`.
