...
- Do not add Hena-specific tables, paths, migrations, post-commit hooks, or domain storage APIs here.
...
- If touching copied Drizzle internals, compare with the installed `drizzle-orm` declarations and runtime JS (currently `1.0.0-rc.4`).
...
- `src/internal/drizzle-utils.ts`: local typed shims for Drizzle runtime internals that are not exposed in declarations.
