// Subprocess test harness for the hena CLI. Spawns the real binary against
...
//
// Configuration flows through hena's built-in test affordances:
//   - HENA_CONFIG_CONTENT      : provider config inline, no files to find
//   - HENA_TEST_HOME           : pins os.homedir() → tmpdir
//   - HENA_DISABLE_PROJECT_CONFIG : skip walking up for hena.json
//   - HENA_PURE                : skip external plugin discovery + install
//   - HENA_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `hena.run` is fully wired. The shape supports adding more
// builders (`hena.serve(opts)`, `hena.acp(opts)`, `hena.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of HenaCli.
...
const henaRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(henaRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    HENA_CONFIG_CONTENT: configJson,
...
// Typed equivalent of constructing argv for `hena run`. New flags should
...
// `hena serve` is a long-lived process — it never exits on its own.
...
// `hena acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type HenaCli = {
  ...
  // Spawn `hena serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `hena acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("hena.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          HENA_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("hena.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("hena.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "hena server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `hena serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("hena.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const hena: HenaCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, hena })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `hena.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "hena") {
...
// every call site. Use this for any test that needs the hena CLI fixture.
//
...
// resources (e.g. `hena.serve`) without an extra `Effect.scoped` wrapper —
