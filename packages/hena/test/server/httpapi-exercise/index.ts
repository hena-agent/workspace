 ...
 *
 * The script intentionally isolates `HENA_DB` before importing modules that touch
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "hena.jsonc"),
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-hena-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-hena-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "hena", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-hena-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
...
const main = Effect.gen(function* () {
  yield* Effect.addFinalizer(() =>
    Effect.promise(() => disposeApps()).pipe(
