...
Možete dodati vanjske alate u Hena Agent koristeći _Model Context Protocol_, ili MCP. Hena Agent podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Hena Agent Config](https://hena.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/hena-agent`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Hena Agent automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Hena Agent će:
...
Ako server zahtijeva autentifikaciju, Hena Agent će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `hena-agent mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Hena Agent sigurno čuva tokene u `~/.local/share/hena-agent/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Hena Agent, zajedno s ugrađenim alatima. Možete njima upravljati kroz Hena Agent konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Hena Agent sa vašim Sentry nalogom.
