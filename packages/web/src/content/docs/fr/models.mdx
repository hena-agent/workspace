...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Hena Agent.
...
Voici plusieurs modèles qui fonctionnent bien avec Hena Agent, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Hena Agent.

```json title="hena-agent.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Hena Zen](/docs/zen), vous utiliserez `hena-agent/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="hena-agent.jsonc" {7-12,19-24}
{
  "$schema": "https://hena.dev/config.json",
  "provider": {
    "openai": {
...
```jsonc title="hena-agent.jsonc" {6-21}
{
  "$schema": "https://hena.dev/config.json",
  "provider": {
    "hena-agent": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Hena Agent est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Hena Agent est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="hena-agent.jsonc" {7-18}
{
  "$schema": "https://hena.dev/config.json",
  "provider": {
    "openai": {
...
Lorsque Hena Agent démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Hena Agent.
