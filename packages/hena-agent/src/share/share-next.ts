...
import type * as SDK from "@hena-agent/sdk/v2"
...
import { SessionShareTable } from "@hena-agent/core/share/sql"
...
const disabled = process.env["HENA_AGENT_DISABLE_SHARE"] === "true" || process.env["HENA_AGENT_DISABLE_SHARE"] === "1"
...
export class Service extends Context.Service<Service, Interface>()("@hena-agent/ShareNext") {}
  ...
  Effect.gen(function* () {
    ...
    const request = Effect.fn("ShareNext.request")(function* () {
      ...
      if (Option.isNone(active) || !active.value.active_org_id) {
        const baseUrl = (yield* cfg.get()).enterprise?.url ?? "https://hena.ai"
