---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Hena Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena Agent koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Hena Agent konfiguraciji.
...
u `~/.local/share/hena-agent/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Hena Agent
config.
...
Hena Zen je lista modela koje je obezbedio Hena Agent tim koji su bili
testirano i potvrđeno da dobro radi sa Hena Agent. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Hena Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite hena-agent i idite na [hena.dev/auth](https://hena.dev/auth).
...
Radi kao i svaki drugi provajder u Hena Agent i potpuno je opcionalan za korištenje.

---

## Hena Go

Hena Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Hena Agent tim i koji su testirani i verificirani da dobro rade s Hena Agent-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Hena Go` i idite na [hena.dev/auth](https://hena.dev/zen).
...
Da biste koristili Amazon Bedrock s Hena Agent:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete hena-agent:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `hena-agent.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Hena Agent.
:::
...
Možete konfigurirati hena-agent za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi hena-agent ispravno radio.
   :::
...
Također možete dodati modele kroz svoju hena-agent konfiguraciju.
...
da pohrani token u hena-agent auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Hena Agent koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Hena Agent
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`hena-agent.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `hena-agent.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s hena-agent:
...
Za korištenje Google Vertex AI s Hena Agent:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće Hena Agent.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem hena-agenta, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Hena Agent razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Hena Agent razgovor naveden kao zasebna sesija.
...
Možete konfigurirati hena-agent za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati hena-agent za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati hena-agent za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Hena Agent. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Hena Agent:
...
3. Kopirajte API ključ za korištenje u Hena Agent.
...
6. **Važno**: Prije upotrebe modela oblaka u Hena Agent, morate lokalno povući informacije o modelu:
...
Hena Zen je lista testiranih i verifikovanih modela koju je obezbedio Hena Agent tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Hena Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Hena Zen**.

```txt
   /connect
```

3. Unesite svoj Hena Agent API ključ.
...
Također možete dodati dodatne modele putem vaše hena-agent konfiguracije.
...
5. Također ih možete prilagoditi putem vaše hena-agent konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Hena Agentom:
...
Također možete prilagoditi modele kroz svoju hena-agent konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s hena-agent-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `hena-agent.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Hena Agent da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `hena-agent auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Hena Agent konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj hena-agent konfiguraciji.
