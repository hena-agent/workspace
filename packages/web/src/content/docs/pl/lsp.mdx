---
title: Serwery LSP
description: Hena Agent integruje się z serwerami LSP.
---

Hena Agent może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

Hena Agent posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `HENA_AGENT_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i Hena Agent otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji Hena Agent.
