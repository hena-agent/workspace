...
U Hena Agent mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Hena Agent koristi nasu `hena-agent` temu.
...
Hena Agent dolazi sa vise ugradenih tema.
...
- Zele da Hena Agent odgovara izgledu njihovog terminala
...
Hena Agent podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/hena-agent/themes/*.json` ili `$XDG_CONFIG_HOME/hena-agent/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.hena-agent/themes/*.json`
4. **Trenutni radni direktorij** - `./.hena-agent/themes/*.json`
