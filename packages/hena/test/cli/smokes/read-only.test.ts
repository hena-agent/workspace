...
describe("hena read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // HENA_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ hena }) =>
      Effect.gen(function* () {
        const r = yield* hena.spawn(["mcp", "list"])
        hena.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ hena }) =>
      Effect.gen(function* () {
        const r = yield* hena.spawn(["providers", "list"])
        hena.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ hena }) =>
      Effect.gen(function* () {
        const r = yield* hena.spawn(["models"])
        hena.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ hena }) =>
      Effect.gen(function* () {
        const r = yield* hena.spawn(["agent", "list"])
        hena.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh HENA_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ hena }) =>
      Effect.gen(function* () {
        const r = yield* hena.spawn(["session", "list"])
        hena.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ hena }) =>
      Effect.gen(function* () {
        const r = yield* hena.spawn(["stats"])
        hena.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under HENA_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ hena }) =>
      Effect.gen(function* () {
        const r = yield* hena.spawn(["db", "path"])
        hena.expectExit(r, 0, "db path")
