...
Ajan becerileri, hena'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
hena su konumlari tarar:

- Proje konfigurasyonu: `.hena/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/hena/skills/<name>/SKILL.md`
...
Proje yerel yollarinda hena, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.hena/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/hena/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.hena/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
hena, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`hena.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`hena.json` icinde):
