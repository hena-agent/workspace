...
hena-agent, belirli bir eylemin otomatik olarak mı çalıştırılacağına, size sorulacağına veya engelleneceğine karar vermek için `permission` yapılandırmasını kullanır.
...
hena-agent'un başlatıldığı çalışma dizini dışındaki yollara dokunan araç çağrılarına izin vermek için `external_directory` kullanın. Bu, girdi olarak bir yolu alan tüm araçlar için geçerlidir (örneğin `read`, `edit`, `glob`, `grep` ve birçok `bash` komutu).
...
hena-agent izinleri araç adına ve birkaç güvenlik önlemine göre anahtarlanır:
...
Hiçbir şey belirtmezseniz hena-agent izin verilen varsayılanlardan başlar:
...
hena-agent onay istediğinde kullanıcı arayüzü üç sonuç sunar:

- `once` — yalnızca bu isteği onayla
- `always` — önerilen kalıplarla eşleşen gelecekteki istekleri onaylayın (geçerli hena-agent oturumunun geri kalanı için)
