...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve hena-agent route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "hena-agent")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "hena-agent")).toBe("openai")
    expect(statProvider("big-pickle", "", "hena-agent")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "hena-agent"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep hena-agent as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "hena-agent"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "hena-agent"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep hena-agent or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "hena-agent"), country: "US" })).toMatchObject([
