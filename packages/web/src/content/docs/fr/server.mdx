---
title: Serveur
description: Interagissez avec le serveur hena-agent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `hena-agent serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client hena-agent peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `hena-agent.local` |
...
Définissez `HENA_AGENT_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `hena-agent`, ou définissez `HENA_AGENT_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `hena-agent serve` et à `hena-agent web`.
...
Lorsque vous exécutez `hena-agent`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur hena-agent pour interagir avec hena-agent par programmation.
:::

Cette architecture permet à hena-agent de prendre en charge plusieurs clients et vous permet d'interagir avec hena-agent par programme.

Vous pouvez exécuter `hena-agent serve` pour démarrer un serveur autonome. Si vous avez le
hena-agent TUI en cours d'exécution, `hena-agent serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Hena Agent [IDE](/docs/ide).
...
Le serveur hena-agent expose les API suivantes.
