...
import { createHenaAgent } from "@hena-agent/sdk"
...
`)
  ...
  const henaAgent = await createHenaAgent({ port: 0 })
  ...
  try {
    ...
    const session = await henaAgent.client.session.create()
    const result = await henaAgent.client.session
      .prompt({
