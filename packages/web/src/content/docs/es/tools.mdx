...
Las herramientas permiten que LLM realice acciones en su código base. Hena Agent viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Hena Agent.
...
:::note
Esta herramienta solo está disponible cuando `HENA_AGENT_EXPERIMENTAL_LSP_TOOL=true` (o `HENA_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Hena Agent o cuando la variable de entorno `HENA_AGENT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Hena Agent:

```bash
HENA_AGENT_ENABLE_EXA=1 hena-agent
```
