...
Możesz dodać dodatkowe informacje do hena-agent za pomocą _Model Context Protocol_, w skrócie MCP. hena-agent obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [hena-agent Config](https://hena.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/hena-agent`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
hena-agent automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, hena-agent:
...
Jeśli serwer wymaga uwierzytelnienia, hena-agent poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `hena-agent mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji hena-agent bezpieczne przechowa tokeny w `~/.local/share/hena-agent/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w hena-agent, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie hena-agent, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć hena-agent z kontem Sentry.
