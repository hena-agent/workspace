...
import { AbsolutePath, Agent, HenaAgent, Location, Model, Prompt, Session, SessionMessage } from "../src/effect"
...
test("sessions.get returns the decoded Effect projection", async () => {
  ...
  const result = await Effect.gen(function* () {
    const client = yield* HenaAgent.make({ baseUrl: "http://localhost:3000" })
