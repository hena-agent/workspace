...
export const InviteEmail = ({
  ...
  return (
    <Html lang="en">
      <Head>
        <Title>{`Hena Agent — ${messagePlain}`}</Title>
      </Head>
            ...
            <Row>
              <Column>
                <A href={`${CONSOLE_URL}zen`}>
                  <Img height="32" alt="Hena Agent Logo" src={`${assetsUrl}/logo.png`} />
                </A>
              </Column>
            </Row>

            <Section style={{ padding: `${unit * 2}px 0 0 0` }}>
              <Text style={headingText}>Join your team's Hena Agent workspace</Text>
              <Text style={contentText}>
                You have been invited by <Span style={contentHighlightText}>{inviter}</Span> to join the{" "}
                <Span style={contentHighlightText}>{workspaceName}</Span> workspace on Hena Agent.
              </Text>
            </Section>
