...
> current private `@hena/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@hena/ai`
...
- Secondary audience: Hena and other durable agent runtimes
...
import { LLM } from "@hena/ai"
import { OpenAI } from "@hena/ai/providers/openai"
...
Hena and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@hena/ai/promise"
import { OpenAI } from "@hena/ai/providers/openai"
...
```ts
import { LLMSchema } from "@hena/ai/schema"
...
```ts
import { Provider, Protocol } from "@hena/ai/provider"
...
@hena/ai/providers/openai
@hena/ai/providers/anthropic
@hena/ai/providers/google
...
```
...
Hena should migrate to `generateTurn` / `streamTurn`, preserving its durable
