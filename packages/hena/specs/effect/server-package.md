...
Practical reference for a future `packages/server` split after the Hena
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/hena`.
...
- `packages/hena` - CLI entrypoints and the legacy server host
...
Until enough shared service code lives outside `packages/hena`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/hena`

It should not import `packages/hena` services while `packages/hena`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on Hena-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/hena` implementation details.
