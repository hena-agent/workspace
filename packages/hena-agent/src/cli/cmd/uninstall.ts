...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall Hena Agent and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Hena Agent")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g hena-agent",
      pnpm: "pnpm uninstall -g hena-agent",
      bun: "bun remove -g hena-agent",
      yarn: "yarn global remove hena-agent",
      brew: "brew uninstall hena-agent",
      choco: "choco uninstall hena-agent",
      scoop: "scoop uninstall hena-agent",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "hena-agent", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Hena Agent!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# hena-agent") || content.includes(".hena-agent/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".hena-agent/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }
    ...
    if (
      (trimmed.startsWith("export PATH=") &&
        trimmed.includes(".hena-agent/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".hena-agent"))
    ) {
      continue
    }
