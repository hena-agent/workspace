...
| HttpApi modes can share one runner                                                                        | Tried `bun run --parallel` for coverage, auth, and effect                                       | ~160s     | failed   | discard  | Auth blocked beyond 600s while effect consumed the runner. Isolate the modes in separate CI matrix jobs instead.                    |
