---
title: Konfiguracja
description: Ustawienia Hena Agent w pliku JSON.
---

Możesz dostosować Hena Agent za pomocą pliku konfiguracyjnego JSON.

---

## Format

Hena Agent obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "hena-agent"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/hena-agent`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/hena-agent/hena.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `HENA_AGENT_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`hena-agent.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.hena-agent`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `HENA_AGENT_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.hena-agent` i `~/.config/hena-agent` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/hena-agent`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Hena Agent w `~/.config/hena-agent/hena.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/hena-agent/tui.json`.
...
Dodaj `hena-agent.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Hena Agent się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `HENA_AGENT_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `HENA_AGENT_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.hena-agent` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.hena-agent`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`hena.dev/config.json`**](https://hena.dev/config.json).
...
Użyj `HENA_AGENT_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `hena-agent.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `hena-agent serve` i `hena-agent web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Hena Agent.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `hena-agent.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Hena Agent wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/hena-agent/agents/` lub `.hena-agent/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Hena Agent powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`hena-agent run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/hena-agent/commands/` lub `.hena-agent/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Hena Agent może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Hena Agent nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Hena Agent **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Hena Agent o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.hena-agent/plugins/` lub `~/.config/hena-agent/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Hena Agent tylko do określonych dostawców.
