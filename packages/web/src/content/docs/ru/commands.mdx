...
Создайте `.hena/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию hena или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем hena [config](/docs/config):
...
- Глобальный: `~/.config/hena/commands/`
- Для каждого проекта: `.hena/commands/`
...
hena включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
