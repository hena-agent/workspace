---
title: カスタムツール
description: LLM が Hena Agent で呼び出すことができるツールを作成します。
---

カスタムツールは、会話中に LLM が呼び出すことができる作成した関数です。これらは、Hena Agent の[組み込みツール](/docs/tools) (`read`、`write`、`bash` など) と連携して動作します。
...
- ローカルでは、プロジェクトの `.hena-agent/tools/` ディレクトリに配置します。
- または、グローバルに、それらを `~/.config/hena-agent/tools/` に配置します。
