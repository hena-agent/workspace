---
title: Server
description: Komunicirajte s hena serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

Komanda `hena serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji hena klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `hena.local` |
...
Postavite `HENA_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `hena`, ili postavite `HENA_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `hena serve` i za `hena web`.
...
Kada pokrenete `hena`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite hena server za programsku interakciju sa hena-om.
:::

Ova arhitektura omogucava hena podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `hena serve` da startate standalone server. Ako je hena TUI vec pokrenut, `hena serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Hena [IDE](/docs/ide) pluginovi.
...
hena server izlaže sljedece API-je.
