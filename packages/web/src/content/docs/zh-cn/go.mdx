...
Go 的工作方式与 Hena 中的任何其他提供商（provider）一样。订阅 Hena Go 后你将获得 API 密钥。它是 **完全可选** 的，并非使用 Hena 所必需的条件。
...
:::tip
我们测试了一组经过精选且与 Hena 配合良好的模型和提供商。
:::
...
你的 Hena 配置中的 [模型 ID](/docs/config/#models) 使用 `hena-go/<model-id>` 格式。例如，对于 Kimi K3，你将在配置中使用 `hena-go/kimi-k3`。
...
4. **无锁定（no lock-in）**，允许你与 Hena 一起使用任何其他提供商。
