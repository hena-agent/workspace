...
import { ToolOutput, type ToolCall, type ToolDefinition, type ToolResultValue } from "@hena/llm"
...
export class Service extends Context.Service<Service, Interface>()("@hena/v2/ToolRegistry") {}
...
const registryLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      ...
      materialize: Effect.fn("ToolRegistry.materialize")(function* (permissions = [], allowed?: ReadonlySet<string>) {
        ...
        for (const [name, registration] of registrations)
          if ((allowed && !allowed.has(name)) || whollyDisabled(permission(registration.tool, name), permissions))
