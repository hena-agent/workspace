...
//
// Renders the full Hena Agent entry logo and a compact [H] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "Hena Agent", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `hena-agent --mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
