...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/hena.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/hena.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/hena.jsonc", dir: ".hena", suggestion: "hena" },
        expected:
          'Directory ".hena" in /tmp/hena.jsonc is not valid. Rename the directory to "hena" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        expected: "Configuration is invalid at /tmp/hena.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `hena models` to list available models",
      "Or check your config (hena.json) provider/model names",
    ].join("\n")
