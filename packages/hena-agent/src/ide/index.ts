...
import { IdeEvent } from "@hena-agent/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["HENA_AGENT_CALLER"] === "vscode" || process.env["HENA_AGENT_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.hena-agent"], {
    nothrow: true,
  })
