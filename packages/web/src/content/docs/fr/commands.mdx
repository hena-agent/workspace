...
Créez `.hena-agent/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration Hena Agent ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre Hena Agent [config](/docs/config) :
...
- Par projet : `.hena-agent/commands/`
...
hena-agent comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
