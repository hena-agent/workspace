---
title: Formatery
description: Hena Agent używa formaterów specyficznych dla języka.
---

Hena Agent automatycznie formatuje pliki po ich zapisaniu lub edycji przy użyciu formaterów specyficznych dla języka. Zapewnia to, że wygenerowany kod jest zgodny ze stylem kodu Twojego projektu.

---

## Wbudowane

Hena Agent zawiera kilka wbudowanych formaterów dla popularnych języków i frameworków. Poniższa tabela zawiera listę formaterów, obsługiwanych plików oraz wymagań konfiguracyjnych.
...
Jeśli więc Twój projekt zawiera `prettier` w `package.json`, Hena Agent automatycznie go użyje.

---

## Jak to działa

Kiedy Hena Agent zapisuje lub edytuje plik:
...
Możesz dostosować formatery za pomocą sekcji `formatter` w konfiguracji Hena Agent.
