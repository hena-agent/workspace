...
Existem dois tipos de agentes no hena-agent; agentes primários e subagentes.
...
hena-agent vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Hena Agent vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Hena Agent vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Hena Agent, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `hena-agent.json`:
...
- Global: `~/.config/hena-agent/agents/`
- Por projeto: `.hena-agent/agents/`

```markdown title="~/.config/hena-agent/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o hena-agent usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do hena-agent quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do hena-agent usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Hena Zen](/docs/zen), você usaria `hena-agent/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="hena-agent.json" {3-6,9-12}
{
  "$schema": "https://hena.dev/config.json",
  "tools": {
    "write": true,
...
```json title="hena-agent.json" {3-5,8-10}
{
  "$schema": "https://hena.dev/config.json",
  "permission": {
    "edit": "deny"
...
```json title="hena-agent.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `hena-agent models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/hena-agent/hena).
:::
...
```markdown title="~/.config/hena-agent/agents/docs-writer.md"
---
...
```markdown title="~/.config/hena-agent/agents/security-auditor.md"
---
