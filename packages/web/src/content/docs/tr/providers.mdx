---
title: Sağlayıcılar
description: hena-agent'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

hena-agent, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı hena-agent yapılandırmanızda yapılandırın.
...
`~/.local/share/hena-agent/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları hena-agent'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Hena Zen, hena-agent ekibi tarafından sağlanan modellerin bir listesidir.
hena-agent ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Hena Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, hena-agent'u seçin ve [hena.dev/auth](https://hena.dev/auth)'ye gidin.
...
hena-agent'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Hena Go

Hena Go, hena-agent ile iyi çalıştığı test edilmiş ve doğrulanmış, hena-agent ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Hena Go`'yu seçin ve [hena.dev/auth](https://hena.dev/zen) adresine gidin.
...
Amazon Bedrock'u hena-agent ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   hena-agent'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `hena-agent.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi hena-agent'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
hena-agent'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   hena-agent'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Hena Agent yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
hena-agent, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. hena-agent'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`hena-agent.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`hena-agent.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi hena-agent'la kullanmak için:
...
Google Vertex AI'yi hena-agent ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları hena-agent çalıştırılırken ayarlayın.
...
Helicone'un hena-agent aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her hena-agent görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her hena-agent görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
hena-agent'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla hena-agent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla hena-agent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini hena-agent için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opencode)'a bakın.
:::
...
Ollama Cloud'u hena-agent ile kullanmak için:
...
3. hena-agent'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: hena-agent'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Hena Zen, hena-agent ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Hena Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Hena Zen**'i arayın.

   ```txt
   /connect
   ```

3. hena-agent API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   hena-agent yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca hena-agent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Hena Agent ile kullanmak için:
...
Modelleri hena-agent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı hena-agent ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `hena-agent.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları hena-agent'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `hena-agent auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için hena-agent yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, hena-agent yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
