...
description: Cree herramientas que LLM pueda llamar en Hena Agent.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de hena-agent como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.hena-agent/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/hena-agent/tools/`.
...
```ts title=".hena-agent/tools/database.ts" {1}
...
```ts title=".hena-agent/tools/math.ts"
import { tool } from "@hena-agent/plugin"
...
```ts title=".hena-agent/tools/bash.ts"
import { tool } from "@hena-agent/plugin"
...
```ts title=".hena-agent/tools/project.ts" {8}
import { tool } from "@hena-agent/plugin"
...
```python title=".hena-agent/tools/add.py"
import sys
...
```ts title=".hena-agent/tools/python-add.ts" {10}
import { tool } from "@hena-agent/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".hena-agent/tools/add.py")
