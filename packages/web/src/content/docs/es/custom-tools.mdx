...
description: Cree herramientas que LLM pueda llamar en Hena.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de hena como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.hena/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/hena/tools/`.
...
```ts title=".hena/tools/database.ts" {1}
...
```ts title=".hena/tools/project.ts" {8}
import { tool } from "@hena/plugin"
...
```python title=".hena/tools/add.py"
import sys
...
```ts title=".hena/tools/python-add.ts" {10}
import { tool } from "@hena/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".hena/tools/add.py")
