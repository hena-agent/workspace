...
import { LLMEvent, ToolFailure } from "@hena-agent/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@hena-agent/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("hena-agent"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("hena-agent") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("hena-agent") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, hena-agent, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored Hena Agent auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("hena-agent") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("hena-agent"),
