...
This is the living design and status document for `@hena-agent/codemode` and its existing V2 Hena Agent adapter.
...
### V2 Hena Agent adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/hena-agent/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
