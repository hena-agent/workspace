---
title: Servidor
description: Interaja com o servidor hena-agent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `hena-agent serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente hena-agent pode usar.
...
```bash
hena-agent serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `hena-agent.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
hena-agent serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `HENA_AGENT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `hena-agent`, ou defina `HENA_AGENT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `hena-agent serve` quanto ao `hena-agent web`.

```bash
HENA_AGENT_SERVER_PASSWORD=your-password hena-agent serve
```
...
Quando você executa `hena-agent`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor hena-agent para interagir com o hena-agent programaticamente.
:::

Essa arquitetura permite que o hena-agent suporte múltiplos clientes e permite que você interaja com o hena-agent programaticamente.

Você pode executar `hena-agent serve` para iniciar um servidor autônomo. Se você tiver o TUI do hena-agent em execução, `hena-agent serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do hena-agent [IDE](/docs/ide).
...
O servidor hena-agent expõe as seguintes APIs.
