...
import type { LLMClientShape } from "@hena/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("hena"))
    return { type: "unsupported", reason: "provider is not Hena, OpenAI, or Anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @hena/llm: native-request lowers session data
