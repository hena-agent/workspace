...
Dzięki hena możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie hena używa naszego własnego motywu `hena`.
...
hena ma kilka wbudowanych motywów.
...
- Chcą, aby Hena pasował do wyglądu ich terminala
...
hena obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/hena/themes/*.json` lub `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.hena/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.hena/themes/*.json`
