...
U Hena mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Hena koristi nasu `hena` temu.
...
Hena dolazi sa vise ugradenih tema.
...
- Zele da Hena odgovara izgledu njihovog terminala
...
Hena podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/hena/themes/*.json` ili `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.hena/themes/*.json`
4. **Trenutni radni direktorij** - `./.hena/themes/*.json`
