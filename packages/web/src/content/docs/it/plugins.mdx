---
title: Plugin
description: Scrivi plugin per estendere Hena.
---

I plugin ti permettono di estendere Hena agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Hena.
...
- `.hena/plugins/` - plugin a livello progetto
- `~/.config/hena/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/hena/node_modules/`.
...
1. Config globale (`~/.config/hena/hena.json`)
2. Config di progetto (`hena.json`)
3. Directory plugin globale (`~/.config/hena/plugins/`)
4. Directory plugin di progetto (`.hena/plugins/`)
...
Hena esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di hena per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere Hena.
...
:::note
Se usi l'app desktop di Hena, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a hena di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a hena:
...
L'helper `tool` crea uno strumento personalizzato che hena può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in hena insieme agli strumenti integrati.
