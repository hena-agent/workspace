...
The Hena Agent server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@hena-agent/llm` tool loop after replacing its
...
- plugins get global "henaAgent" instance like in that post i showed
- henaAgent instance has stuff like `henaAgent.session.prompt()` or
  `henaAgent.tool.register({...})`
...
- decide whether to preserve deprecated `@hena-agent/llm` orchestration exports
