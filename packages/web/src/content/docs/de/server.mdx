---
title: Server
description: Nutze den hena-agent-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `hena-agent serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein hena-agent-Client nutzen kann.
...
Setze `HENA_AGENT_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `hena-agent`, kann aber mit `HENA_AGENT_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `hena-agent serve` und `hena-agent web`.
...
Wenn du `hena-agent` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den hena-agent-Server, um hena-agent programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `hena-agent serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `hena-agent serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Hena Agent-[IDE](/docs/ide)-Plugins.
...
Der hena-agent-Server stellt folgende APIs bereit.
