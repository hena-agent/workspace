...
Go radi kao bilo koji drugi provajder u Hena-u. Pretplatite se na Hena Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Hena.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Hena-om.
:::
...
Hena Go radi kao bilo koji drugi provajder u Hena-u.
...
[Model id](/docs/config/#models) u vašoj Hena konfiguraciji
koristi format `hena-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`hena-go/kimi-k3` u svojoj konfiguraciji.
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Hena koristite i bilo kojeg drugog provajdera.
