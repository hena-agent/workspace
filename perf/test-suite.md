...
Speed up the `packages/hena` test suite without reducing coverage or hiding failures.

## Benchmark Command

Run from `packages/hena`:
...
`packages/hena/test/**`, test fixtures, package test scripts, and implementation setup paths only when a benchmarked bottleneck points there.
...
| `config.test` waits after dependencies even though `.gitignore` is written synchronously                  | Removed obsolete 1000ms sleep from writable `HENA_CONFIG_DIR` test                         | 10.270s   | 9.433s  | keep     | Median from 5 targeted runs because one run was noisy; simpler test and no fixed sleep.                                          |
...
| HTTP provider tests generate local plugins without dependency-ready fixture state                         | Marked generated `.hena` plugin fixtures dependency-ready                                  | 7.905s    | 2.980s  | keep     | Median from 3 targeted runs; avoids unrelated plugin dependency setup in route tests.                                            |
...
| `.hena` agent and command file-loading cases can use Effect-aware instance fixtures                   | Migrated singular/plural agent and command markdown fixture cases to `it.instance`             | 7.21s     | 1.87s   | keep     | Parent baseline was noisy (7.42, 7.21, 2.83); after runs were stable at 1.87, 1.98, 1.83. Keep as cleanup with no broad claim.   |
...
| Git fixture setup is dominated by process startup                                                         | Replaced four `git config` calls and fsmonitor teardown with one config-file write              | 370.127s  | 351.220s | keep     | Full-suite single runs before/after the optimization batch; each of 431 git fixtures now starts two git processes instead of seven. |
| CLI help subcommand snapshots duplicate top-level command coverage                                       | Removed 14 subcommand snapshots while retaining root and all 20 top-level command snapshots     | 35 boots  | 21 boots | keep     | Cuts full CLI subprocess boots by 40% while preserving a snapshot for every documented top-level command.                         |
| Serial test files can be split across isolated CI runners                                                 | Split Hena into two Linux shards and three Windows shards                                       | 351.220s  | 216.550s | keep     | Heavier Linux shard result; both two-way shards passed independently. This reduces wall time, not total runner work.               |
| Bun test workers can parallelize the Hena suite on one runner                                             | Tried `bun test --parallel=2`                                                                   | 344.770s  | failed   | discard  | Exceeded 600s with resource-contention timeouts and a plugin fixture race. Keep each shard serial.                                  |
| CLI subprocess cases can remain concurrent inside CI shards                                               | Removed the CI serialization guard from `cliIt.concurrent`                                     | 278.870s  | failed   | discard  | The unknown-model regression case exhausted its 15s process timeout under shard load. Keep CLI subprocess tests serial in CI.       |
| HttpApi modes can execute concurrently                                                                   | Tried same-runner parallelism and separate CI matrix jobs                                       | ~160s     | failed   | discard  | Same-runner auth blocked and isolated effect jobs hung beyond 20m. Keep the modes sequential in one dedicated job.                  |
...
| `bun run bench:test` | 370.127s | Baseline before the CI optimization batch; machine was concurrently running other worktrees.                                                    |
| `bun run bench:test` | 351.220s | Final passing serial suite after fixture and test-selection optimizations; use shard wall time as the primary CI metric.                        |
...
| Config content env cases can use Effect-aware instance fixtures        | Migrated two `HENA_CONFIG_CONTENT` token substitution cases to `it.instance`         |  1.95s |  2.06s | discard  | Passing but not neutral-or-better in focused reruns; keep existing explicit env cleanup.      |
