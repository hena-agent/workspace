---
title: Server
description: Interact with hena-agent server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `hena-agent serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an hena-agent client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `hena-agent.local` |
...
Set `HENA_AGENT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `hena-agent`, or set `HENA_AGENT_SERVER_USERNAME` to override it. This applies to both `hena-agent serve` and `hena-agent web`.
...
When you run `hena-agent` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the hena-agent server to interact with hena-agent programmatically.
:::

This architecture lets hena-agent support multiple clients and allows you to interact with hena-agent programmatically.

You can run `hena-agent serve` to start a standalone server. If you have the
hena-agent TUI running, `hena-agent serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Hena Agent [IDE](/docs/ide) plugins.
...
The hena-agent server exposes the following APIs.
