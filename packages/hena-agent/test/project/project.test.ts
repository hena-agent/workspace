...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const henaAgentFile = path.join(tmp, ".git", "hena-agent")
      expect(yield* Effect.promise(() => Bun.file(henaAgentFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "hena-agent")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "hena-agent")
      const wrongCache = path.join(parentDir, ".git", "hena-agent")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "hena-agent")
      const cacheB = path.join(bareB, "hena-agent")
      const wrongCache = path.join(parentDir, ".git", "hena-agent")
