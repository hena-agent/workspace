---
title: Plugins
description: Escreva seus próprios plugins para estender o Hena Agent.
---

Plugins permitem que você estenda o hena-agent conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do Hena Agent.
...
- `.hena-agent/plugins/` - Plugins em nível de projeto
- `~/.config/hena-agent/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/hena-agent/node_modules/`.
...
1. Configuração global (`~/.config/hena-agent/hena.json`)
2. Configuração do projeto (`hena-agent.json`)
3. Diretório de plugins global (`~/.config/hena-agent/plugins/`)
4. Diretório de plugins do projeto (`.hena-agent/plugins/`)
...
O hena-agent executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do hena-agent para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o Hena Agent.
...
:::note
Se você estiver usando o aplicativo desktop hena-agent, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o hena-agent de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao hena-agent:
...
O helper `tool` cria uma ferramenta personalizada que o hena-agent pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o hena-agent junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://hena.dev/docs/sdk) para detalhes.
