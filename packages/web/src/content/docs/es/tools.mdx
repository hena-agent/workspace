...
Las herramientas permiten que LLM realice acciones en su código base. Hena viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Hena.
...
:::note
Esta herramienta solo está disponible cuando `HENA_EXPERIMENTAL_LSP_TOOL=true` (o `HENA_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Hena o cuando la variable de entorno `HENA_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Hena:

```bash
HENA_ENABLE_EXA=1 hena
```
