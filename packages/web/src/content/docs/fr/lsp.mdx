---
title: Serveurs LSP
description: Hena Agent s'intègre à vos serveurs LSP.
---

Hena Agent peut s'intégrer aux serveurs Language Server Protocol (LSP) pour utiliser les diagnostics comme retour pour l'agent.

---

## Serveurs intégrés

Hena Agent est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `HENA_AGENT_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque LSP est activé et qu'hena-agent ouvre un fichier, il :
...
Vous pouvez activer et personnaliser les serveurs LSP via la section `lsp` de votre configuration Hena Agent.
...
```json title="hena-agent.json" {5-7}
{
  "$schema": "https://hena.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="hena-agent.json" {5-9}
{
  "$schema": "https://hena.ai/config.json",
  "lsp": {
    "typescript": {
