...
**Command review path.** `pr-review.yml` selects OpenCode's built-in `review` command and passes the pull request number. The action disables project config so a pull request cannot load a plugin while CI credentials are available, denies edits, and permits only `gh pr view` and `gh pr diff` shell commands so the built-in template can gather pull request context. It executes through the documented `opencode run --command` interface, extracts the final Markdown response from the JSON event stream, revalidates the current base and head, and submits the response as a pull request review. `edited` events rerun reviews after retargeting. Prompt-based callers continue through `opencode github run`.
