...
> current private `@hena-agent/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@hena-agent/ai`
...
- Secondary audience: Hena Agent and other durable agent runtimes
...
import { LLM } from "@hena-agent/ai"
import { OpenAI } from "@hena-agent/ai/providers/openai"
...
```ts
import { Message } from "@hena-agent/ai"
...
import { LLM, Tool } from "@hena-agent/ai"
...
Hena Agent and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@hena-agent/ai/promise"
import { OpenAI } from "@hena-agent/ai/providers/openai"
...
```ts
import { LLMSchema } from "@hena-agent/ai/schema"
...
```ts
import { Provider, Protocol } from "@hena-agent/ai/provider"
...
@hena-agent/ai/promise
  Promise/AsyncIterable LLM facade

@hena-agent/ai/schema
  serializable domain schemas

@hena-agent/ai/provider
  experimental Provider and Protocol authoring APIs

@hena-agent/ai/providers/openai
@hena-agent/ai/providers/anthropic
@hena-agent/ai/providers/google
...
```
...
Hena Agent should migrate to `generateTurn` / `streamTurn`, preserving its durable
