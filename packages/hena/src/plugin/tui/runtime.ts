...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@hena/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@hena/core/effect/app-node-builder"
import { errorData, errorMessage } from "@hena/tui/util/error"
...
import { hasTheme, upsertTheme } from "@hena/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@hena/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@hena/core/v1/config/plugin"
import { createCommandShim } from "@hena/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@hena/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".hena"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".hena", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".hena", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.HENA_PURE ? [] : pluginOrigins
    if (Flag.HENA_PURE && pluginOrigins.length) {
    }
