...
Sie können externe Tools zu Hena hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Hena unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Hena Config](https://hena.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.

```jsonc title="hena.jsonc" {6}
{
...
Organisationen können über ihren `.well-known/hena`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Hena übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Hena Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Hena beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `hena mcp auth <server-name>` verwenden.
...
```bash
hena mcp logout my-oauth-server
```

Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Hena die Token sicher in `~/.local/share/hena/mcp-auth.json`.
...
hena mcp debug my-oauth-server
```
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Hena verfügbar. Sie können sie also wie jedes andere Tool über die Hena-Konfiguration verwalten.
...
```bash
hena mcp auth sentry
```

Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Hena mit Ihrem Sentry-Konto zu verbinden.
