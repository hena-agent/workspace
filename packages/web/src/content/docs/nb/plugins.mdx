...
description: Skriv dine egne programtillegg for å utvide Hena Agent.
---

Plugins lar deg utvide Hena Agent ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Hena Agent.
...
- `.hena-agent/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/hena-agent/node_modules/`.
...
1. Global konfigurasjon (`~/.config/hena-agent/hena.json`)
2. Prosjektkonfigurasjon (`hena-agent.json`)
3. Global plugin-katalog (`~/.config/hena-agent/plugins/`)
4. Prosjektpluginkatalog (`.hena-agent/plugins/`)
...
Hena Agent kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Hena Agent SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Hena Agent.
...
:::note
Hvis du bruker Hena Agent-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Hena Agent fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Hena Agent:
...
`tool`-hjelperen lager et tilpasset verktøy som Hena Agent kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Hena Agent sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://hena.dev/docs/sdk) for detaljer.
