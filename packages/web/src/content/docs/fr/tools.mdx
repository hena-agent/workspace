...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. Hena Agent est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans Hena Agent.
...
:::note
Cet outil n'est disponible que lorsque `HENA_AGENT_EXPERIMENTAL_LSP_TOOL=true` (ou `HENA_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur Hena Agent ou lorsque la variable d'environnement `HENA_AGENT_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de Hena Agent :
