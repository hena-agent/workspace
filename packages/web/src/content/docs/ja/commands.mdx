...
`.hena-agent/commands/test.md` を作成します。
...
カスタムコマンドは、Hena Agent 設定を通じて、または `commands/` ディレクトリに Markdown ファイルを作成することによって追加できます。

---

### JSON

Hena Agent [設定](/docs/config)で `command` オプションを使用します:
...
- グローバル: `~/.config/hena-agent/commands/`
- プロジェクトごと: `.hena-agent/commands/`
...
Hena Agent には、`/init`、`/undo`、`/redo`、`/share`、`/help` などのいくつかの組み込みコマンドが含まれています。 [詳細はこちら](/docs/tui#commands)。
