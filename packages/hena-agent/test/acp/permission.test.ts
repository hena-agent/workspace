...
import type { Event, HenaAgentClient } from "@hena-agent/sdk/v2"
...
type PermissionReplyParams = Parameters<HenaAgentClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "hena-agent-acp-permission-"))
