...
Kreiraj `.hena-agent/commands/test.md`:
...
Možete dodati prilagođene komande kroz Hena Agent konfiguraciju ili kreiranjem markdown datoteka u direktoriju `commands/`.

---

### JSON

Koristite opciju `command` u svom Hena Agent [config](/docs/config):
...
- Globalno: `~/.config/hena-agent/commands/`
- Po projektu: `.hena-agent/commands/`
...
Hena Agent uključuje nekoliko ugrađenih naredbi kao što su `/init`, `/undo`, `/redo`, `/share`, `/help`; [saznaj više](/docs/tui#commands).
:::note
