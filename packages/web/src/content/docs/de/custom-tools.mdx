...
description: Erstellen Sie Tools, die der LLM in Hena Agent aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Hena Agent wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.hena-agent/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/hena-agent/tools/` platzieren.
