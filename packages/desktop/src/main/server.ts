...
const SIDECAR_SERVICE_NAME = "hena-agent server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...
    HENA_AGENT_EXPERIMENTAL_ICON_DISCOVERY: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`hena-agent:${password}`).toString("base64")
...
function createSidecarEnv(): Record<string, string> {
  ...
  if (!app.isPackaged) env.HENA_AGENT_DISABLE_CHANNEL_DB = "1"
  return env
}
