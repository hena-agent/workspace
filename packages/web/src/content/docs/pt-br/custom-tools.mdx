...
description: Crie ferramentas que o LLM pode chamar no Hena.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do hena, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.hena/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/hena/tools/`.
