---
title: LSPサーバー
description: Hena Agent は LSP サーバーと統合します。
---

Hena Agent は Language Server Protocol (LSP) サーバーと統合し、診断を agent へのフィードバックとして使用できます。

---

## 組み込み

Hena Agent には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
:::note
`HENA_AGENT_DISABLE_LSP_DOWNLOAD` 環境変数を `true` に設定すると、LSP サーバーの自動ダウンロードを無効にできます。
:::
...
LSP が有効で Hena Agent がファイルを開くと、次のようになります。
...
Hena Agent 設定の `lsp` セクションを通じて LSP サーバーを有効化し、カスタマイズできます。
