---
title: Complementos
description: Escriba sus propios complementos para extender Hena.
---

Los complementos le permiten extender Hena al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Hena.
...
- `.hena/plugins/` - Complementos a nivel de proyecto
- `~/.config/hena/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/hena/node_modules/`.
...
1. Configuración global (`~/.config/hena/hena.json`)
2. Configuración del proyecto (`hena.json`)
3. Directorio global de complementos (`~/.config/hena/plugins/`)
4. Directorio de complementos del proyecto (`.hena/plugins/`)
...
Hena ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".hena/plugins/my-plugin.ts"
...
```js title=".hena/plugins/example.js"
...
- `client`: Un cliente SDK hena para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar Hena.
...
```js title=".hena/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "hena"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Hena, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que hena lea archivos `.env`:

```javascript title=".hena/plugins/env-protection.js"
...
```javascript title=".hena/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a hena:

```ts title=".hena/plugins/custom-tools.ts"
import { type Plugin, tool } from "@hena/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que hena puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para hena junto con las herramientas integradas.
...
```ts title=".hena/plugins/compaction.ts"
import type { Plugin } from "@hena/plugin"
...
```ts title=".hena/plugins/custom-compaction.ts"
import type { Plugin } from "@hena/plugin"
