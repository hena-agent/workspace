---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Hena Agent.
---

Dodaci vam omogućavaju da proširite Hena Agent spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Hena Agent.
...
- `.hena-agent/plugins/` - Dodaci na nivou projekta
- `~/.config/hena-agent/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/hena-agent/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/hena-agent/hena.json`)
2. Konfiguracija projekta (`hena-agent.json`)
3. Globalni direktorij dodataka (`~/.config/hena-agent/plugins/`)
4. Direktorij dodataka projekta (`.hena-agent/plugins/`)
...
Hena Agent pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Hena Agent SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Hena Agent.
...
Spriječite hena-agent da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u hena-agent:
...
Pomoćnik `tool` kreira prilagođeni alat koji hena-agent može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za hena-agent zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://hena.dev/docs/sdk) za detalje.
