...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Hena Agent dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Hena Agent.
...
:::note
Ovaj alat je dostupan samo kada je `HENA_AGENT_EXPERIMENTAL_LSP_TOOL=true` (ili `HENA_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Hena Agent provajdera ili kada je varijabla `HENA_AGENT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Hena Agent:
