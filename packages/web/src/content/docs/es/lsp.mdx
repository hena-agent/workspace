...
description: Hena Agent se integra con sus servidores LSP.
---

Hena Agent puede integrarse con servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para el agente.

---

## Incorporados

Hena Agent viene con varios servidores LSP integrados para idiomas populares:
...
:::note
Puede deshabilitar las descargas automáticas del servidor LSP configurando la variable de entorno `HENA_AGENT_DISABLE_LSP_DOWNLOAD` en `true`.
:::
...
Cuando LSP está habilitado y hena-agent abre un archivo:
...
Puede habilitar y personalizar los servidores LSP a través de la sección `lsp` en su configuración Hena Agent.
