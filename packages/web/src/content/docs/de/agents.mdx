...
Es gibt zwei Arten von Agenten in Hena Agent; Primäragenten und Subagenten.
...
Hena Agent verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `hena-agent.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Hena Agent modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Hena Agent-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Hena Agent-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Hena Zen](/docs/zen) verwenden, würden Sie `hena-agent/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `hena-agent models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/hena-agent/hena).
:::
