...
import { LLMEvent, ToolFailure } from "@hena/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@hena/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("hena"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("hena") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("hena") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not Hena, OpenAI, or Anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored Hena auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("hena") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("hena"),
