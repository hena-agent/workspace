...
import type { ConsoleState } from "@hena/core/v1/config/console-state"
...
import { RemoteAuthError } from "@hena/core/v1/config/error"
import { ConfigPermissionV1 } from "@hena/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@hena/Config") {}
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "HENA_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/hena`
        ...
        if (Flag.HENA_CONFIG) {
          yield* merge(Flag.HENA_CONFIG, yield* loadFile(Flag.HENA_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.HENA_CONFIG })
        }

        if (!Flag.HENA_DISABLE_PROJECT_CONFIG) {
          const files = yield* ConfigPaths.files("hena", ctx.directory, ctx.worktree).pipe(Effect.orDie)
        ...
        if (Flag.HENA_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from HENA_CONFIG_DIR", { path: Flag.HENA_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".hena") || dir === Flag.HENA_CONFIG_DIR) {
            for (const file of ["hena.json", "hena.jsonc"]) {
          ...
          // Auto-discovered plugins under `.hena/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.HENA_CONFIG_CONTENT) {
          const source = "HENA_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.HENA_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from HENA_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["HENA_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("HENA_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.HENA_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.HENA_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("HENA_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.HENA_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.HENA_DISABLE_PRUNE) {
