...
This is the living design and status document for `@hena/codemode` and its existing V2 Hena adapter.
...
This section describes the `v2` branch integration. On `develop`, CodeMode is integrated through
`packages/hena/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
