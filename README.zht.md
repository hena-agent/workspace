...
brew install hena-agent/tap/hena-agent # macOS 與 Linux（推薦，始終保持最新）
brew install hena-agent              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S hena-agent            # Arch Linux (Stable)
paru -S hena-agent-bin               # Arch Linux (Latest from AUR)
mise use -g hena-agent               # 任何作業系統
...
1. `$HENA_AGENT_INSTALL_DIR` - 自定義安裝目錄
2. `$XDG_BIN_DIR` - 符合 XDG 基礎目錄規範的路徑
3. `$HOME/bin` - 標準使用者執行檔目錄 (若存在或可建立)
4. `$HOME/.hena-agent/bin` - 預設備用路徑
...
Hena Agent 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
如果您有興趣參與 Hena Agent 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 Hena Agent 進行開發

如果您正在開發與 Hena Agent 相關的專案，並在名稱中使用了 "hena-agent"（例如 "hena-agent-dashboard" 或 "hena-agent-mobile"），請在您的 README 中加入聲明，說明該專案並非由 Hena Agent 團隊開發，且與我們沒有任何隸屬關係。
