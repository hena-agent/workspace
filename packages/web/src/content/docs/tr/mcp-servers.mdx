...
_Model Bağlam Protokolü_ veya MCP kullanarak hena'a harici araçlar ekleyebilirsiniz. hena hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [hena Config](https://hena.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/hena` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
hena, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde hena şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa hena, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `hena mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından hena, belirteçleri `~/.local/share/hena/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra hena'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi hena yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve hena'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
