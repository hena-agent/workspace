...
Da biste otklonili probleme s Hena Agent, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\hena-agent\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `hena-agent --log-level DEBUG`.

---

## Pohrana

Hena Agent pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/hena-agent/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\hena-agent`
...
Hena Agent Desktop pokreće lokalni Hena Agent server (`hena-agent-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Hena Agent` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/hena-agent/hena.jsonc` (ili `~/.config/hena-agent/hena.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/hena-agent/hena.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\hena-agent\hena-agent.jsonc`
...
Hena Agent također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/hena-agent/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\hena-agent\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Hena Agent mogao ponovo izgraditi.

1. Potpuno zatvorite Hena Agent Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/hena-agent`
- **Linux**: obrišite `~/.cache/hena-agent` (ili pokrenite `rm -rf ~/.cache/hena-agent`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\hena-agent`

3. Ponovo pokrenite Hena Agent Desktop.

---

### Rješavanje problema sa vezom na serveru

Hena Agent Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `hena-agent.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `HENA_AGENT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `HENA_AGENT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Hena Agent Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Hena Agent karakteristikama.

---

### Obavještenja se ne prikazuju

Hena Agent Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Hena Agent u postavkama vašeg OS-a, i
...
1. Zatvorite Hena Agent Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Hena Agent Desktop aplikacije):

- `hena-agent.settings.dat` (URL zadanog servera za desktop)
- `hena-agent.global.dat` i `hena-agent.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Hena Agent:
...
Da saznate kojim modelima imate pristup, pokrenite `hena-agent models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\hena-agent`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Hena Agent dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\hena-agent`

2. Ponovo pokrenite Hena Agent da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Hena Agent da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Hena Agent će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
