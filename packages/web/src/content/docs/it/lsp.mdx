---
title: Server LSP
description: Hena si integra con i tuoi server LSP.
---

Hena può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Hena include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `HENA_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e hena apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di Hena.
