import type { PermissionV1 } from "@hena-agent/core/v1/permission"
import { FSUtil } from "@hena-agent/core/fs-util"
// CLI entry point for `hena-agent run` and `hena-agent --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`hena-agent --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`hena-agent --mini --attach`): connects to a running
//      Hena Agent server and runs interactive mode against it.
//
...
import { createHenaAgentClient, type HenaAgentClient, type ToolPart } from "@hena-agent/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<HenaAgentClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run Hena Agent with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running Hena Agent server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to HENA_AGENT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to HENA_AGENT_SERVER_USERNAME or 'hena-agent')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: HenaAgentClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: HenaAgentClient, sessionID: string) {
      ...
      async function current(sdk: HenaAgentClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: HenaAgentClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: HenaAgentClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: HenaAgentClient) {
        ...
        async function loop(client: HenaAgentClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createHenaAgentClient({
        baseUrl: "http://hena-agent.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
