...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ hena }) =>
    Effect.gen(function* () {
      const result = yield* hena.spawn(["--replay-limit", "10"])

      hena.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ hena }) =>
    Effect.gen(function* () {
      const result = yield* hena.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ hena }) =>
    Effect.gen(function* () {
      const result = yield* hena.spawn(["--mini", "--port", "4096"])
