---
title: Plugins
description: Écrivez vos propres plugins pour étendre Hena Agent.
---

Les plugins vous permettent d'étendre Hena Agent en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Hena Agent.
...
- `.hena-agent/plugins/` - Plugins au niveau du projet
- `~/.config/hena-agent/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/hena-agent/node_modules/`.
...
1. Configuration globale (`~/.config/hena-agent/hena.json`)
2. Configuration du projet (`hena-agent.json`)
3. Répertoire global des plugins (`~/.config/hena-agent/plugins/`)
4. Répertoire des plugins du projet (`.hena-agent/plugins/`)
...
Hena Agent exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK hena-agent pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre Hena Agent.
...
:::note
Si vous utilisez l'application de bureau Hena Agent, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez hena-agent de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à hena-agent :
...
L'assistant `tool` crée un outil personnalisé que hena-agent peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour hena-agent aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://hena.dev/docs/sdk) pour plus de détails.
