---
title: Tjener
description: Samhandle med Hena-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `hena serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Hena-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `hena.local` |
...
Sett `HENA_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `hena` som standard, eller sett `HENA_SERVER_USERNAME` for å overstyre det. Dette gjelder både `hena serve` og `hena web`.
...
Når du kjører `hena` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Hena-serveren til å samhandle med Hena programmatisk.
:::

Denne arkitekturen lar Hena støtte flere klienter og lar deg samhandle med Hena programmatisk.

Du kan kjøre `hena serve` for å starte en frittstående server. Hvis du har
Hena TUI kjører, vil `hena serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Hena [IDE](/docs/ide) plugins.
...
hena-serveren viser følgende APIer.
