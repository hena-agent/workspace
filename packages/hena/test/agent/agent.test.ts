...
it.instance("plan agent denies edits except .hena/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".hena/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".hena", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.HENA_TEST_HOME
      process.env.HENA_TEST_HOME = test.directory
