...
Con Hena puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Hena usa nuestro propio tema `hena`.
...
Hena viene con varios temas integrados.
...
- Quiere que Hena coincida con la apariencia de su terminal
...
Hena admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/hena/themes/*.json` o `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.hena/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.hena/themes/*.json`
