...
أنشئ الملف `.hena-agent/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Hena Agent أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Hena Agent:
...
- على مستوى النظام: `~/.config/hena-agent/commands/`
- لكل مشروع: `.hena-agent/commands/`
...
يتضمن hena-agent عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
