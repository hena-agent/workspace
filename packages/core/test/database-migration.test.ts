...
import sessionUsageMigration from "@hena/core/database/migration/20260510033149_session_usage"
import normalizeStoragePathsMigration from "@hena/core/database/migration/20260601010001_normalize_storage_paths"
import sessionMessageProjectionOrderMigration from "@hena/core/database/migration/20260603040000_session_message_projection_order"
import eventSourcedSessionInputMigration from "@hena/core/database/migration/20260604172448_event_sourced_session_input"
  ...
  test("restores foreign keys and leaves the journal incomplete when a table rebuild fails", async () => {
    await run(
      Effect.gen(function* () {
        ...
        yield* db.run(sql`CREATE TABLE __new_project (id text PRIMARY KEY)`)

        const exit = yield* Effect.exit(DatabaseMigration.applyOnly(db, [folderlessProjectMigration]))
