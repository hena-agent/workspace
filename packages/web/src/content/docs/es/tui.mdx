---
title: TUI
description: Usando la interfaz de usuario del terminal Hena.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hena proporciona una interfaz de terminal interactiva o TUI para trabajar en sus proyectos con un LLM.

Al ejecutar Hena se inicia TUI para el directorio actual.
...
Cuando utilice Hena TUI, puede escribir `/` seguido de un nombre de comando para ejecutar acciones rápidamente. Por ejemplo:
...
Agregue un proveedor a Hena. Le permite seleccionar entre proveedores disponibles y agregar sus claves API.
...
Esto es independiente de `hena.json`, que configura el comportamiento del servidor/tiempo de ejecución.
...
- `leader_timeout`: controla cuánto tiempo espera Hena después de la leader key. El valor predeterminado es `2000`.
...
- `sound_pack`: ID del sound pack que se usará. El valor predeterminado es `hena.default`.
