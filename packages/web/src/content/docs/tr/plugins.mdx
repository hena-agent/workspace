---
title: Eklentiler
description: Hena Agent'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Hena Agent'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Hena Agent'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.hena-agent/plugins/` - Proje düzeyinde eklentiler
- `~/.config/hena-agent/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/hena-agent/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/hena-agent/hena.json`)
2. Project config (`hena-agent.json`)
3. Global eklenti dizini (`~/.config/hena-agent/plugins/`)
4. Proje eklenti dizini (`.hena-agent/plugins/`)
...
Hena Agent bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Hena Agent SDK istemcisi.
...
Hena Agent'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Hena Agent masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Hena Agent'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Hena Agent'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Hena Agent'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Hena Agent'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://hena.dev/docs/sdk) bakın.
