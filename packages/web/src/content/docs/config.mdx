---
title: Config
description: Using the Hena Agent JSON config.
---

You can configure Hena Agent using a JSON config file.

---

## Format

Hena Agent supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="hena-agent.jsonc"
{
...
1. **Remote config** (from `.well-known/hena-agent`) - organizational defaults
2. **Global config** (`~/.config/hena-agent/hena.json`) - user preferences
3. **Custom config** (`HENA_AGENT_CONFIG` env var) - custom overrides
4. **Project config** (`hena-agent.json` in project) - project-specific settings
5. **`.hena-agent` directories** - agents, commands, plugins
6. **Inline config** (`HENA_AGENT_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/hena-agent/` on macOS) - admin-controlled
...
:::note
The `.hena-agent` and `~/.config/hena-agent` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/hena-agent` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/hena-agent"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Hena Agent config in `~/.config/hena-agent/hena.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/hena-agent/tui.json`.
...
Add `hena-agent.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Hena Agent starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `HENA_AGENT_CONFIG` environment variable.

```bash
export HENA_AGENT_CONFIG=/path/to/my/custom-config.json
hena-agent run "Hello world"
```
...
Specify a custom config directory using the `HENA_AGENT_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.hena-agent` directory, and should
follow the same structure.

```bash
export HENA_AGENT_CONFIG_DIR=/path/to/my/config-directory
hena-agent run "Hello world"
```

The custom directory is loaded after the global config and `.hena-agent` directories, so it **can override** their settings.
...
Drop an `hena-agent.json` or `hena-agent.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/hena-agent/` |
| Linux    | `/etc/hena-agent/`                         |
| Windows  | `%ProgramData%\hena-agent`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Hena Agent reads managed preferences from the `ai.hena-agent.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Hena Agent checks these paths:

1. `/Library/Managed Preferences/<user>/ai.hena-agent.managed.plist`
2. `/Library/Managed Preferences/ai.hena-agent.managed.plist`

The plist keys map directly to `hena-agent.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.hena-agent.managed` PayloadType. The Hena Agent config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.hena-agent.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.hena-agent.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.hena-agent</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`hena.dev/config.json`**](https://hena.dev/config.json).

TUI config uses [**`hena.dev/tui.json`**](https://hena.dev/tui.json).
...
Use `HENA_AGENT_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `hena-agent.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `hena-agent serve` and `hena-agent web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Hena Agent server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `hena-agent.local`. Useful for running multiple instances on the same network.
...
If not specified, Hena Agent will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Hena Agent config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Hena Agent tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Hena Agent actions on configured resources. Currently, policies can control which providers Hena Agent may use.
...
Hena Agent normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Hena Agent omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/hena-agent/agents/` or `.hena-agent/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Hena Agent will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`hena-agent run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/hena-agent/commands/` or `.hena-agent/commands/`. [Learn more here](/docs/commands).
...
Hena Agent uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Hena Agent will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, hena-agent **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Hena Agent with custom tools, hooks, and integrations.

Place plugin files in `.hena-agent/plugins/` or `~/.config/hena-agent/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Hena Agent to only use specific providers rather than disabling them one by one.
