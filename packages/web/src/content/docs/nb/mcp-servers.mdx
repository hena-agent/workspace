...
Du kan legge til eksterne verktøy i Hena Agent ved å bruke _Model Context Protocol_ eller MCP. Hena Agent støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Hena Agent Config](https://hena.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="hena-agent.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/hena-agent`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="hena-agent.jsonc" {15}
{
  "$schema": "https://hena.dev/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Hena Agent håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Hena Agent:
...
Hvis serveren krever autentisering, vil Hena Agent be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `hena-agent mcp auth <server-name>`.
...
```json title="hena-agent.json" {7-11}
{
  "$schema": "https://hena.dev/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
hena-agent mcp auth my-oauth-server
```
...
```bash
hena-agent mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Hena Agent lagre tokens sikkert i `~/.local/share/hena-agent/mcp-auth.json`.
...
hena-agent mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Hena Agent, sammen med innebygde verktøy. Så du kan administrere dem gjennom Hena Agent-konfigurasjonen som alle andre verktøy.
...
```json title="hena-agent.json" {11, 14-18}
{
  "$schema": "https://hena.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="hena-agent.json" {4-8}
{
  "$schema": "https://hena.dev/config.json",
  "mcp": {
    "sentry": {
...
```bash
hena-agent mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Hena Agent til Sentry-kontoen din.
...
```json title="hena-agent.json" {4-7}
{
  "$schema": "https://hena.dev/config.json",
  "mcp": {
    "context7": {
...
```json title="hena-agent.json" {7-9}
{
  "$schema": "https://hena.dev/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Hena Agent-samtale oppført som en separat økt.
