---
title: Özel Araçlar
description: LLM'un hena-agent'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. hena-agent'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.hena-agent/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/hena-agent/tools/` içine yerleştirerek.
