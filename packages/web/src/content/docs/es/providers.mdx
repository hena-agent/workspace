---
title: Proveedores
description: Usando cualquier proveedor LLM en Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Hena.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Hena
configuración.
...
Hena Zen es una lista de modelos proporcionados por el equipo Hena que han sido
probado y verificado para funcionar bien con Hena. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Hena Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione hena y diríjase a [hena.dev/auth](https://hena.dev/auth).
...
Funciona como cualquier otro proveedor en Hena y su uso es completamente opcional.

---

## Hena Go

Hena Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Hena que han sido
probado y verificado para funcionar bien con Hena.
...
Para usar Amazon Bedrock con Hena:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta hena:
   ...
   Para una configuración persistente o específica del proyecto, utilice `hena.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Hena no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar hena para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que hena funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración Hena.
...
para almacenar el token en el almacenamiento de autenticación Hena.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Hena utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Hena
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `hena.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `hena.json`:
...
Para utilizar su suscripción GitHub Copilot con hena:
...
Para utilizar Google Vertex AI con Hena:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta Hena.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de hena, siempre podrá configurarlo usted mismo.
...
Puede configurar hena para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar hena para usar modelos locales a través de LM Studio.
...
Puedes configurar hena para usar modelos locales a través de Ollama.
...
Para usar Ollama Cloud con Hena:
...
3. Copie la clave API para usarla en Hena.
...
6. **Importante**: Antes de usar modelos en la nube en Hena, debe obtener la información del modelo localmente:
...
Hena Zen es una lista de modelos probados y verificados proporcionada por el equipo Hena. [Más información](/docs/zen).
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración Hena.
...
5. También puedes personalizarlos a través de tu configuración Hena. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Hena:
...
También puede personalizar modelos a través de su configuración Hena. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con Hena. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `hena.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Hena comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `hena auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración hena y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración Hena.
