...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from hena.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const hena = await henaFiles(input)
  for (const file of hena) {
...
async function henaFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "hena"),
    ...(await Filesystem.findUp(["hena.json", "hena.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "hena"))
  }
  if (Flag.HENA_CONFIG) files.push(Flag.HENA_CONFIG)
