...
Las habilidades del agente permiten a Hena descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Hena busca estas ubicaciones:

- Configuración del proyecto: `.hena/skills/<name>/SKILL.md`
- Configuración global: `~/.config/hena/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Hena sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.hena/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/hena/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.hena/skills/git-release/SKILL.md` así:
...
Hena enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `hena.json`:
...
**Para agentes integrados** (en `hena.json`):
