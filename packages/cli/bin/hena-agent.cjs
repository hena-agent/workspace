...
const envPath = process.env.HENA_AGENT_BIN_PATH
const scriptDir = path.dirname(fs.realpathSync(__filename))
const cached = path.join(scriptDir, ".hena-agent")
const platform = { darwin: "darwin", linux: "linux", win32: "windows" }[os.platform()] || os.platform()
const arch = { x64: "x64", arm64: "arm64", arm: "arm" }[os.arch()] || os.arch()
const base = "@hena-agent/cli-" + platform + "-" + arch
const binary = platform === "windows" ? "hena-agent.exe" : "hena-agent"
...
if (!resolved) {
  console.error(
    "It seems that your package manager failed to install the right Hena Agent CLI package. Try manually installing " +
