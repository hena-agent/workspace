...
import { ConfigProvider } from "@hena-agent/core/config/provider"
...
import { ConfigMigrateV1 } from "@hena-agent/core/v1/config/migrate"
...
import { Policy } from "@hena-agent/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads Hena Agent JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "hena-agent.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "hena-agent.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "hena-agent.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "hena-agent.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "hena-agent.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "hena-agent.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "hena-agent.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .hena-agent configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".hena-agent"), { recursive: true })
            await fs.mkdir(path.join(directory, ".hena-agent"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "hena-agent.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "hena-agent.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "hena-agent.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "hena-agent.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "hena-agent.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".hena-agent", "hena-agent.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".hena-agent", "hena-agent.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".hena-agent")),
              AbsolutePath.make(path.join(directory, ".hena-agent")),
            ])
