---
title: Konfiguracija
description: Korištenje Hena Agent JSON konfiguracije.
---

Možete konfigurirati Hena Agent koristeći JSON konfiguracijski fajl.

---

## Format

Hena Agent podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/hena-agent`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/hena-agent/hena.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`HENA_AGENT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`hena-agent.json` u projektu) - postavke specifične za projekat
5. **`.hena-agent` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`HENA_AGENT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.hena-agent` i `~/.config/hena-agent` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/hena-agent` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Hena Agent konfiguraciju u `~/.config/hena-agent/hena.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/hena-agent/tui.json`.
...
Dodajte `hena-agent.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Hena Agent pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `HENA_AGENT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `HENA_AGENT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.hena-agent` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.hena-agent`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`hena.dev/config.json`**](https://hena.dev/config.json).

TUI konfiguracija koristi [**`hena.dev/tui.json`**](https://hena.dev/tui.json).
...
Koristite `HENA_AGENT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `hena-agent.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `hena-agent serve` i `hena-agent web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Hena Agent server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `hena-agent.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Hena Agent konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Hena Agent pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/hena-agent/agents/` ili `.hena-agent/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Hena Agent će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`hena-agent run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/hena-agent/commands/` ili `.hena-agent/commands/`. [Saznajte više ovdje](/docs/commands).
...
Hena Agent će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Hena Agent **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Hena Agent sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.hena-agent/plugins/` ili `~/.config/hena-agent/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Hena Agent da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
