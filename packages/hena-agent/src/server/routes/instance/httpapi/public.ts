...
import { HenaAgentHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for hena-agent serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(HenaAgentHttpApi)
...
export const PublicApi = HenaAgentHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "Hena Agent",
    version: "1.0.0",
    description: "Hena Agent API",
