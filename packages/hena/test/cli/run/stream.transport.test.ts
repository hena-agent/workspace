...
import { HenaClient, type GlobalEvent } from "@hena/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<HenaClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<HenaClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<HenaClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<HenaClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<HenaClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://hena.test"),
...
function sdk(
  input: {
    ...
    subscribe?: HenaClient["event"]["subscribe"]
    globalEvent?: HenaClient["global"]["event"]
    promptAsync?: HenaClient["session"]["promptAsync"]
    status?: HenaClient["session"]["status"]
    messages?: HenaClient["session"]["messages"]
    children?: HenaClient["session"]["children"]
    permissions?: HenaClient["permission"]["list"]
    questions?: HenaClient["question"]["list"]
  } = {},
) {
  const client = new HenaClient()

  const subscribe: HenaClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: HenaClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: HenaClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: HenaClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: HenaClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: HenaClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: HenaClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: HenaClient["question"]["list"] = input.questions ?? (() => ok([]))
