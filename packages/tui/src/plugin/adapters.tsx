import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@hena/plugin/tui"
...
import type { useHenaKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useHenaKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getHenaModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getHenaModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
