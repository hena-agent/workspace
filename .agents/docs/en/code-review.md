...
**Command review path.** `pr-review.yml` selects OpenCode's built-in `review` command and passes the pull request number. The workflow gives OpenCode a separate read-only App token while retaining the write-scoped token for publication. The action disables project config so a pull request cannot load a plugin while CI credentials are available, denies edits and external-directory access, and permits only the exact `gh pr view <number>` and `gh pr diff <number>` shell commands required by the built-in template. It preflights both commands so unavailable pull request context fails the job before model execution. It then executes through the documented `opencode run --command` interface, extracts the final Markdown response from the JSON event stream, revalidates the current base and head, and submits the response as a pull request review. `edited` events rerun reviews after retargeting. Prompt-based callers continue through `opencode github run`.
