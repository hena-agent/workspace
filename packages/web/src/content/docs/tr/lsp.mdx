...
description: Hena Agent, LSP sunucularınızla bütünleşir.
---

Hena Agent, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Hena Agent, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`HENA_AGENT_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve hena-agent bir dosyayı açtığında:
...
LSP sunucularını, hena-agent yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
