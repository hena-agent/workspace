...
Her er alle de innebygde verktøyene som er tilgængelige i Hena Agent.
...
:::note
Dette verktøyet er bare tilgængelig når `HENA_AGENT_EXPERIMENTAL_LSP_TOOL=true` (eller `HENA_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Hena Agent-leverandøren eller når miljøvariabelen `HENA_AGENT_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
