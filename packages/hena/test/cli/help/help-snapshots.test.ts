...
// Top-level commands. Order matches what `hena --help` prints today;
...
// top-level help on `--help` and exits 1; not a real hena command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("hena CLI help-text snapshots", () => {
    ...
    ({ hena }) =>
      Effect.gen(function* () {
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* hena.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`hena ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`hena ${argv.join(" ")} --help`)
        }
