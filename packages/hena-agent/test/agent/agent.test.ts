...
it.instance("plan agent denies edits except .hena-agent/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".hena-agent/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".hena-agent", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.HENA_AGENT_TEST_HOME
      process.env.HENA_AGENT_TEST_HOME = test.directory
