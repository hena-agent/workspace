...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ henaAgent }) =>
    Effect.gen(function* () {
      const result = yield* henaAgent.spawn(["--replay-limit", "10"])

      henaAgent.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ henaAgent }) =>
    Effect.gen(function* () {
      const result = yield* henaAgent.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ henaAgent }) =>
    Effect.gen(function* () {
      const result = yield* henaAgent.spawn(["--mini", "--port", "4096"])
