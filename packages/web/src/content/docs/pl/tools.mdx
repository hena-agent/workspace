...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Hena Agent posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Hena Agent.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `HENA_AGENT_EXPERIMENTAL_LSP_TOOL=true` (lub `HENA_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Hena Agent lub gdy zmienna środowiskowa `HENA_AGENT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Hena Agent:
