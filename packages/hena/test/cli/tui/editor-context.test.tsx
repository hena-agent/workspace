...
import { EditorContextProvider, useEditorContext, type EditorIntegration } from "@hena/tui/context/editor"
import { tmpdir } from "../../fixture/fixture"
import { FakeWebSocket } from "../../lib/websocket"
import { TestTuiContexts } from "../../fixture/tui-environment"
import { discoverEditorConnection } from "@hena/tui/editor"

const originalClaudePort = process.env.CLAUDE_CODE_SSE_PORT
const originalHenaPort = process.env.HENA_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.HENA_EDITOR_SSE_PORT = originalHenaPort
})
...
function mountEditorContext(WebSocketImpl?: typeof WebSocket) {
  ...
  createRoot((nextDispose) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.HENA_EDITOR_SSE_PORT
    return (
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.HENA_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with HENA_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.HENA_EDITOR_SSE_PORT = "4020"
