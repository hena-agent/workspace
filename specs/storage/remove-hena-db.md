# Remove `packages/hena/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/hena/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the Hena legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/hena/src/storage/db.ts` are concentrated in 22 source files:

- `packages/hena/src/account/repo.ts`
- `packages/hena/src/cli/cmd/db.ts`
- `packages/hena/src/cli/cmd/import.ts`
- `packages/hena/src/cli/cmd/stats.ts`
- `packages/hena/src/control-plane/workspace.ts`
- `packages/hena/src/index.ts`
- `packages/hena/src/node.ts`
- `packages/hena/src/permission/index.ts`
- `packages/hena/src/project/project.ts`
- `packages/hena/src/server/projectors.ts`
- `packages/hena/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/hena/src/server/shared/fence.ts`
- `packages/hena/src/session/message-v2.ts`
- `packages/hena/src/session/projectors.ts`
- `packages/hena/src/session/prompt.ts`
- `packages/hena/src/session/session.ts`
- `packages/hena/src/session/todo.ts`
- `packages/hena/src/share/share-next.ts`
- `packages/hena/src/storage/db.ts`
- `packages/hena/src/sync/index.ts`
- `packages/hena/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy Hena database wrapper; `packages/hena/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the Hena wrapper.
...
Status: Completed. `SyncEvent` and the Hena projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy Hena database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy Hena database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy Hena database wrapper.
...
All migration groups are complete or superseded. `packages/hena/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No Hena data-migration group remains.

The previous Hena `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate Hena data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/hena`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/hena/src`
- `bun typecheck` from `packages/hena`
- Relevant package tests from `packages/hena`, not the repo root
