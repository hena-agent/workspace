---
title: Servidor
description: Interactuar con el servidor hena a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

El comando `hena serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente hena puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `hena.local` |
...
Configure `HENA_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `hena`, o configure `HENA_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `hena serve` como a `hena web`.
...
Cuando ejecuta `hena`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor hena para interactuar con hena mediante programación.
:::

Esta arquitectura permite que hena admita múltiples clientes y le permite interactuar con hena mediante programación.

Puede ejecutar `hena serve` para iniciar un servidor independiente. Si tienes el
hena TUI ejecutándose, `hena serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Hena [IDE](/docs/ide).
...
El servidor hena expone las siguientes API.
