...
- Author package entrypoint is `@hena-agent/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/hena-agent-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@hena-agent/plugin/tui`.
- `@hena-agent/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@hena-agent/plugin/tui"
...
- The value is a semver range checked against the running Hena Agent version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.hena-agent` is absent, no check is performed (backward compatible).
...
- `hena-agent plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `hena-agent plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.hena-agent` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.hena-agent`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.hena-agent`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.hena-agent` dir or `HENA_AGENT_CONFIG_DIR`, Hena Agent installs `@hena-agent/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@hena-agent/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Hena Agent's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Hena Agent registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Hena Agent pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@hena-agent/plugin/tui`.
...
- `message` is required; `title` defaults to `"Hena Agent"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `hena-agent.default`.
...
- Local plugins persist installed themes under the local `.hena-agent/themes` area near the plugin config source.
...
- `--pure` / `HENA_AGENT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.hena-agent/plugins/tui-smoke.tsx`
- Local vim plugin: `.hena-agent/plugins/tui-vim.tsx`
- Local smoke config: `.hena-agent/tui.json`
- Local smoke theme: `.hena-agent/plugins/smoke-theme.json`
