...
description: Usa la configurazione JSON di Hena Agent.
---

Puoi configurare Hena Agent usando un file di configurazione JSON.

---

## Formato

Hena Agent supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/hena-agent`) - default dell'organizzazione
2. **Config globale** (`~/.config/hena-agent/hena.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `HENA_AGENT_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`hena-agent.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.hena-agent`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `HENA_AGENT_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.hena-agent` e `~/.config/hena-agent` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/hena-agent`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Hena Agent in `~/.config/hena-agent/hena.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/hena-agent/tui.json`.
...
Aggiungi `hena-agent.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Hena Agent si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `HENA_AGENT_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `HENA_AGENT_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.hena-agent` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.hena-agent`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`hena.dev/config.json`**](https://hena.dev/config.json).

La configurazione TUI usa [**`hena.dev/tui.json`**](https://hena.dev/tui.json).
...
Usa `HENA_AGENT_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `hena-agent.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `hena-agent serve` e `hena-agent web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Hena Agent.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `hena-agent.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Hena Agent tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Hena Agent prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/hena-agent/agents/` o `.hena-agent/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Hena Agent fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`hena-agent run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/hena-agent/commands/` o `.hena-agent/commands/`. [Scopri di piu](/docs/commands).
...
Hena Agent scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, hena-agent **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Hena Agent con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.hena-agent/plugins/` o `~/.config/hena-agent/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Hena Agent a usare solo alcuni provider invece di disabilitarli uno per uno.
