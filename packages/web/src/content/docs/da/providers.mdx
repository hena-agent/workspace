---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Hena-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/hena/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Hena-konfigurationen.
...
Hena Zen er en liste over modeller leveret af Hena-teamet, som er blevet testet og verificeret til at fungere godt med Hena. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Hena Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg hena og gå til [hena.dev/auth](https://hena.dev/auth).
...
Det fungerer som alle andre udbydere i Hena og er helt valgfrit at bruge.

---

## Hena Go

Hena Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Hena-teamet, som er testet og verificeret til at fungere godt med Hena.
...
Sådan bruger du Amazon Bedrock med Hena:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører hena:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `hena.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Hena understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere hena til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at hena skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem hena-konfigurationen.
...
gemme tokenet i hena auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Hena bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Hena
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`hena.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Hena:
...
Sådan bruger du Google Vertex AI med Hena:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem hena, kan du altid konfigurere den selv.
...
Du kan konfigurere hena til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere hena til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere hena til at bruge lokale modeller gennem Ollama.
...
Sådan bruger du Ollama Cloud med Hena:
...
3. Kopier API-nøglen til brug i Hena.
...
6. **Vigtigt**: Før du bruger skymodeller i Hena, skal du hente modelinformationen lokalt:
...
Hena Zen er en liste over testede og verificerede modeller leveret af Hena-teamet. [Læs mere](/docs/zen).
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem hena-konfigurationen.
...
5. Du kan også tilpasse dem gennem hena-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Hena:
...
Du kan også tilpasse modeller gennem hena-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med Hena. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `hena.json`-filen i projektkataloget:
...
`limit`-felterne lader Hena forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `hena auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek hena-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i hena-konfigurationen.
