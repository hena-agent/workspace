...
Możesz dodać dodatkowe informacje do hena za pomocą _Model Context Protocol_, w skrócie MCP. hena obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [hena Config](https://hena.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/hena`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
hena automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, hena:
...
Jeśli serwer wymaga uwierzytelnienia, hena poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `hena mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji hena bezpieczne przechowa tokeny w `~/.local/share/hena/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w hena, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie hena, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć hena z kontem Sentry.
