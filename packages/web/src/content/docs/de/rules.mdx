---
title: Regeln
description: Lege eigene Anweisungen fuer hena-agent fest.
---

Du kannst hena-agent ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in hena-agent den Befehl `/init` aus.
...
So kann hena-agent sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht hena-agent sie zu erweitern.
...
hena-agent kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/hena-agent/AGENTS.md` speichern.
Diese gelten in allen hena-agent-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Hena Agent dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/hena-agent/AGENTS.md` existiert)
...
Beim Start sucht hena-agent in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/hena-agent/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `hena-agent.json` oder in `~/.config/hena-agent/hena.json` angeben.
...
hena-agent parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit hena-agent.json

Empfohlen ist das `instructions`-Feld in der `hena-agent.json`:
...
Du kannst hena-agent explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `hena-agent.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
