...
Umiejętności agenta pozwalają hena-agent odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
hena-agent przeszukuje te lokalizacje:

- Project config: `.hena-agent/skills/<name>/SKILL.md`
- Global config: `~/.config/hena-agent/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu hena-agent przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.hena-agent/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/hena-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.hena-agent/skills/git-release/SKILL.md` w ten sposób:
...
hena-agent wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `hena-agent.json`:
...
**Dla agentów wbudowanych** (w `hena-agent.json`):
