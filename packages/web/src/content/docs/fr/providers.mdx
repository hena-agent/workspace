---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Hena.
...
en `~/.local/share/hena/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Hena
configuration.
...
Hena Zen est une liste de modèles fournis par l'équipe Hena qui ont été
testé et vérifié pour fonctionner correctement avec Hena. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Hena Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez hena et dirigez-vous vers [hena.dev/auth](https://hena.dev/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Hena et son utilisation est totalement facultative.

---

## Hena Go

Hena Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Hena qui ont été
testé et vérifié pour fonctionner correctement avec Hena.
...
Pour utiliser Amazon Bedrock avec Hena :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de hena :
...
Pour une configuration spécifique au projet ou persistante, utilisez `hena.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Hena n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer hena pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que hena fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration Hena.
...
pour stocker le jeton dans le stockage d'authentification Hena.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Hena utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Hena
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `hena.json`. Il est également recommandé de désactiver le partage de session.
...
Pour utiliser votre abonnement GitHub Copilot avec hena :
...
Pour utiliser Google Vertex AI avec Hena :
...
Définissez-les lors de l'exécution de Hena.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via hena, vous pouvez toujours le configurer vous-même.
...
Vous pouvez configurer hena pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer hena pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer hena pour utiliser des modèles locaux via Ollama.
...
Pour utiliser Ollama Cloud avec Hena :
...
3. Copiez la clé API à utiliser dans Hena.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Hena, vous devez extraire les informations du modèle localement :
...
Hena Zen est une liste de modèles testés et vérifiés fournie par l'équipe Hena. [En savoir plus](/docs/zen).
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration Hena.
...
5. Vous pouvez également les personnaliser via votre configuration Hena. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Hena :
...
Vous pouvez également personnaliser les modèles via votre configuration Hena. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec Hena. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `hena.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Hena de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `hena auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration hena et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration Hena.
