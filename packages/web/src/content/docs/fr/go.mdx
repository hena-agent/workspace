...
Go fonctionne comme n'importe quel autre fournisseur dans Hena. Vous vous abonnez à Hena Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Hena.
...
Hena Go fonctionne comme n'importe quel autre fournisseur dans Hena.
...
L'[ID de modèle](/docs/config/#models) dans votre configuration Hena utilise le format `hena-go/<model-id>`. Par exemple, pour Kimi K3, vous utiliseriez `hena-go/kimi-k3` dans votre configuration.
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec Hena également.
