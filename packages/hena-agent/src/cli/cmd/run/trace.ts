...
//
// Enable with HENA_AGENT_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/hena-agent/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.HENA_AGENT_DIRECT_TRACE) {
    state = false
    return undefined
  }
