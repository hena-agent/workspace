...
Erstellen Sie `.hena-agent/commands/test.md`:

```md title=".hena-agent/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Hena Agent-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Hena Agent [config](/docs/config):

```json title="hena-agent.jsonc" {4-12}
{
...
- Global: `~/.config/hena-agent/commands/`
- Pro Projekt: `.hena-agent/commands/`

```markdown title="~/.config/hena-agent/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".hena-agent/commands/component.md"
---
...
```md title=".hena-agent/commands/create-file.md"
---
...
```md title=".hena-agent/commands/analyze-coverage.md"
---
...
```md title=".hena-agent/commands/review-changes.md"
---
...
```md title=".hena-agent/commands/review-component.md"
---
...
Hena Agent enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
