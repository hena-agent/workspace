---
title: サーバー
description: HTTP 経由で Hena サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

`hena serve` コマンドは、Hena クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `hena.local` |
...
HTTP 基本認証でサーバーを保護するには、`HENA_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `hena` になるか、`HENA_SERVER_USERNAME` を設定してオーバーライドします。これは、`hena serve` と `hena web` の両方に当てはまります。
...
`hena` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Hena サーバーを使用して、プログラムで Hena と対話します。
:::

`hena serve` を実行してスタンドアロンサーバーを起動できます。すでに Hena TUI を実行している場合でも、`hena serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Hena [IDE](/docs/ide) プラグインによって使用されます。
