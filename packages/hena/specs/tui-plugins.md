...
- Author package entrypoint is `@hena/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/hena-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@hena/plugin/tui`.
- `@hena/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@hena/plugin/tui"
...
- The value is a semver range checked against the running Hena version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.hena` is absent, no check is performed (backward compatible).
...
- `hena plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `hena plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.hena` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.hena`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.hena`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.hena` dir or `HENA_CONFIG_DIR`, Hena installs `@hena/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@hena/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Hena's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Hena registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Hena pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@hena/plugin/tui`.
...
- `message` is required; `title` defaults to `"Hena"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `hena.default`.
...
- Local plugins persist installed themes under the local `.hena/themes` area near the plugin config source.
...
- `--pure` / `HENA_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.hena/plugins/tui-smoke.tsx`
- Local vim plugin: `.hena/plugins/tui-vim.tsx`
- Local smoke config: `.hena/tui.json`
- Local smoke theme: `.hena/plugins/smoke-theme.json`
