...
Le skill degli agenti permettono a Hena di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Hena cerca in queste posizioni:

- Config di progetto: `.hena/skills/<name>/SKILL.md`
- Config globale: `~/.config/hena/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Hena risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.hena/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/hena/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.hena/skills/git-release/SKILL.md` cosi':
...
Hena elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `hena.json`:
...
**Per agenti integrati** (in `hena.json`):
