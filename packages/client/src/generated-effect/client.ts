...
type Endpoint3_3Request = Parameters<RawClient["server.session"]["session.attach"]>[0]
type Endpoint3_3Input = {
  readonly sessionID: Endpoint3_3Request["params"]["sessionID"]
  readonly directory: Endpoint3_3Request["payload"]["directory"]
}
const Endpoint3_3 = (raw: RawClient["server.session"]) => (input: Endpoint3_3Input) =>
  raw["session.attach"]({ params: { sessionID: input["sessionID"] }, payload: { directory: input["directory"] } }).pipe(
    Effect.mapError(mapClientError),
    Effect.map((value) => value.data),
  )
