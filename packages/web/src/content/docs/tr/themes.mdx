...
hena ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak hena kendi `hena` temasını kullanır.
...
hena birden fazla yerleşik temayla gelir.
...
- hena'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
hena, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/hena/themes/*.json` veya `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.hena/themes/*.json`
4. **Geçerli çalışma dizini** - `./.hena/themes/*.json`
