...
import type { AssistantMessage, Message, HenaAgentClient, SessionMessageResponse } from "@hena-agent/sdk/v2"
...
export const AuthMethodID = "hena-agent-login"
...
export class Service extends Context.Service<Service, Interface>()("@hena-agent/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `hena-agent auth login` in the terminal",
      name: "Login with Hena Agent",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: HenaAgentClient) {
...
function makeUsageService(sdk: HenaAgentClient) {
...
async function loadDirectorySnapshot(sdk: HenaAgentClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, Hena Agent provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const henaAgentProvider =
    providers[ProviderV2.ID.make("hena-agent")]
  const henaAgentModel = henaAgentProvider ? Provider.sort(Object.values(henaAgentProvider.models))[0] : undefined
  if (henaAgentProvider && henaAgentModel) return { providerID: henaAgentProvider.id, modelID: henaAgentModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "Hena Agent prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Hena Agent service failure", service })
}
