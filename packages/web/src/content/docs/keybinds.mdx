...
Hena Agent has a list of keybinds that you can customize through `tui.json`.
...
Hena Agent uses a `leader` key for many keybinds. This avoids conflicts in your terminal.
...
`leader_timeout` controls how long Hena Agent waits for the next key after the leader key. It defaults to `2000` milliseconds.
...
The Hena Agent desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `hena-agent.json`.
