---
title: Plugins
description: Skriv dine egne plugins for at udvide Hena Agent.
---

Plugins giver dig mulighed for at udvide Hena Agent ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Hena Agents standardadfærd.
...
- `.hena-agent/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/hena-agent/node_modules/`.
...
1. Global konfiguration (`~/.config/hena-agent/hena.json`)
2. Projektkonfiguration (`hena-agent.json`)
3. Global plugin-mappe (`~/.config/hena-agent/plugins/`)
4. Projekt plugin bibliotek (`.hena-agent/plugins/`)
...
Hena Agent kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En hena-agent SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide Hena Agent.
...
:::note
Hvis du bruger Hena Agent desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå hena-agent i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til hena-agent:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som hena-agent kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for hena-agent sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://hena.dev/docs/sdk) for detaljer.
