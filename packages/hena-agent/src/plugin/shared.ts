...
export async function checkPluginCompatibility(target: string, henaAgentVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(henaAgentVersion) || semver.major(henaAgentVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines["hena-agent"]
  if (typeof range !== "string") return
  if (!semver.satisfies(henaAgentVersion, range)) {
    throw new Error(`Plugin requires Hena Agent ${range} but running ${henaAgentVersion}`)
  }
}
