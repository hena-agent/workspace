...
import { isRecord } from "@hena-agent/tui/util/record"
import { Global } from "@/global"
import { FSUtil } from "@hena-agent/core/fs-util"
import { CurrentWorkingDirectory } from "./tui-cwd"
import { ConfigPlugin } from "@/config/plugin"
import { TuiKeybind } from "@hena-agent/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@hena-agent/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old hena-agent.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any legacy or current project config
  // folders between cwd and home, and HENA_AGENT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.HENA_AGENT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit HENA_AGENT_TUI_CONFIG override, if set.
  if (Flag.HENA_AGENT_TUI_CONFIG) {
    const configFile = Flag.HENA_AGENT_TUI_CONFIG
  ...
  // 4. Project config directories (and HENA_AGENT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".hena-agent") || dir === Flag.HENA_AGENT_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".hena-agent") && dir !== Flag.HENA_AGENT_CONFIG_DIR) continue
