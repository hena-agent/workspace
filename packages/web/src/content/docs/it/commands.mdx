...
Puoi aggiungere comandi personalizzati tramite la config di Hena o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di Hena:
...
- Globale: `~/.config/hena/commands/`
- Per progetto: `.hena/commands/`
...
hena include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
