...
مع Hena Agent يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Hena Agent سمتنا `hena-agent`.
...
يأتي Hena Agent مع عدة سمات مدمجة.
...
- يريدون أن يطابق Hena Agent مظهر terminal لديهم
...
يدعم Hena Agent نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/hena-agent/themes/*.json` أو `$XDG_CONFIG_HOME/hena-agent/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.hena-agent/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.hena-agent/themes/*.json`
