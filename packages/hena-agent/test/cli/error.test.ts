...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/hena-agent.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/hena-agent.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/hena-agent.jsonc", dir: ".hena-agent", suggestion: "hena-agent" },
        expected:
          'Directory ".hena-agent" in /tmp/hena-agent.jsonc is not valid. Rename the directory to "hena-agent" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/hena-agent.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/hena-agent.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.hena.dev/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.hena.dev/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `hena-agent models` to list available models",
      "Or check your config (hena-agent.json) provider/model names",
    ].join("\n")
