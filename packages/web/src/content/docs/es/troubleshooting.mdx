...
Para depurar problemas con Hena, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\hena\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `hena --log-level DEBUG`.

---

## Almacenamiento

hena almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/hena/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\hena`
...
Hena Desktop ejecuta un servidor Hena local (el sidecar `hena-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Hena` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/hena/hena.jsonc` (o `~/.config/hena/hena.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/hena/hena.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\hena\hena.jsonc`
...
Hena también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/hena/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\hena\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Hena pueda reconstruirlo.

1. Salga de Hena Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/hena`
- **Linux**: eliminar `~/.cache/hena` (o ejecutar `rm -rf ~/.cache/hena`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\hena`

3. Reinicie el escritorio Hena.

---

### Solucionar problemas de conexión del servidor

Hena Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `hena.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `HENA_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `HENA_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Hena requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Hena.

---

### Notificaciones que no se muestran

Hena Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Hena en la configuración de su sistema operativo, y
...
1. Salga del escritorio Hena.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Hena):

- `hena.settings.dat` (URL del servidor predeterminado de escritorio)
- `hena.global.dat` y `hena.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `hena upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `hena models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\hena`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. hena instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\hena`

2. Reinicie hena para reinstalar los paquetes del proveedor más recientes.

Esto obligará a hena a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
hena detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
