...
Ajan becerileri, hena-agent'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
hena-agent su konumlari tarar:

- Proje konfigurasyonu: `.hena-agent/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/hena-agent/skills/<name>/SKILL.md`
...
Proje yerel yollarinda hena-agent, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.hena-agent/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/hena-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.hena-agent/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
hena-agent, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`hena-agent.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`hena-agent.json` icinde):
