...
import { FileSystemWatcher } from "@hena-agent/schema/filesystem-watcher"
...
declare const HENA_AGENT_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof HENA_AGENT_LIBC === "undefined" ? undefined : HENA_AGENT_LIBC
...
export class Service extends Context.Service<Service, Interface>()("@hena-agent/v2/FileWatcher") {}

const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.HENA_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.HENA_AGENT_EXPERIMENTAL_FILEWATCHER)) {
