...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".hena-agent")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .hena-agent/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const henaAgent = path.join(test.directory, ".hena-agent")
      const tool = path.join(henaAgent, "tool")
  ...
  it.instance("ignores non-tool exports in .hena-agent/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".hena-agent", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools from .hena-agent/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const henaAgent = path.join(test.directory, ".hena-agent")
      const tools = path.join(henaAgent, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".hena-agent", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const henaAgent = path.join(test.directory, ".hena-agent")
        const customTools = path.join(henaAgent, "tools")
        const plugin = path.join(henaAgent, "node_modules", "@hena-agent", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(henaAgent, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@hena-agent/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".hena-agent", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(henaAgent, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@hena-agent/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(henaAgent, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(henaAgent, "node_modules", "cowsay")
