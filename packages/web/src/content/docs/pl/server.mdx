---
title: Serwer
description: Interakcja z serwerem hena za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

Komenda `hena serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient Hena.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `hena.local` |
...
Ustaw `HENA_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `hena` lub ustaw `HENA_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `hena serve`, jak i `hena web`.
...
Kiedy uruchomisz `hena`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera hena do programowej interakcji z Hena.
:::

Ta architektura umożliwia obsługę wielu klientów przez hena i programową interakcję z kodem otwartym.

Możesz uruchomić `hena serve`, aby uruchomić samodzielny serwer. Jeśli masz
hena TUI działa, `hena serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki hena [IDE](/docs/ide).
...
Serwer hena udostępnia następujące interfejsy API.
