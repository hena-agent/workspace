---
title: Regeln
description: Lege eigene Anweisungen fuer hena fest.
---

Du kannst hena ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in hena den Befehl `/init` aus.
...
So kann hena sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht hena sie zu erweitern.
...
hena kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/hena/AGENTS.md` speichern.
Diese gelten in allen hena-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Hena dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/hena/AGENTS.md` existiert)
...
Beim Start sucht hena in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/hena/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `hena.json` oder in `~/.config/hena/hena.json` angeben.
...
hena parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit hena.json

Empfohlen ist das `instructions`-Feld in der `hena.json`:
...
Du kannst hena explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `hena.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
