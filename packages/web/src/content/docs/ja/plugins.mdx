---
title: プラグイン
description: Hena Agent を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Hena Agent を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Hena Agent のデフォルトの動作を変更したりできます。
...
- `.hena-agent/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/hena-agent/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/hena-agent/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/hena-agent/hena.json`)
2. プロジェクト設定 (`hena-agent.json`)
3. グローバルプラグインディレクトリ (`~/.config/hena-agent/plugins/`)
4. プロジェクトプラグインディレクトリ (`.hena-agent/plugins/`)
...
Hena Agent は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Hena Agent SDK クライアント。
...
ここでは、Hena Agent を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://hena.dev/docs/sdk) を参照してください。
...
Hena Agent が `.env` ファイルを読み取らないようにします。
...
プラグインは Hena Agent にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Hena Agent が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Hena Agent で使用できます。
