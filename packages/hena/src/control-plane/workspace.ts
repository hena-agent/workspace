...
import { EventSequenceTable, EventTable } from "@hena/core/event/sql"
...
import { WorkspaceEvent } from "@hena/schema/workspace-event"
...
export class Service extends Context.Service<Service, Interface>()("@hena/Workspace") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Workspace.create")(function* (input: CreateInput) {
      ...
      const env = {
        HENA_AUTH_CONTENT: JSON.stringify(yield* auth.all()),
        HENA_WORKSPACE_ID: config.id,
        HENA_EXPERIMENTAL_WORKSPACES: "true",
