...
Umiejętności agenta pozwalają hena odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
hena przeszukuje te lokalizacje:

- Project config: `.hena/skills/<name>/SKILL.md`
- Global config: `~/.config/hena/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu hena przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.hena/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/hena/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.hena/skills/git-release/SKILL.md` w ten sposób:
...
hena wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `hena.json`:
...
**Dla agentów wbudowanych** (w `hena.json`):
