...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Hena posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Hena.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `HENA_EXPERIMENTAL_LSP_TOOL=true` (lub `HENA_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Hena lub gdy zmienna środowiskowa `HENA_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Hena:
