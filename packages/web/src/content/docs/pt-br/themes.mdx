...
Com o hena-agent, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o hena-agent usa nosso próprio tema `hena-agent`.
...
O hena-agent vem com vários temas embutidos.
...
- Querem que o hena-agent corresponda à aparência do seu terminal
...
O hena-agent suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/hena-agent/themes/*.json` ou `$XDG_CONFIG_HOME/hena-agent/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.hena-agent/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.hena-agent/themes/*.json`
