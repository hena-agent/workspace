...
W Hena Agent obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Hena Agent zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Hena Agent ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Hena Agent ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Hena Agent, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `hena-agent.json`:
...
- Globalnie: `~/.config/hena-agent/agents/`
- W projekcie: `.hena-agent/agents/`
...
Jeśli nie zostanie skonfigurowana, Hena Agent używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Hena Agent, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Hena Agent w `provider/model-id`. Na przykład, [Hena Zen](/docs/zen), użyj `hena-agent/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `hena-agent models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/hena-agent/hena).
:::
