  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://hena-agent.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://hena-agent.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://hena-agent.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://hena-agent.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://hena-agent.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://hena-agent.example.com"),
      ),
    ).toEqual({
