...
const model = "hena-agent/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/hena-agent")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with Hena Agent...`)
  ...
  try {
    await $`hena-agent run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  Hena Agent failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with Hena Agent")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with Hena Agent...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/hena-agent`.",
