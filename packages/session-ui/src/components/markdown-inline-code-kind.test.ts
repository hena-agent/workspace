...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@hena/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/hena`)).toBe("path")
    expect(inlineCodeKind(`@hena/app`)).toBe("path")
  ...
  test("detects urls", () => {
    ...
    expect(inlineCodeKind(`file:///tmp/hena`)).toBeUndefined()
