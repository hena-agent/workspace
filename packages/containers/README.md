...
```
REGISTRY=ghcr.io/hena-agent/hena TAG=24.04 bun ./packages/containers/script/build.ts
REGISTRY=ghcr.io/hena-agent/hena TAG=24.04 bun ./packages/containers/script/build.ts --push
```
...
jobs:
  build-cli:
    ...
    container:
      image: ghcr.io/hena-agent/hena/build/bun-node:24.04
```
