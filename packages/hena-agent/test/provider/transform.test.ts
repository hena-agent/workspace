    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".hena-agent/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, henaAgentModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.["hena-agent"]?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.["hena-agent"]?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          "hena-agent": { itemId: "msg_hena_agent" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              "hena-agent": { itemId: "msg_hena_agent_part" },
    ...
    expect(result[0].providerOptions?.["hena-agent"]?.itemId).toBe("msg_hena_agent")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.["hena-agent"]?.itemId).toBe("msg_hena_agent_part")
