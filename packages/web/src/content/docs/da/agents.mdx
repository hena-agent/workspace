...
Der er to typer agenter i Hena Agent; primære agenter og subagenter.
...
Hena Agent leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Hena Agent leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Hena Agent leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Hena Agent's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `hena-agent.json`-konfigurationsfil:
...
- Globalt: `~/.config/hena-agent/agents/`
- Pr. projekt: `.hena-agent/agents/`
...
Hvis ingen temperatur er angivet, bruger Hena Agent modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Hena Agent-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Hena Agent-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Hena Zen](/docs/zen), vil du bruge `hena-agent/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `hena-agent models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/hena-agent/hena).
:::
