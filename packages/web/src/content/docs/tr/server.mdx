---
title: Sunucu
description: hena-agent sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`hena-agent serve` komutu, hena-agent istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `hena-agent.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `HENA_AGENT_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `hena-agent` değeridir; değiştirmek isterseniz `HENA_AGENT_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `hena-agent serve` hem de `hena-agent web` için geçerlidir.
...
`hena-agent` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
hena-agent ile programatik etkileşim için hena-agent sunucusunu kullanın.
:::

Bu mimari, hena-agent'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`hena-agent serve` ile bağımsız bir sunucu başlatabilirsiniz. hena-agent TUI açıksa, `hena-agent serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum hena-agent [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
hena-agent sunucusu aşağıdaki API'leri sunar.
