...
description: Hena, LSP sunucularınızla bütünleşir.
---

Hena, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Hena, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`HENA_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve hena bir dosyayı açtığında:
...
LSP sunucularını, hena yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
