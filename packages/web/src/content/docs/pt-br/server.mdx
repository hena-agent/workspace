---
title: Servidor
description: Interaja com o servidor hena via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

O comando `hena serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente hena pode usar.
...
```bash
hena serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `hena.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
hena serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `HENA_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `hena`, ou defina `HENA_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `hena serve` quanto ao `hena web`.

```bash
HENA_SERVER_PASSWORD=your-password hena serve
```
...
Quando você executa `hena`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor hena para interagir com o hena programaticamente.
:::

Essa arquitetura permite que o hena suporte múltiplos clientes e permite que você interaja com o hena programaticamente.

Você pode executar `hena serve` para iniciar um servidor autônomo. Se você tiver o TUI do hena em execução, `hena serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do hena [IDE](/docs/ide).
...
O servidor hena expõe as seguintes APIs.
