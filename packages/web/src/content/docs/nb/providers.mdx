---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Hena-konfigurasjonen.
...
i `~/.local/share/hena/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Hena
konfig.
...
Hena Zen er en liste over modeller levert av Hena-teamet som har vært
testet og verifisert for å fungere godt med Hena. [Finn ut mer](/docs/zen).
...
Det fungerer som alle andre leverandører i Hena og er helt valgfritt å bruke.

---

## Hena Go

Hena Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Hena-teamet som har vært
testet og verifisert for å fungere godt med Hena.
...
Slik bruker du Amazon Bedrock med Hena:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Hena:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `hena.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i hena støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere hena til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Hena skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Hena-konfigurasjonen.
...
for å lagre token i Hena auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Hena bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Hena
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`hena.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Hena:
...
Slik bruker du Google Vertex AI med Hena:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Hena, kan du alltid konfigurere den selv.
...
Du kan konfigurere Hena for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Hena for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Hena for å bruke lokale modeller gjennom Ollama.
...
Slik bruker du Ollama Cloud med Hena:
...
3. Kopier API-nøkkelen for bruk i Hena.
...
6. **Viktig**: Før du bruker skymodeller i Hena, må du hente modellinformasjonen lokalt:
...
Hena Zen er en liste over testede og verifiserte modeller levert av Hena-teamet. [Finn ut mer](/docs/zen).
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Hena-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Hena-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Hena:
...
Du kan også tilpasse modeller gjennom Hena-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Hena. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `hena.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Hena forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `hena auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Hena-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Hena-konfigurasjonen.
