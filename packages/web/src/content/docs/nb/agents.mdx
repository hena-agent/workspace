...
Det er to typer agenter i Hena Agent; primære agenter og underagenter.
...
Hena Agent kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Hena Agent kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Hena Agent kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `hena-agent.json`:
...
- Per prosjekt: `.hena-agent/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Hena Agent modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Hena Agent-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Hena Agent-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Hena Zen](/docs/zen), vil du bruke `hena-agent/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `hena-agent models` for å se en liste over tilgjengelige modeller.
:::
