---
title: Sunucu
description: hena sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/develop/packages/sdk/js/src/gen/types.gen.ts`

`hena serve` komutu, hena istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `hena.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `HENA_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `hena` değeridir; değiştirmek isterseniz `HENA_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `hena serve` hem de `hena web` için geçerlidir.
...
`hena` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
hena ile programatik etkileşim için hena sunucusunu kullanın.
:::

Bu mimari, hena'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`hena serve` ile bağımsız bir sunucu başlatabilirsiniz. hena TUI açıksa, `hena serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum hena [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
hena sunucusu aşağıdaki API'leri sunar.
