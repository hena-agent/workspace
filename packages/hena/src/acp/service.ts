...
import type { AssistantMessage, Message, HenaClient, SessionMessageResponse } from "@hena/sdk/v2"
...
export const AuthMethodID = "hena-login"
...
export class Service extends Context.Service<Service, Interface>()("@hena/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `hena auth login` in the terminal",
      name: "Login with Hena",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: HenaClient) {
...
function makeUsageService(sdk: HenaClient) {
...
async function loadDirectorySnapshot(sdk: HenaClient, directory: string) {
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Hena service failure", service })
}
