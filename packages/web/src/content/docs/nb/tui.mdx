---
title: TUI
description: Bruke hena-agent-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hena Agent gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Hena Agent starter TUI for gjeldende katalog.
...
Når du bruker Hena Agent TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Hena Agent. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Hena Agent. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `hena-agent.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge Hena Agent venter etter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `hena-agent.default`.
