// Subprocess integration tests for `hena run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `hena.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `HENA_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("hena run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, hena }) =>
      Effect.gen(function* () {
        ...
        const result = yield* hena.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, hena }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* hena.run("think", { extraArgs: ["--thinking"] })
        hena.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* hena.run("think again")
        hena.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ hena }) =>
      Effect.gen(function* () {
        const result = yield* hena.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, hena }) =>
      Effect.gen(function* () {
        ...
        const result = yield* hena.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, hena }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* hena.run("request permission", { permission: { bash: "ask" } })
        hena.expectExit(denied, 0)
        ...
        const allowed = yield* hena.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        hena.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* hena.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        hena.expectExit(explicitlyDenied, 0)
  ...
  cliIt.concurrent(
    ...
    ({ home, hena }) =>
      Effect.gen(function* () {
        const result = yield* hena.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, hena }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* hena.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
