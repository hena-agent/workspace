---
title: TUI
description: Korištenje korisničkog interfejsa Hena Agent terminala.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hena Agent pruža interaktivni terminalski interfejs ili TUI za rad na vašim projektima sa LLM.

Pokretanje Hena Agent pokreće TUI za trenutni direktorij.
...
Kada koristite Hena Agent TUI, možete upisati `/` nakon čega slijedi ime komande da biste brzo izvršili radnje. na primjer:
...
Dodajte provajdera u Hena Agent. Omogućava vam da odaberete između dostupnih provajdera i dodate njihove API ključeve.
...
Izađite iz Hena Agent. _Aliases_: `/quit`, `/q`
...
Ovo je odvojeno od `hena-agent.json`, koji konfiguriše ponašanje servera/izvršavanja.
...
- `leader_timeout` - Kontroliše koliko dugo Hena Agent čeka nakon leader key. Podrazumijevano je `2000`.
...
Koristite `HENA_AGENT_TUI_CONFIG` da učitate prilagođenu putanju TUI konfiguracije.

### Attention

Attention omogućava da vas TUI obavijesti kada Hena Agent čeka odgovor, treba odobrenje dozvole, prijavi grešku sesije ili završi sesiju. Omogućite ga pomoću `attention.enabled`; ugrađeni događaji reproduciraju zvuk kada se dogode. Desktop obavještenja se šalju samo kada prozor terminala nije u fokusu i ne koriste se za subagent događaje.
...
- `sound_pack` - Sound pack ID koji se koristi. Podrazumijevano je `hena-agent.default`.
