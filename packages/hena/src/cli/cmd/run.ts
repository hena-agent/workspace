...
export const RunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: HenaClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: HenaClient, sessionID: string) {
      ...
      async function current(sdk: HenaClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: HenaClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: HenaClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: HenaClient) {
        ...
        async function loop(client: HenaClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
