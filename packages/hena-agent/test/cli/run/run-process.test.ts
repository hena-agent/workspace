// Subprocess integration tests for `hena-agent run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `henaAgent.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `HENA_AGENT_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("hena-agent run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, henaAgent }) =>
      Effect.gen(function* () {
        ...
        const result = yield* henaAgent.run("say hi")
        henaAgent.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, henaAgent }) =>
      Effect.gen(function* () {
        ...
        const result = yield* henaAgent.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, henaAgent }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* henaAgent.run("think", { extraArgs: ["--thinking"] })
        henaAgent.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* henaAgent.run("think again")
        henaAgent.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, henaAgent }) =>
      Effect.gen(function* () {
        ...
        const result = yield* henaAgent.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, henaAgent }) =>
      Effect.gen(function* () {
        ...
        const result = yield* henaAgent.run("say hi", { format: "json" })
        henaAgent.expectExit(result, 0)

        const events = henaAgent.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ henaAgent }) =>
      Effect.gen(function* () {
        const result = yield* henaAgent.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, henaAgent }) =>
      Effect.gen(function* () {
        ...
        const result = yield* henaAgent.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, henaAgent }) =>
      Effect.gen(function* () {
        ...
        const result = yield* henaAgent.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, henaAgent }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* henaAgent.run("request permission", { permission: { bash: "ask" } })
        henaAgent.expectExit(denied, 0)
        ...
        const allowed = yield* henaAgent.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        henaAgent.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* henaAgent.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        henaAgent.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, henaAgent }) =>
      Effect.gen(function* () {
        ...
        const server = yield* henaAgent.serve()

        const result = yield* henaAgent.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, henaAgent }) =>
      Effect.gen(function* () {
        const result = yield* henaAgent.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, henaAgent }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* henaAgent.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
