---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Hena-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Hena anpassen
config.
...
Hena Zen ist eine Liste von Modellen, die vom Hena-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Hena funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Hena Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Hena aus und gehen Sie zu [hena.dev/auth](https://hena.dev/auth).
...
Es funktioniert wie jeder andere Anbieter in Hena und ist völlig optional.

---

## Hena Go

Hena Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Hena-Team getestet und verifiziert wurden, dass sie gut mit Hena funktionieren.
...
So verwenden Sie Amazon Bedrock mit Hena:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Hena ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `hena.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Hena wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können hena so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Hena ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Hena-Konfiguration hinzufügen.
...
um das Token im Hena-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Hena verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Hena
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`hena.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Hena:
...
So verwenden Sie Google Vertex AI mit Hena:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Hena ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Hena konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Sie können Hena für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Hena über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Hena über Ollama für die Verwendung lokaler Modelle konfigurieren.
...
So verwenden Sie Ollama Cloud mit Hena:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Hena.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Hena verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Hena Zen ist eine Liste getesteter und verifizierter Modelle, die vom Hena-Team bereitgestellt werden. [Learn more](/docs/zen).
...
3. Geben Sie Ihren Hena API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Hena-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Hena-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Hena:
...
Sie können Modelle auch über Ihre Hena-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Hena verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `hena.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Hena erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `hena auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Hena-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Hena-Konfiguration übereinstimmt.
