...
const TUI_SCHEMA_URL = "https://hena.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from hena-agent.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const henaAgent = await henaAgentFiles(input)
  for (const file of henaAgent) {
...
async function henaAgentFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "hena-agent"),
    ...(await Filesystem.findUp(["hena-agent.json", "hena-agent.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "hena-agent"))
  }
  if (Flag.HENA_AGENT_CONFIG) files.push(Flag.HENA_AGENT_CONFIG)
