---
title: TUI
description: Using the Hena Agent terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hena Agent provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Hena Agent starts the TUI for the current directory.
...
When using the Hena Agent TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Hena Agent. Allows you to select from available providers and add their API keys.
...
Exit Hena Agent. _Aliases_: `/quit`, `/q`
...
This is separate from `hena-agent.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Hena Agent waits after the leader key. Defaults to `2000`.
...
Use `HENA_AGENT_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `hena-agent.default`.
