...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@hena-agent/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@hena-agent/core/effect/app-node-builder"
import { errorData, errorMessage } from "@hena-agent/tui/util/error"
...
import { hasTheme, upsertTheme } from "@hena-agent/tui/context/theme"
import { Global } from "@/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@hena-agent/core/util/flock"
import { Flag } from "@/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@hena-agent/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@hena-agent/core/v1/config/plugin"
import { createCommandShim } from "@hena-agent/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@hena-agent/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".hena-agent"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".hena-agent", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".hena-agent", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.HENA_AGENT_PURE ? [] : pluginOrigins
    if (Flag.HENA_AGENT_PURE && pluginOrigins.length) {
    }
