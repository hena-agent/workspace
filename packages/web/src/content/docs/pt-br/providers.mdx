---
title: Provedores
description: Usando qualquer provedor LLM no Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

hena usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do Hena.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/hena/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do Hena.
...
Hena Zen é uma lista de modelos fornecidos pela equipe do hena que foram testados e verificados para funcionar bem com o Hena. [Saiba mais](/docs/zen).
...
Funciona como qualquer outro provedor no hena e é completamente opcional.

---

## Hena Go

Hena Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do hena que foram
testados e verificados para funcionar bem com o Hena.
...
Para usar o Amazon Bedrock com o hena:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o hena:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `hena.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no hena não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o hena para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o hena funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do Hena.
...
armazenar o token no armazenamento de autenticação do Hena.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
hena usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o hena
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `hena.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `hena.json`:
...
Para usar sua assinatura do GitHub Copilot com o hena:
...
Para usar o Google Vertex AI com o hena:
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do hena, você sempre pode configurá-lo você mesmo.
...
Você pode configurar o hena para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o hena para usar modelos locais através do LM Studio.
...
Você pode configurar o hena para usar modelos locais através do Ollama.
...
Para usar o Ollama Cloud com o hena:
...
3. Copie a chave da API para uso no Hena.
...
6. **Importante**: Antes de usar modelos em nuvem no hena, você deve puxar as informações do modelo localmente:
...
Hena Zen é uma lista de modelos testados e verificados fornecidos pela equipe do Hena. [Saiba mais](/docs/zen).
...
3. Insira sua chave da API do Hena.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do Hena.
...
5. Você também pode personalizá-los através da sua configuração do Hena. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o hena:
...
Você também pode personalizar modelos através da sua configuração do Hena. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o Hena. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no hena.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `hena.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o hena entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `hena auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do hena e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do Hena.
