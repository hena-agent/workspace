...
مع Hena يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Hena سمتنا `hena`.
...
- يريدون أن يطابق Hena مظهر terminal لديهم
...
يدعم Hena نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/hena/themes/*.json` أو `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.hena/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.hena/themes/*.json`
