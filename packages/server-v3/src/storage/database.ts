...
export function createSyncDatabase(database: Database) {
  ...
  return {
    ...
    compact(input: { now?: number; changeMaxAgeMs?: number; changeMaxRows?: number; idempotencyMaxAgeMs?: number } = {}) {
      return database.transaction(() => {
        ...
        database.query("DELETE FROM idempotency_record WHERE created_at < ?").run(
          (input.now ?? Date.now()) - (input.idempotencyMaxAgeMs ?? 30 * 24 * 60 * 60_000),
        )
