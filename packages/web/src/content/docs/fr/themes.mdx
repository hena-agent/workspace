...
Avec Hena, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Hena utilise notre propre thème `hena`.
...
Hena est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Hena corresponde à l'apparence de leur terminal
...
Hena prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/hena/themes/*.json` ou `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.hena/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.hena/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/hena/themes
vim ~/.config/hena/themes/my-theme.json
```
