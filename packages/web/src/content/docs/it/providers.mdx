---
title: Provider
description: Usa qualsiasi provider LLM in Hena.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Hena.
...
Hena Zen e una lista di modelli forniti dal team Hena, testati e
verificati per funzionare bene con Hena. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Hena Zen.
:::

1. Esegui `/connect` nella TUI, seleziona hena e vai su [hena.dev/auth](https://hena.dev/auth).
...
Funziona come qualsiasi altro provider in Hena ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Hena:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui hena:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `hena.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Hena non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare hena per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché hena funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di Hena.
...
salvare il token nello storage di auth di Hena.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Hena usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Hena
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `hena.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `hena.json`:
...
Per usare il tuo abbonamento GitHub Copilot con hena:
...
Per usare Google Vertex AI con Hena:
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da hena, puoi sempre configurarlo tu stesso.
...
Puoi configurare hena per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare hena per usare modelli locali tramite LM Studio.
...
Puoi configurare hena per usare modelli locali tramite Ollama.
...
Per usare Ollama Cloud con Hena:
...
3. Copia la chiave API da usare in Hena.
...
6. **Importante**: Prima di usare modelli cloud in Hena, devi scaricare le informazioni del modello localmente:
...
Hena Zen è una lista di modelli testati e verificati forniti dal team Hena. [Scopri di più](/docs/zen).
...
3. Inserisci la tua chiave API di Hena.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di Hena.
...
5. Puoi anche personalizzarli tramite la configurazione di Hena. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Hena:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di Hena. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con Hena. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `hena.json` nella directory del progetto:
...
I campi `limit` permettono a Hena di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `hena auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di hena e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config Hena.
