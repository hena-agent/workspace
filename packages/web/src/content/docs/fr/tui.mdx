---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Hena.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hena fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Hena démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Hena TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Hena. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `hena.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Hena attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `hena.default`.
