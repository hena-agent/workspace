...
import { useTheme } from "@hena-agent/ui/theme"
...
const featuredProviders = ["hena-agent", "hena-agent-go", "openai", "anthropic", "google", "github-copilot"]
...
export const DialogSelectModelUnpaidV2: Component<{ model?: ModelState }> = (props) => {
  ...
  const isFree = (item: ReturnType<ModelState["list"]>[number]) =>
    item.provider.id === "hena-agent" && (!item.cost || item.cost.input === 0)
