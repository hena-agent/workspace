---
title: Config
description: Using the Hena JSON config.
---

You can configure Hena using a JSON config file.

---

## Format

Hena supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="hena.jsonc"
{
...
1. **Remote config** (from `.well-known/hena`) - organizational defaults
2. **Global config** (`~/.config/hena/hena.json`) - user preferences
3. **Custom config** (`HENA_CONFIG` env var) - custom overrides
4. **Project config** (`hena.json` in project) - project-specific settings
5. **`.hena` directories** - agents, commands, plugins
6. **Inline config** (`HENA_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/hena/` on macOS) - admin-controlled
...
:::note
The `.hena` and `~/.config/hena` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/hena` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/hena"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Hena config in `~/.config/hena/hena.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/hena/tui.json`.
...
Add `hena.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Hena starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `HENA_CONFIG` environment variable.

```bash
export HENA_CONFIG=/path/to/my/custom-config.json
hena run "Hello world"
```
...
Specify a custom config directory using the `HENA_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.hena` directory, and should
follow the same structure.

```bash
export HENA_CONFIG_DIR=/path/to/my/config-directory
hena run "Hello world"
```

The custom directory is loaded after the global config and `.hena` directories, so it **can override** their settings.
...
Drop an `hena.json` or `hena.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/hena/` |
| Linux    | `/etc/hena/`                         |
| Windows  | `%ProgramData%\hena`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Hena reads managed preferences from the `ai.hena.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Hena checks these paths:

1. `/Library/Managed Preferences/<user>/ai.hena.managed.plist`
2. `/Library/Managed Preferences/ai.hena.managed.plist`

The plist keys map directly to `hena.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.hena.managed` PayloadType. The Hena config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.hena.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.hena.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.hena</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Use `HENA_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `hena.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `hena serve` and `hena web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Hena server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `hena.local`. Useful for running multiple instances on the same network.
...
If not specified, Hena will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Hena config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Hena tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Hena actions on configured resources. Currently, policies can control which providers Hena may use.
...
Hena normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Hena omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/hena/agents/` or `.hena/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Hena will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`hena run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/hena/commands/` or `.hena/commands/`. [Learn more here](/docs/commands).
...
Hena uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Hena will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, hena **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Hena with custom tools, hooks, and integrations.

Place plugin files in `.hena/plugins/` or `~/.config/hena/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Hena to only use specific providers rather than disabling them one by one.
