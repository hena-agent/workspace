...
Delete `packages/hena-agent/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/hena-agent/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/hena-agent/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/hena-agent`
- TUI plugin loader tests in `packages/hena-agent` if runtime plugin API wiring changed
