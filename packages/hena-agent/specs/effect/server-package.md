...
Practical reference for a future `packages/server` split after the Hena Agent
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/hena-agent`.
...
Until enough shared service code lives outside `packages/hena-agent`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/hena-agent`

It should not import `packages/hena-agent` services while `packages/hena-agent`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on Hena Agent-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/hena-agent` implementation details.
