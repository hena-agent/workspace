...
import type { IntegrationOAuthMethodRegistration } from "@hena/plugin/v2/effect/integration"
import { define } from "@hena/plugin/v2/effect/plugin"
import type { CredentialValue } from "@hena/sdk/v2/types"
...
export const HenaPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "hena",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("HenaPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("hena")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Hena provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("hena", (integration) => {
        integration.name = "Hena"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "hena", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("hena")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("hena")
      ...
      const item = catalog.provider.get(ProviderV2.ID.hena)
      if (!item) return
      const hasKey = Boolean(environment("HENA_API_KEY") || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("hena")),
