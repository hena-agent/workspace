...
export type PatchDeps = {
  ...
  files: (dir: string, name: "hena-agent" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".hena-agent")
}

function patchName(kind: Kind): "hena-agent" | "tui" {
  if (kind === "server") return "hena-agent"
  return "tui"
}
