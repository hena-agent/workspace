...
import { IdeEvent } from "@hena/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["HENA_CALLER"] === "vscode" || process.env["HENA_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.hena"], {
    nothrow: true,
  })
