...
import { llmClient } from "@hena/core/effect/app-node-platform"
...
import type { LLMEvent } from "@hena/llm"
import { LLMClient } from "@hena/llm/route"
import type { LLMClientService } from "@hena/llm/route"
...
export class Service extends Context.Service<Service, Interface>()("@hena/LLM") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // from the workflow service are executed via Hena's tool system
      ...
      if (language instanceof GitLabWorkflowLanguageModel) {
        ...
        workflowModel.sessionPreapprovedTools = Object.keys(prepared.tools).filter((name) => {
          const match = ruleset.findLast((rule) => Wildcard.match(Permission.catalogPermission(name), rule.permission))
