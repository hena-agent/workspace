---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć Hena.
---

Wtyczki umożliwiające wyłączenie hena poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania Hena.
...
- `.hena/plugins/` - Wtyczki na poziomie projektu
- `~/.config/hena/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/hena/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/hena/hena.json`)
2. Konfiguracja projektu (`hena.json`)
3. Globalny katalog wtyczek (`~/.config/hena/plugins/`)
4. Katalog wtyczek projektu (`.hena/plugins/`)
...
hena uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z hena do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do Hena.
...
:::note
Jeśli korzystasz z aplikacji komputerowej hena, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj hena czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do hena:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod Hena. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
