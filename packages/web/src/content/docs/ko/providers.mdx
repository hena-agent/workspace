---
title: 공급자
description: Hena에서 LLM 공급자를 사용합니다.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena는 [AI SDK](https://ai-sdk.dev/) 및 [Models.dev](https://models.dev)를 사용하여 **75개 이상의 LLM 공급자**를 지원하며 로컬 모델도 실행할 수 있습니다.

필요한 공급자를 추가하려면:

1. `/connect` 명령을 사용하여 공급자의 API 키를 추가하십시오.
2. Hena 구성에서 공급자를 구성하십시오.
...
`~/.local/share/hena/auth.json`

---

### 구성

Hena 설정의 `provider` 섹션을 통해 공급자를 사용자 정의할 수 있습니다.
...
Hena Zen은 Hena 팀이 Hena와 잘 작동하도록 테스트하고 검증한 모델 목록입니다. [더 알아보기](/docs/zen).
...
Hena의 다른 공급자처럼 작동하며 사용은 완전히 선택 사항입니다.

---

## Hena Go

Hena Go는 Hena 팀이 테스트하고 검증하여 Hena와 잘 작동하는 인기 있는 오픈 코딩 모델에 안정적으로 액세스할 수 있는 저렴한 구독 요금제입니다.
...
2. 다음 방법 중 하나를 사용하여 **인증을 구성**합니다:
   ...
   ***
   ...
   Hena를 실행하는 동안 다음 환경 변수 중 하나를 설정합니다:
   ...
   ***
   ...
   프로젝트별 또는 영구 구성을 위해 `hena.json`을 사용하십시오:
...
:::info
Hena에서 Claude Pro/Max 구독을 사용하는 것은 [Anthropic](https://anthropic.com)에서 공식적으로 지원하지 않습니다.
:::
...
OpenAI 호환 API 서버 뒤에서 로컬 LLM을 실행하는 데스크톱 애플리케이션인 [Atomic Chat](https://atomic.chat)을 통해 로컬 모델을 사용하도록 hena를 구성할 수 있습니다(기본 엔드포인트 `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)로 이동하여 모델을 배포합니다.
   ...
   :::note
   배포 이름은 제대로 작동하려면 Hena의 모델 이름과 일치해야 합니다.
   :::
...
:::note[규정 준수 참고 사항]
Hena는 세션 제목 생성과 같은 일부 AI 작업을 위해 작은 모델을 사용합니다.
기본적으로 Zen에서 호스팅되는 gpt-5-nano를 사용하도록 구성됩니다.
Hena를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `hena.json` 파일에 다음을 추가하십시오.
...
Hena에서 GitHub Copilot 구독을 사용하려면:
...
Hena를 통해 자동으로 구성되지 않는 Helicone의 기능이나 모델이 있는 경우 직접 구성할 수 있습니다.
...
6. **중요**: Hena에서 클라우드 모델을 사용하기 전에, 로컬에서 모델 정보를 가져와야 합니다:
...
Hena Zen은 Hena 팀에서 제공하는 테스트 및 검증된 모델 목록입니다. [더 알아보기](/docs/zen).
...
5. 또한 Hena 구성을 통해 사용자 정의할 수도 있습니다. 다음은 공급자 순서를 지정하는 예입니다.
...
Hena로 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)를 사용하려면:
...
Hena 구성을 통해 모델을 사용자 정의할 수도 있습니다. 다음은 공급자 라우팅 순서를 지정하는 예입니다.
...
:::tip
Hena에서 모든 OpenAI 호환 공급자를 사용할 수 있습니다. 대부분의 최신 AI 공급자는 OpenAI 호환 API를 제공합니다.
:::
...
4. 프로젝트 디렉토리에 `hena.json` 파일을 만들거나 업데이트하십시오.
...
`limit` 필드를 사용하면 Hena가 남은 컨텍스트 양을 파악할 수 있습니다. 표준 공급자는 models.dev에서 자동으로 이를 가져옵니다.
...
1. **인증 설정 확인**: `hena auth list`를 실행하여 공급자의 자격 증명이 구성에 추가되었는지 확인하십시오.

   Amazon Bedrock과 같이 인증을 위해 환경 변수에 의존하는 공급자에는 적용되지 않습니다.

2. 사용자 정의 공급자의 경우, Hena 구성을 확인하고 다음을 수행하십시오:
   - `/connect` 명령에 사용된 공급자 ID가 Hena 구성의 ID와 일치하는지 확인하십시오.
