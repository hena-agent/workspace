---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć Hena Agent.
---

Wtyczki umożliwiające wyłączenie hena-agent poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania Hena Agent.
...
- `.hena-agent/plugins/` - Wtyczki na poziomie projektu
- `~/.config/hena-agent/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/hena-agent/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/hena-agent/hena.json`)
2. Konfiguracja projektu (`hena-agent.json`)
3. Globalny katalog wtyczek (`~/.config/hena-agent/plugins/`)
4. Katalog wtyczek projektu (`.hena-agent/plugins/`)
...
hena-agent uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z hena-agent do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do Hena Agent.
...
:::note
Jeśli korzystasz z aplikacji komputerowej hena-agent, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj hena-agent czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do hena-agent:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod Hena Agent. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://hena.dev/docs/sdk).
