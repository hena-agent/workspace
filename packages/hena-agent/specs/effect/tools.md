...
Practical reference for the current tool-migration state in `packages/hena-agent`.
...
Tool tests should use the existing Effect helpers in `packages/hena-agent/test/lib/effect.ts`:
