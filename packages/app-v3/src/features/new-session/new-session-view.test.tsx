...
describe("NewSessionView", () => {
  ...
  test("sending the composer calls onStart with the text, agent, and model", async () => {
    ...
    let started: { text: string; agentId: string; modelId: string } | undefined
    ...
    expect(started).toEqual({ text: "Set up the new feature flag", agentId: agents[0].id, modelId: models[0].id })
  })
})
