...
import { VariantPlugin } from "@hena-agent/core/plugin/variant"
  ...
  it.effect("adds GLM 5.2 variants after catalog sources", () =>
    Effect.gen(function* () {
      ...
      yield* service.transform((catalog) => {
        ...
        catalog.model.update(ProviderV2.ID.henaAgent, ModelV2.ID.make("glm-5.2"), (model) => {
          model.api = {
      ...
      expect((yield* service.model.get(ProviderV2.ID.henaAgent, ModelV2.ID.make("glm-5.2")))?.variants).toEqual([
