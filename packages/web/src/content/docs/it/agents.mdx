...
In Hena Agent esistono due tipi di agenti: agenti primari e subagenti.
...
Hena Agent include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Hena Agent include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

Hena Agent include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di Hena Agent, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `hena-agent.json`:
...
- Globale: `~/.config/hena-agent/agents/`
- Per progetto: `.hena-agent/agents/`
...
Se non specifichi una temperature, Hena Agent usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Hena Agent sia per la config specifica del progetto.
...
L'ID modello nella configurazione Hena Agent usa il formato `provider/model-id`. Per esempio, se stai usando [Hena Zen](/docs/zen), useresti `hena-agent/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `hena-agent models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/hena-agent/hena).
:::
