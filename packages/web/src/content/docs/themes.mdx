...
With Hena Agent you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Hena Agent uses our own `hena-agent` theme.
...
Hena Agent comes with several built-in themes.
...
- Want Hena Agent to match their terminal's appearance
...
Hena Agent supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/hena-agent/themes/*.json` or `$XDG_CONFIG_HOME/hena-agent/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.hena-agent/themes/*.json`
4. **Current working directory** - Defined in `./.hena-agent/themes/*.json`
