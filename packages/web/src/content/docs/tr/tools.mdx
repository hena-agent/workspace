...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. hena yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
hena içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `HENA_EXPERIMENTAL_LSP_TOOL=true` (veya `HENA_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca hena provider kullanılırken veya `HENA_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

hena başlatırken etkinleştirmek için:
