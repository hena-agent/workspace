---
title: Tjener
description: Samhandle med Hena Agent-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `hena-agent serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Hena Agent-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `hena-agent.local` |
...
Sett `HENA_AGENT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `hena-agent` som standard, eller sett `HENA_AGENT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `hena-agent serve` og `hena-agent web`.
...
Når du kjører `hena-agent` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Hena Agent-serveren til å samhandle med Hena Agent programmatisk.
:::

Denne arkitekturen lar Hena Agent støtte flere klienter og lar deg samhandle med Hena Agent programmatisk.

Du kan kjøre `hena-agent serve` for å starte en frittstående server. Hvis du har
Hena Agent TUI kjører, vil `hena-agent serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Hena Agent [IDE](/docs/ide) plugins.
...
hena-agent-serveren viser følgende APIer.
