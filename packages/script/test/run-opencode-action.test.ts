...
describe("run-opencode review action", () => {
  ...
  test("shares one pinned OpenCode setup", async () => {
    ...
    expect(setup).toContain('OPENCODE_VERSION="${REQUESTED_VERSION:-1.18.29}"')
  ...
  test("runs maintenance commands without exposing GitHub tokens", async () => {
    ...
    expect(scanReusable).not.toContain("1.18.29")
    expect(resolveReusable).not.toContain("1.18.29")
