...
`,
  ...
  test("preserves JSONC comments when force replacing plugin version", async () => {
    ...
    const cfg = path.join(tmp.path, ".hena", "hena.jsonc")
...
`,
  ...
  test("does not change configured package version without force", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".hena", "hena.json")
    await fs.mkdir(path.dirname(cfg), { recursive: true })
  ...
  test("writes to global scope when global flag is set", async () => {
    ...
    expect(await Filesystem.exists(path.join(global, "hena.jsonc"))).toBe(true)
  ...
  test("writes local scope under directory when vcs is not git", async () => {
    ...
    expect(await Filesystem.exists(path.join(worktree, ".hena", "hena.jsonc"))).toBe(false)
  })
  ...
  test("force replaces only the retained server config", async () => {
    ...
    const tui = path.join(tmp.path, ".hena", "tui.json")
