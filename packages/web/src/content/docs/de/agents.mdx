...
Es gibt zwei Arten von Agenten in Hena; Primäragenten und Subagenten.
...
Hena verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `hena.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Hena modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Hena-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Hena-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Hena Zen](/docs/zen) verwenden, würden Sie `hena/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `hena models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
