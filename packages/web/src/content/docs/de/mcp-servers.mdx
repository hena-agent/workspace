...
Sie können externe Tools zu Hena Agent hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Hena Agent unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Hena Agent Config](https://hena.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/hena-agent`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Hena Agent übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Hena Agent Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Hena Agent beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `hena-agent mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Hena Agent die Token sicher in `~/.local/share/hena-agent/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Hena Agent verfügbar. Sie können sie also wie jedes andere Tool über die Hena Agent-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Hena Agent mit Ihrem Sentry-Konto zu verbinden.
