...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  const chat = (yield* InstanceState.context).project.mode === "chat"
  ...
  if (!chat) yield* input.plugin.trigger(
  ...
  const henaProjectID = input.model.providerID.startsWith("hena")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("hena")
        ? {
            ...(henaProjectID ? { "x-hena-project": henaProjectID } : {}),
            "x-hena-session": input.sessionID,
            "x-hena-request": input.user.id,
            "x-hena-client": input.flags.client,
