...
Agentferdigheter lar Hena oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
hena søker etter disse stedene:

- Prosjektkonfigurasjon: `.hena/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/hena/skills/<name>/SKILL.md`
...
For prosjektlokale stier går hena opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.hena/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/hena/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.hena/skills/git-release/SKILL.md` slik:
...
hena viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `hena.json`:
...
**For innebygde agenter** (i `hena.json`):
