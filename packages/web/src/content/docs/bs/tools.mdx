...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Hena dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Hena.
...
:::note
Ovaj alat je dostupan samo kada je `HENA_EXPERIMENTAL_LSP_TOOL=true` (ili `HENA_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Hena provajdera ili kada je varijabla `HENA_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Hena:
