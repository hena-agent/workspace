...
Com o hena, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o hena usa nosso próprio tema `hena`.
...
O hena vem com vários temas embutidos.
...
- Querem que o hena corresponda à aparência do seu terminal
...
O hena suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/hena/themes/*.json` ou `$XDG_CONFIG_HOME/hena/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.hena/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.hena/themes/*.json`
