...
description: Crea strumenti che l'LLM puo chiamare in Hena Agent.
---

Gli strumenti personalizzati sono funzioni che crei e che l'LLM puo chiamare durante le conversazioni. Funzionano insieme agli [strumenti integrati](/docs/tools) di hena-agent come `read`, `write` e `bash`.
...
- In locale, mettendoli nella directory `.hena-agent/tools/` del progetto.
- Oppure in globale, mettendoli in `~/.config/hena-agent/tools/`.
