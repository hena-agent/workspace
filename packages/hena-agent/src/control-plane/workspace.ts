...
import { EventSequenceTable, EventTable } from "@hena-agent/core/event/sql"
...
import { WorkspaceEvent } from "@hena-agent/schema/workspace-event"
...
export class Service extends Context.Service<Service, Interface>()("@hena-agent/Workspace") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Workspace.create")(function* (input: CreateInput) {
      ...
      const env = {
        HENA_AGENT_AUTH_CONTENT: JSON.stringify(yield* auth.all()),
        HENA_AGENT_WORKSPACE_ID: config.id,
        HENA_AGENT_EXPERIMENTAL_WORKSPACES: "true",
