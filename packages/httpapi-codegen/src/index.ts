...
function normalizeTransport(
  ...
  const decoded: Schema.Top = Schema.toType(schema)
...
function responseSchemas(schema: Schema.Top, path: string): Array<readonly [string, Schema.Top]> {
  ...
  return [
    [`${path}.${schema.sseMode}`, Schema.make(value.ast)],
...
function streamSourceSchema(schema: Extract<HttpApiSchema.StreamSchema, { readonly _tag: "StreamSse" }>) {
  ...
  if (typeof fields === "object" && fields !== null && "data" in fields && Schema.isSchema(fields.data)) {
    ...
    if (Schema.isSchema(source)) return source
  }
...
function streamEffectPortable(schema: Schema.Top) {
  ...
  return sameEncoding(streamSourceSchema(schema).ast, streamDataSchema(schema).ast)
}
...
function renderGroup(group: Group, groupIndex: number) {
  ...
  function renderSuccess(schema: Schema.Top, name: string) {
    ...
    const value = addSlot(
      Schema.make((schema.sseMode === "data" ? streamDataSchema(schema) : schema.events).ast),
