...
description: Créez des outils que le LLM peut appeler dans Hena.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de hena comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.hena/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/hena/tools/`.
