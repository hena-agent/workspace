...
import type { IntegrationOAuthMethodRegistration } from "@hena-agent/plugin/v2/effect/integration"
import { define } from "@hena-agent/plugin/v2/effect/plugin"
import type { CredentialValue } from "@hena-agent/sdk/v2/types"
...
export const HenaAgentPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "hena-agent",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("HenaAgentPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("hena-agent")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Hena Agent provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("hena-agent", (integration) => {
        integration.name = "Hena Agent"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "hena-agent", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("hena-agent")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("hena-agent")
      ...
      const item = catalog.provider.get(ProviderV2.ID.henaAgent)
      if (!item) return
      const hasKey = Boolean(environment("HENA_AGENT_API_KEY") || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("hena-agent")),
