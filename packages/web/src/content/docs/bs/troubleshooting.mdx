...
Da biste otklonili probleme s Hena, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\hena\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `hena --log-level DEBUG`.

---

## Pohrana

Hena pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/hena/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\hena`
...
Hena Desktop pokreće lokalni Hena server (`hena-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Hena` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/hena/hena.jsonc` (ili `~/.config/hena/hena.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/hena/hena.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\hena\hena.jsonc`
...
Hena također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/hena/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\hena\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Hena mogao ponovo izgraditi.

1. Potpuno zatvorite Hena Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/hena`
- **Linux**: obrišite `~/.cache/hena` (ili pokrenite `rm -rf ~/.cache/hena`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\hena`

3. Ponovo pokrenite Hena Desktop.

---

### Rješavanje problema sa vezom na serveru

Hena Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `hena.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `HENA_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `HENA_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Hena Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Hena karakteristikama.

---

### Obavještenja se ne prikazuju

Hena Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Hena u postavkama vašeg OS-a, i
...
1. Zatvorite Hena Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Hena Desktop aplikacije):

- `hena.settings.dat` (URL zadanog servera za desktop)
- `hena.global.dat` i `hena.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `hena upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `hena models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\hena`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Hena dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\hena`

2. Ponovo pokrenite Hena da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Hena da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Hena će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
