---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Hena Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

Hena Agent verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Hena Agent-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Hena Agent anpassen
config.
...
Hena Zen ist eine Liste von Modellen, die vom Hena Agent-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Hena Agent funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Hena Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Hena Agent aus und gehen Sie zu [hena.dev/auth](https://hena.dev/auth).
...
Es funktioniert wie jeder andere Anbieter in Hena Agent und ist völlig optional.

---

## Hena Go

Hena Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Hena Agent-Team getestet und verifiziert wurden, dass sie gut mit Hena Agent funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Hena Go` und gehen Sie zu [hena.dev/auth](https://hena.dev/zen).
...
So verwenden Sie Amazon Bedrock mit Hena Agent:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Hena Agent ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `hena-agent.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Hena Agent wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können hena-agent so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Hena Agent ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Hena Agent-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `Hena Agent`, Bereiche: `api`
...
um das Token im Hena Agent-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Hena Agent verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Hena Agent
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`hena-agent.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Hena Agent:
...
So verwenden Sie Google Vertex AI mit Hena Agent:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Hena Agent ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Hena Agent konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede Hena Agent-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Hena Agent-Konversation als separate Sitzung aufgeführt.
...
Sie können Hena Agent für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Hena Agent über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Hena Agent über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Hena Agent konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit Hena Agent:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Hena Agent.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Hena Agent verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Hena Zen ist eine Liste getesteter und verifizierter Modelle, die vom Hena Agent-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Hena Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Hena Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Hena Agent API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Hena Agent-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Hena Agent-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Hena Agent:
...
Sie können Modelle auch über Ihre Hena Agent-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Hena Agent verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `hena-agent.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Hena Agent erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `hena-agent auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Hena Agent-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Hena Agent-Konfiguration übereinstimmt.
