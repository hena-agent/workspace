...
Her er alle de innebygde verktøyene som er tilgængelige i Hena.
...
:::note
Dette verktøyet er bare tilgængelig når `HENA_EXPERIMENTAL_LSP_TOOL=true` (eller `HENA_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Hena-leverandøren eller når miljøvariabelen `HENA_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
