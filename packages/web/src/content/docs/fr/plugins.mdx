---
title: Plugins
description: Écrivez vos propres plugins pour étendre Hena.
---

Les plugins vous permettent d'étendre Hena en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Hena.
...
- `.hena/plugins/` - Plugins au niveau du projet
- `~/.config/hena/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/hena/node_modules/`.
...
1. Configuration globale (`~/.config/hena/hena.json`)
2. Configuration du projet (`hena.json`)
3. Répertoire global des plugins (`~/.config/hena/plugins/`)
4. Répertoire des plugins du projet (`.hena/plugins/`)
...
Hena exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK hena pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre Hena.
...
:::note
Si vous utilisez l'application de bureau Hena, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez hena de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à hena :
...
L'assistant `tool` crée un outil personnalisé que hena peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour hena aux côtés des outils intégrés.
