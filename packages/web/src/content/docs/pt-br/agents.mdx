...
Existem dois tipos de agentes no hena; agentes primários e subagentes.
...
hena vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Hena vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Hena vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Hena, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `hena.json`:
...
- Global: `~/.config/hena/agents/`
- Por projeto: `.hena/agents/`

```markdown title="~/.config/hena/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o hena usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do hena quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do hena usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Hena Zen](/docs/zen), você usaria `hena/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `hena models` para ver uma lista dos modelos disponíveis.
:::
...
```markdown title="~/.config/hena/agents/docs-writer.md"
---
...
```markdown title="~/.config/hena/agents/security-auditor.md"
---
