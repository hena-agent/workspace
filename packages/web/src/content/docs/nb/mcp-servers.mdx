...
Du kan legge til eksterne verktøy i Hena ved å bruke _Model Context Protocol_ eller MCP. Hena støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Hena Config](https://hena.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/hena`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Hena håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Hena:
...
Hvis serveren krever autentisering, vil Hena be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `hena mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Hena lagre tokens sikkert i `~/.local/share/hena/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Hena, sammen med innebygde verktøy. Så du kan administrere dem gjennom Hena-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Hena til Sentry-kontoen din.
