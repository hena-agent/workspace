...
Puoi aggiungere strumenti esterni a Hena Agent usando il _Model Context Protocol_, o MCP. Hena Agent supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Hena Agent Config](https://hena.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/hena-agent`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Hena Agent gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Hena Agent:
...
Se il server richiede autenticazione, Hena Agent ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `hena-agent mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Hena Agent memorizzera' i token in modo sicuro in `~/.local/share/hena-agent/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Hena Agent insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Hena Agent come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Hena Agent al tuo account Sentry.
