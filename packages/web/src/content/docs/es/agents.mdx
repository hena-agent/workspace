...
Hay dos tipos de agentes en Hena Agent; agentes primarios y subagentes.
...
Hena Agent viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Hena Agent viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

Hena Agent viene con dos agentes primarios integrados y tres subagentes integrados.
...
Un agente de solo lectura para investigar documentación externa y dependencias. Úsalo cuando necesites clonar el repositorio de una dependencia en la caché administrada de Hena Agent, inspeccionar el código fuente de una librería o contrastar el código local con implementaciones upstream sin modificar tu espacio de trabajo.
...
Configure agentes en su archivo de configuración `hena-agent.json`:
...
- Por proyecto: `.hena-agent/agents/`
...
Si no se especifica ninguna temperatura, Hena Agent utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Hena Agent como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Hena Agent usa el formato `provider/model-id`. Por ejemplo, si usa [Hena Zen](/docs/zen), usaría `hena-agent/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `hena-agent models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/hena-agent/hena).
:::
