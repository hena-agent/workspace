...
export function exactOriginCors(origins: readonly string[]) {
  ...
  const middleware = cors({
    origin: (origin, context) => allowed.has(origin) || origin === new URL(context.req.url).origin ? origin : "",
  ...
  return async (context: Parameters<typeof middleware>[0], next: Parameters<typeof middleware>[1]) => {
    ...
    if (origin && origin !== new URL(context.req.url).origin && !allowed.has(origin))
