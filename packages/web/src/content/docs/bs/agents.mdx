...
Postoje dvije vrste agenata u Hena: primarni agenti i podagenti.
...
Hena dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Hena dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Hena dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Hena-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `hena.json`:
...
- Globalno: `~/.config/hena/agents/`
- Po projektu: `.hena/agents/`
...
Ako temperatura nije navedena, Hena koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Hena konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Hena konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Hena Zen](/docs/zen), koristili biste `hena/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `hena models` da vidite listu dostupnih modela.
:::
