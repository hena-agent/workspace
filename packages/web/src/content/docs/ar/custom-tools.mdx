...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في Hena Agent.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في hena-agent مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.hena-agent/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/hena-agent/tools/`.
