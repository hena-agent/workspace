...
_Model Bağlam Protokolü_ veya MCP kullanarak hena-agent'a harici araçlar ekleyebilirsiniz. hena-agent hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [hena-agent Config](https://hena.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/hena-agent` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
hena-agent, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde hena-agent şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa hena-agent, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `hena-agent mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından hena-agent, belirteçleri `~/.local/share/hena-agent/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra hena-agent'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi hena-agent yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve hena-agent'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
