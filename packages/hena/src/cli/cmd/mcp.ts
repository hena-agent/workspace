...
import { ConfigMCPV1 } from "@hena/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: hena mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in hena.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .hena/ subdirectory too)
  const candidates = [path.join(baseDir, "hena.json"), path.join(baseDir, "hena.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".hena", "hena.json"), path.join(baseDir, ".hena", "hena.jsonc"))
  }
  ...
  // Default to hena.json if none exist
  return candidates[0]
}
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "hena-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
