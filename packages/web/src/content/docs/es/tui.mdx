---
title: TUI
description: Usando la interfaz de usuario del terminal Hena Agent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hena Agent proporciona una interfaz de terminal interactiva o TUI para trabajar en sus proyectos con un LLM.

Al ejecutar Hena Agent se inicia TUI para el directorio actual.
...
Cuando utilice Hena Agent TUI, puede escribir `/` seguido de un nombre de comando para ejecutar acciones rápidamente. Por ejemplo:
...
Agregue un proveedor a Hena Agent. Le permite seleccionar entre proveedores disponibles y agregar sus claves API.
...
Salga de Hena Agent. _Alias_: `/quit`, `/q`
...
Esto es independiente de `hena-agent.json`, que configura el comportamiento del servidor/tiempo de ejecución.
...
- `leader_timeout`: controla cuánto tiempo espera Hena Agent después de la leader key. El valor predeterminado es `2000`.
...
- `sound_pack`: ID del sound pack que se usará. El valor predeterminado es `hena-agent.default`.
